...
if $args[0] = 'list':
	...
	if $car['fuel'] >= 3:
		if $car['location'] ! 'center': $_str += '<a href="exec:gs ''car'',''drive'',''center''">Ехать в Центр</a><br/>'
		if $car['location'] ! 'south': $_str += '<a href="exec:gs ''car'',''drive'',''south''">Ехать в Южный район</a><br/>'
		if $car['location'] ! 'lake': $_str += '<a href="exec:gs ''car'',''drive'',''lake'',''start''">Ехать к озеру</a><br/>'
		if $car['location'] ! 'zz_park': $_str += '<a href="exec:$control_point = ''south'' & gs ''car'',''drive'',''zz_park''">Ехать в городской парк</a><br/>'
		if $car['location'] ! 'north': $_str += '<a href="exec:gs ''car'',''drive'',''north''">Ехать в Северный район</a><br/>'
