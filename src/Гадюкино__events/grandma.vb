...
if $loc = 'Gadhouse':
	...
	if month < 4 or month > 9 or sunWeather < 0:
		if hour > 7 and hour < 13:
			gs 'zz_render','','images/qwest/gadukino/new/grandma_cooking'+rand(1,5)+'.jpg'
		elseif hour > 13 and hour < 18:
			gs 'zz_render','','images/qwest/gadukino/new/grandma_knits.jpg'
		end
	end
elseif $loc = 'Gadgarden':
	gs 'zz_render','','images/qwest/gadukino/new/grandma_gadgarden.jpg'
elseif $loc = 'Gadsarai':
	if hour = 19:
		gs 'zz_render','','images/qwest/gadukino/new/grandma_cow.jpg'
	else
		gs 'zz_render','','images/qwest/gadukino/new/grandma_gadsarai.jpg'
	end
elseif $loc = 'gadukino':
	gs 'zz_render','','images/qwest/gadukino/new/grandma_gadukino.jpg'
elseif $loc = 'gadmarket':
	gs 'zz_render','','images/qwest/gadukino/new/grandma_gadmarket.jpg'
else
	gs 'zz_render','','images/qwest/gadukino/grandma.jpg'
end
gs 'zz_render','','','Ваша бабушка много лет проработала дояркой в колхозе, но после его развала осталась без работы. И уже много лет занимается домашним хозяйством - многочисленной живностью и садом-огородом. Деревенская нелёгкая жизнь рано её состарила. Хотя она и чуть моложе деда, но выглядит настоящей "старушкой".'
...
if cloth[0] = 0:
	grandma_notalk = 1
	grandmaQW -= 10
	if grandmaknownude > 0:' - Света, опять за старое принялась? Ты чего опять голая ходишь? А ну марш домой одеваться, пока дубца не получила!!!.'
	if grandmaknownude = 0:grandmaknownude += 1 & ' - Света, ты что здесь делаешь в таком виде? А ну марш домой одеваться!Не хватало, чтобы вся деревня увидела, что ты тут голышом разгуливаешь, стыда потом не оберешься!.'
end
if cumlip + cumface + cumfrot > 0:
	grandma_notalk = 1
	grandmaQW -= 10
	if grandmaQW < 40:
		gs 'zz_render','','','Бабушка посмотрела на вас, выпучила глаза и покраснела. На вас была заметна сперма.'
		...
		if grandma_know = 0:
			grandma_know = 1
			gs 'zz_render','','','Бабушка присмотрелась к вам внимательным взглядом. На вас была заметна сперма. Она ойкнула и запричитала.'
			gs 'zz_render','','','- Светуля, внучка, что такое на тебе? Что же это делается? Я то думаю, что у меня внучка приличная, а оно вот как выходит. Быстро приведи себя в порядок, пока еще кто-нибудь не увидел!'
		else
			gs 'zz_render','','','Бабушка строго посмотрела на вас. На вас была заметна сперма..'
			gs 'zz_render','','','- Света, опять ты за своё? Быстро приведи себя в порядок, пока еще кто-нибудь не увидел!'
		end
	end
end
if alko > 0 and age < 20:
	...
	if alko < 3:
		grandmaQW -= 1
		gs 'zz_render','','','Онa принюхалась:'
		gs 'zz_render','','',' - Света, от тебя спиртным пахнет! Что за безобразие? Ты еще слишком молода, чтобы пить! Иди приведи себя в порядок.'
	elseif alko >= 3 and alko < 6:
		grandmaQW -= 5
		gs 'zz_render','','','Она принюхалаь и внимательно глянула на вас:'
		gs 'zz_render','','',' - Господи, Света, ты пьяная? Ты же еще так молода, как тебе не стыдно! Иди приведи себя в порядок, сейчас же!'
	elseif alko >= 6:
		grandpaQW -= 5
		minut += 20
		gs 'zz_bathroom', 'clean_me'
		gs 'zz_render','','','Она внимательно глянула и грозно посмотрела на вас:'
		gs 'zz_render','','',' - Света ты что, напилась? Господи ты, боже мой! Что же это делается такое!'
		gs 'zz_render','','','Бабушка ещё что-то говорила и причитала, но вы слабо понимали, что она говорит и делает. Спустя пять минут причитаний, она обняла вас за плечи и подвела в баню, где хорошенько умыла вас. После бабушка отвела вас в избу и уложила в кровать.'
	end
end
if grandmahelpday ! daystart and grandma_notalk = 0 and cloth[0] > 0 and grandmaknowsick = 0 and alko = 0:act 'Предложить помощь':grandmahelpday = daystart & gt'grandmahelp'
if grandmatalkgad = 0 and grandma_notalk = 0 and cloth[0] > 0 and alko = 0:
	act 'Спросить про деревню':
		...
		*clr
		...
		gs 'stat'
		gs 'zz_render','','images/qwest/gadukino/grandma.jpg'
		gs 'zz_render','','','Вы сели рядом с бабушкой, решив узнать побольше о деревне.'
		gs 'zz_render','','',' - А ты что не помнишь, Светуля? Ты же бывала не раз у нас в деревне, - удивилась бабушка.'
		gs 'zz_render','','',' - Так я ж мелкая была, дома в основном сидела, да с Митькой на речку купаться бегали, - ответили вы.'
		gs 'zz_render','','',' - А сейчас уже крупная стала! Поглядите-ка на неё, - засмеялась бабушка. - Ну ладно, слушай.'
		gs 'zz_render','','','И начала рассказывать про деревню.'
		gs 'zz_render','','',' - Деревня у нас небольшая, ну ты это и сама знаешь. Про речку и пляж рассказывать не буду, помнишь же, что с Митькой там купались и загорали. Еще там рыбаков можно встретить, если Степка будет, может дать и удочку порыбачить.'
		...
		gs 'zz_render','','',' Если любишь спортом заниматься, то можно побегать по дороге, ведущей в лес. Или на нашем старом велосипеде покататься. Кстати поищи его в сарае. А если за фигурой следишь, то сходи в сад, покушай фруктов, они как ваша городская еда диетическая, если не лучше.'
		gs 'zz_render','','',' Ну все внучка, беги гуляй, не отвлекай меня. Сейчас десять тысяч стопятьдесят восьмая серия "Санта-Барбары" начнется, не хочу пропустить.'
...
if $loc = 'gadmarket' and mesec > 0 and tampon < 5 and alko = 0:
	act 'Попросить купить тампоны':
		...
		*clr
		...
		gs 'stat'
		gs 'zz_render','','images/qwest/gadukino/grandma.jpg'
		gs 'zz_render','','','Увидев бабушку в магазине вы подошли к ней.'
		gs 'zz_render','','',' - Бабуль, - начали вы, отчаянно краснея. - У меня тут .... ээээ... женские дела, короче. Не могла бы ты мне тампонов купить?.'
		if grandmaQW < 20: ' - Нет Света, - ответила она. - Ты сначала научись вести себя нормально, тогда и другие тебе тем же отвечать будут. А пока выпутывайся сама как хочешь.'
		if grandmaQW >= 20 and grandmaQW < 40:
			gs 'zz_render','','',' - Во, как что-то нужно, так сразу "бабуля". А как хорошо вести себя, так мы не умеем, - проворчала она. - Ладно уж, держи, что с тобой поделаешь.'
			gs 'zz_render','','','И заплатив продавщице протянула вам упаковку тампонов.'
		end
		if grandmaQW >= 20:	' - Конечно Светуля, держи, - сказала бабушка и заплатив продавщице протянула вам упаковку тампонов..'
...
if sick > 0 and grandmaknowsick = 0 and alko = 0:
	act 'Сказать, что вы заболели':
		...
		*clr
		...
		gs 'stat'
		gs 'zz_render','','images/qwest/gadukino/grandma.jpg'
		gs 'zz_render','','','Вы подошли к бабушке и сказали, что плохо себя чувствуете. Она внимательно осмотрев вас и потрогав лоб воскликнула:'
		gs 'zz_render','','',' - Господи, да ты заболела! А ну-ка выпей таблетку и марш в постель, чтобы на улице я тебя не видела.'
		act 'Уйти':gt 'Gadhouse'
	end
end
if grandmatalk ! hour and grandma_notalk = 0 and cloth[0] > 0 and alko = 0:
	act 'Поболтать':
		*clr & cla
		grandmatalk = hour
		gs 'stat'
		...
		if $loc = 'Gadhouse' or $loc = 'gadukino' or $loc = 'gadmarket':
			...
			minut += 1
			manna -= 5
			gs 'zz_render','','','- Нет времени внучка, работать нужно.'
		end
		act 'Закончить':gt $loc, $metka
	end
end
if grandmastory ! daystart and grandma_notalk = 0 and cloth[0] > 0 and alko = 0:
	act 'Попросить рассказать историю':
		*clr & cla
		grandmastory = daystart
		gs 'stat'
		gs 'zz_render','','images/qwest/gadukino/'+iif($loc = 'Gadhouse','Babushka','grandma')+'.jpg'
		if $loc = 'Gadhouse' or $loc = 'gadukino':
			minut += 20
			manna += rand(5,10)
			temp = RAND(0,7)
			if grandmaQW < 80:grandmaQW += 1
			gs 'zz_render','','','Вы просите свою бабушку рассказать вам какую-нибудь из ее многочисленных историй и она, немного поворчав, все же охотно соглашается...'
			if temp = 0:
...
Мужик с саней спрыгнул и бежать бросился в сторону деревни, прямо по сугробам и бежал, пока головой в церковь, что в его деревне стоит, не врезался. На следующее утро на службу в церковь пришел, батюшке покаялся. Лошадь его так и не нашли, а самого мужика через десять дней нашли в колодце... мертвого...'
			...
			elseif temp = 6:
				gs 'zz_render','','','- Помню, сидели мы как-то, начинает бабуля, но потом, словно что-то вспомнив, горестно вздыхает. - А спичек-то больше нет...
...
Этот страх, этот ужас, который я испытала тогда, в детском, ещё дошкольном возрасте, запал в мою душу настолько, что я незабуду его до конца моей жизни...'
			...
			minut += 1
			manna -= 5
			gs 'zz_render','','','- Не время сейчас внучка, потом расскажу.'
		end
		act 'Закончить':gt $loc, $metka
	end
end
act 'Уйти':
	if alko < 6 and grandmaknowsick = 0 and cloth[0] > 0:gt $loc, $metka
	if alko < 6 and grandmaknowsick = 1 or cloth[0] = 0:gt 'Gadhouse'
