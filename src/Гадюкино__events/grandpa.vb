...
if $loc = 'Gadgarden':
	if month = 5 and month = 9:
		gs 'zz_render','','images/qwest/gadukino/new/grandpa_gadgarden.jpg'
	else
		gs 'zz_render','','images/qwest/gadukino/new/grandpa_fruit_garden.jpg'
	end
elseif $loc = 'Gaddvor':
	gs 'zz_render','','images/qwest/gadukino/new/grandpa_gaddvor.jpg'
elseif $loc = 'Gadhouse' and (hour = 7 or hour = 13 or hour = 18):
	gs 'zz_render','','images/qwest/gadukino/grandpa1.jpg'
elseif $loc = 'Gadsarai':
	gs 'zz_render','','images/qwest/gadukino/new/grandpa_gadsarai.jpg'
elseif $loc = 'gadukino':
	if week = 6:
		gs 'zz_render','','images/qwest/gadukino/new/grandpa_gadukino1.jpg'
	else
		gs 'zz_render','','images/qwest/gadukino/new/grandpa_gadukino2.jpg'
	end
else
...
if cloth[0] = 0:
	...
	grandpaQW -= 5
	grandmaknownude += 1
	gs 'zz_render','','',' - Света, ты чего в таком виде разгуливаешь? Я ведь мужик, хоть и твой дед. А ну марш одеваться, пока бабка не увидела!.'
end
if cumface or cumfrot > 0:
	grandpa_notalk = 1
	grandpaQW -= 10
	if grandpaQW < 40:
		gs 'zz_render','','','Дед посмотрел на вас гневным взглядом. На вас была заметна сперма.'
		if grandpa_know = 0:grandpa_know = 1 & '- Света, это что такое на тебе? Я думаю, что у меня внучка приличная, а она ебется где-то. Пошла отсюда, глаза б мои тебя не видели!'
		...
		gs 'zz_render','','','Дед присмотрелся к вам внимательным взглядом. На вас была заметна сперма. Он нахмурился, но промолчал, и покачав головой пошел прочь.'
	end
end
if alko > 0 and age < 20:
	...
	if alko >= 3 and alko < 6:
		grandpaQW -= 1
		grandpa_notalk = 1
		gs 'zz_render','','','Он принюхался и внимательно глянул на вас :'
		gs 'zz_render','','','-Света, да ты никак пьяная? Ты же еще так молодая совсем, зачем тебе это нужно? Иди приведи себя в порядок, пока бабка не увидела!'
	end
	if alko >= 6:
		grandpaQW -= 5
		grandpa_notalk = 1
		gs 'zz_render','','','Он глянул на вас и удивлённо приподнял брови:'
		gs 'zz_render','','','-Света, да ты напилась как сапожник! Иди-ка ты отдохни, пока бабка тебя, такую красивую, не увидела!'
	end
end
if grandpahelpday ! daystart and grandpa_notalk = 0 and cloth[0] > 0 and grandmaknowsick = 0 and alko < 3:act 'Предложить помощь':grandpahelpday = daystart & gt'grandpahelp'
if grandpatalkforest = 0 and grandpa_notalk = 0 and cloth[0] > 0 and alko < 3:
	act 'Спросить про лес':
		...
		*clr
		...
		gs 'zz_render','','',' Если все таки заблудилась внучка, то по темноте лучше костер разведи, да посиди на одном месте до утра. Зверь лесной огня-то боится, близко даже не подходит.'
		gs 'zz_render','','',' Да вот, еще... Ты в лес особо не наряжайся, попортишь всю свою красивую одежку. В лес лучше ходить в этих.... как их... в жинсах. Во!.'
		gs 'zz_render','','',' Ну все Света, беги гуляй, все что знал я рассказал.'
		act 'Закончить':gt $loc, $metka
	end
end
if hour >= grandpatalk + 3 and grandpa_notalk = 0 and cloth[0] > 0 and alko < 3:
	act 'Поболтать':
		cla
		*clr
		grandpatalk = hour
...
act 'Уйти':
	if alko < 6 and cloth[0] > 0:gt $loc, $metka
	if alko < 6 and cloth[0] = 0:gt 'Gadhouse'
