if $ARGS[0] = 'mira_in_river':
	...
	if mira_go_blok = 0 and mirainriver = 1 and (i = 5 or hour = 20):
		*clr & cla
		...
		mira_no_home = 0
		gs 'stat'
		gs 'zz_render', $_str, 'gadukino/river/events/mira_go_home'+iif(cloth[0] = 0,'_nude','')+'.jpg', ' - Светик, мне пора домой, - сказала вам Мира и стала собираться.'
	...
	if miraQW > 0 and inriver = 1 and Mira_no = 0 and mirainriver = 0 and i > 7 and hour > 8 and hour < 20 and sunWeather >= 0 and temper >= 20:
		*clr & cla
		...
		mira_go_blok = 2
		gs 'stat'
		gs 'zz_render', $_str, 'gadukino/river/events/mira_go_river'+iif(cloth[0] = 0,'_nude','')+'.jpg', 'Вы загорали, когда на речке появилась Мира.'+iif(cloth[0] = 0 and miratalkforest < 3 and shameless['flag'] = 0,' Покраснев, вы быстренько натянули свой купальник, и улеглись загорать дальше.','')+' Улыбнувшись вам, она разделась и принялась растилать свое покрывало рядом с вашим.'
		if cloth[0] = 0 and miratalkforest < 3 and shameless['flag'] = 0: gs 'zz_clothing2','redress',2
		act 'Далее': gt 'gadbeach'
	end
...
if $ARGS[0] = 'sunbathe_guys':
	...
	if guysriver ! day:
		...
		elseif mirainriver = 1 and cloth[0] = 2 and miragopQW >= 15 and i = 1 and hour >= 8 and hour < 20:
			*clr & cla
			minut += 15
			horny += 20
			dom += 5
...
if $ARGS[0] = 'guys_beach':
	*clr & cla
	...
	i = rand(1,3)
	gs 'stat'
	gs 'zz_render', $_str, 'gadukino/river/events/gays_river.jpg',' - О, Светик, нам-то как раз тебя и не хватало, - заулыбался Колямба, увидев вас на пляже.'
	if i = 1:
		...
		act 'Обслужить Колямбу':
			*clr & cla
			...
			i = RAND(0,2)
			gs 'stat'
			gs 'zz_render', $_str, 'gadukino/river/guys/8.gif','Вы быстро '+iif(cloth[1] > 0,'сняли трусы и ','')+'залезли на умостившегося Колямбу, рукой нащупывая его член.'
			gs 'zz_dynamic_sex', 'sex_start'
			gs 'zz_dynamic_sex', 'vaginal', 'dick'
			gs 'zz_dynamic_sex', 'sex_cum'
		...
		act 'Вымыть коня':
			*clr & cla
			...
			elseif cloth[0] = 0:
				...
				gs 'zz_clothing2', 'decrease_condition', 1
			end
			act 'Закончить':gt 'gadbeach'
		end
	end
		...
		act 'Отвести коня домой':
			if cloth[0] > 2 or (cloth[0] >= 0 and exhibi > 0):
				*clr & cla
				...
				inriver = 0
				gs 'stat'
				gs 'zz_render', $_str, 'images/gadukino/horse/lead'+iif(cloth[0] = 0,'_nude',iif(cloth[0] = 2,'_swim',''))+'.jpg', 'Вы взяли коня под уздцы и отправились с ним домой.'
				act 'Далее':
					killvar '$_str'
					gt'Gadsarai'
				end
				...
				*clr & cla
				gs 'stat'
				gs 'zz_render', $_str, 'images/gadukino/horse/lead_'+iif(cloth[0] = 0,'nude','swim')+'.jpg', '<b>Вы не можете появиться в таком виде в деревне.</b>'
