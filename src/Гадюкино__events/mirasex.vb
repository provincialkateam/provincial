if $ARGS[0] = 'miralick_first':
	*clr & cla
	...
	gs'time'
	if cloth[1] = 1:
		gs 'zz_render','','images/qwest/gadukino/new/mira_start_tanga1.jpg'
		gs 'zz_render','','','Опустившись на колени, Мира несмело потянулась к вашим трусам, глядя вам в глаза и робко улыбаясь. Вы, облокотившись на дерево и прикрыв глаза, стали ждать продолжения...'
	else
		gs 'zz_render','','images/qwest/gadukino/new/mira_start_tanga0.jpg'
		gs 'zz_render','','','Опустившись на колени и не обнаружив на вас трусов, Мира несмело коснулась языком вашей вагине, как будто пробовала на вкус, вызвав у вас непроизвольный стон наслаждения. Вы, облокотившись на дерево и прикрыв глаза, стали ждать продолжения...'
	end
...
if $ARGS[0] = 'mirakiss':
	*clr & cla
	...
	gs 'stat'
	gs 'zz_render','','images/qwest/gadukino/new/mirakiss'+rand(1,4)
	temp = rand(1,4)
...
if $ARGS[0] = 'foreplay':
	*clr & cla
	...
	gs 'stat'
	if horny >= mirahorny:
		gs 'zz_render','','images/qwest/gadukino/new/miraforeplay'+rand(1,4)
		...
		gs 'zz_render','','images/qwest/gadukino/new/miraforeplay'+rand(5,7)
		horny += 10
		temp = rand(1,3)
...
if $ARGS[0] = 'miralick_start':
	*clr & cla
	...
	gs'time'
	if cloth[1] = 1:
		gs 'zz_render','','qwest/gadukino/new/mira_start_tanga1.jpg'
		gs 'zz_render','','','Опустившись на колени, Мира потянулась к вашим трусам с улыбкой на лице, заглядывая вам в глаза. Вы, облокотившись на дерево и прикрыв глаза, стали ждать продолжения...'
	else
		gs 'zz_render','','qwest/gadukino/new/mira_start_tanga0.jpg'
		gs 'zz_render','','','Опустившись на колени и не обнаружив на вас трусов, Мира лизнула языком вашу <<$vaginatipe2>> вагину, пробуя её вкус, вызвав у вас непроизвольный стон наслаждения. Вы, облокотившись на дерево и прикрыв глаза, стали ждать продолжения...'
	end
	if cloth[1] = 1:
	...
	if cloth[1] = 0:act 'Стонать':gt 'mirasex','miralick_stand2'
end
if $ARGS[0] = 'lickmira_first':
	*clr & cla
	...
	gs 'stat'
	gs'time'
	gs 'zz_render','','qwest/gadukino/new/lickmira_start.jpg'
	if lesbian < 10:
		gs 'zz_render','','','Опустившись на колени, вы осторожно прикоснулись к трусам Миры и принялись неловко их стягивать с неё. Она, сгорая от возбуждения, приподняла попку с камня, решив помочь вам и убыстряя процесс...'
	else
		gs 'zz_render','','','Опустившись на колени, вы потянулись к трусам Миры и принялись привычным движением их стягивать с неё. Она, сгорая от возбуждения, приподняла попку с камня, решив помочь вам и убыстряя процесс...'
	end
...
if $ARGS[0] = 'horny_talk':
	*clr & cla
	...
	gs 'stat'
	gs'time'
	gs 'zz_render','','qwest/gadukino/new/horny_talk.jpg'
	gs 'zz_render','','','Мира поймав ваш взгляд усмехнулась. Подойдя к вам в плотную, она развернула вас спиной к себе и занув руку в ваши трусики воскликнула:'
	if horny >= 80:
		gs 'zz_render','','',' - О, да тут целый водопад!'
	else
		gs 'zz_render','','',' - Смотри ты, сучка потекла!'
	end
	gs 'zz_render','','','Поелозив в ваших трусах еще пару мгновений, Мира сказала:'
	if miraQW < 15:
		gs 'zz_render','','',' - Всё, вали отсюда, шлюшка, некогда мне с тобой заниматься.'
		gs 'zz_render','','','И напевая под какую-то песенку отправилась домой. Вы посмотрев ей в след пару мгновений, тоже пошли в сторону своего дома.'
	else
		gs 'zz_render','','',' - Ладно, становись раком, помогу своей шлюшке.'
		gs 'zz_render','','','Вы с готовностью стянули свои трусики и бухнулись на колени, становясь перед ней раком.'
	end
...
if $ARGS[0] = 'miralick_orgasm':
	if horny >= 100:
		...
		manna += 15
		you_orgasm = 1
		gs 'zz_render','','','Неожиданно для вас Отлизывая Мире вы начали кончать сами. Между ног полыхал пожар и волны удовольствия охватывали тело, вы стонали, но лизать не прекращали.'
	end
end
if $ARGS[0] = 'lickmira_mis1':
	*clr & cla
	...
	gs 'stat'
	if mirahorny < 100:
		gs 'zz_render','','qwest/gadukino/new/lickmira_mis'+rand(1,3)
		...
		miraorgasm = 1
		mirahorny = 0
		gs 'zz_render','','qwest/gadukino/new/lickmira_mis_cum.jpg'
		$textmis[rand(1,3)]+$textlick[iif(lesbian < 10,rand(7,9),rand(10,12))]+$text_emotions[7]+$text_cum[2]
		gs 'mirasex','miralick_orgasm'
		if horny < 60:
			gs 'zz_render','','','Отойдя от оргазма, Мира разжала свои ноги, выпустив вашу голову из плена. Стерев её соки со своего лица и приведя себя себя в порядок, вы отправились домой.'
		else
			gs 'zz_render','','','Отойдя от оргазма, Мира разжала свои ноги, выпустив вашу голову из плена. Стерев её соки со своего лица и приведя себя себя в порядок, вы посмотрели на Миру.'
		end
	end
	killvar '$textmis'
	killvar '$textlick'
...
if $ARGS[0] = '69_1':
	*clr & cla
	...
	if mirahorny < 100 and horny < 100:
		gs 'zz_render','','qwest/gadukino/new/lickmira_69_'+rand(1,3)+'.jpg'
	...
	elseif mirahorny >= 100 and horny < 100:
		miraorgasm = 1
		mirahorny = 0
		gs 'zz_render','','qwest/gadukino/new/lickmira_69_cum2.jpg'
		...
		gs 'zz_render','','','Отойдя от оргазма, Мира разжала свои ноги, выпустив вашу голову из плена.'
	elseif mirahorny < 100 and horny >= 100:
		orgasm += 1
		you_orgasm = 1
		horny = 0
		gs 'zz_render','','images/qwest/gadukino/new/lickmira_69_cum1.jpg'
		...
		gs 'zz_render','','','Через некоторое время, отойдя от оргазма, вы улыбнулись Мире и продолжили...'
	else
		orgasm += 1
		you_orgasm = 1
		horny = 0
		miraorgasm = 1
		mirahorny = 0
		gs 'zz_render','','images/qwest/gadukino/new/lickmira_69_cum'+rand(1,2)+'.jpg'
		...
		gs 'zz_render','','','Через некоторое время, отойдя от оргазма, вы с Мирой привели себя в проядок и счастливые отправились домой...'
	end
	killvar '$text69'
	killvar '$textlick'
	killvar '$text_emotions'
...
if $ARGS[0] = '69_2':
	*clr & cla
	...
	gs 'stat'
	if mirahorny < 100 and horny < 100:
		gs 'zz_render','','qwest/gadukino/new/lickmira_69_'+rand(1,3)
	...
	elseif mirahorny < 100 and horny >= 100:
		...
		you_orgasm = 1
		horny = 0
		gs 'zz_render','','qwest/gadukino/new/lickmira_69_cum1.jpg'
		...
		miraorgasm = 1
		mirahorny = 0
		gs 'zz_render','','qwest/gadukino/new/lickmira_69_cum'+rand(1,2)
		...
		gs 'zz_render','','','Через некоторое время, отойдя от оргазма, вы с Мирой привели себя в проядок и счастливы отправились домой...'
	end
	killvar '$text69'
	killvar '$textlick'
	killvar '$text_emotions'
...
if $ARGS[0] = 'miralick_stand1':
	*clr & cla
	...
	gs'time'
	if horny < 100:
		gs 'zz_render','','qwest/gadukino/new/miralick_stand'+rand(1,2)
		$textstand[rand(1,3)]+$textlick[iif(miralick < 10,rand(1,3),rand(4,6))]+$text_emotions[iif(horny < 60,1,iif(horny < 80,2,3))]
	else
		orgasm += 1
		you_orgasm = 1
		horny = 0
		gs 'zz_render','','qwest/gadukino/new/miralick_stand_cum.jpg'
		$textstand[rand(1,3)]+$textlick[iif(miralick < 10,rand(1,3),rand(4,6))]+$text_emotions[3]+$text_cum[1]
		gs 'zz_render','','','Ноги вас не держат и вы сползаете вниз, не в состоянии шевельнуться. Через некоторое время, отойдя от оргазма, вы привели себя в порядок и отправились домой.'
	end
	killvar '$textstand'
	killvar '$textlick'
	killvar '$text_emotions'
...
if $ARGS[0] = 'miralick_dog':
	*clr & cla
	...
	gs'time'
	if horny < 100:
		gs 'zz_render','','qwest/gadukino/new/miralick_dog'+rand(1,2)
		$textdog[rand(1,3)]+$textlick[iif(miralick < 10,rand(1,3),rand(4,6))]+$text_emotions[iif(horny < 60,1,iif(horny < 80,2,4))]
	else
		orgasm += 1
		you_orgasm = 1
		horny = 0
		gs 'zz_render','','qwest/gadukino/new/miralick_dog_cum.jpg'
...
if $ARGS[0] = 'mirafinger':
	*clr & cla
	...
	gs'time'
	if horny < 100:
		gs 'zz_render','','qwest/gadukino/new/mirafinger_dog'+rand(1,2)
		$textdog[4]+$textfinger[rand(1,3)]+$text_emotions[iif(horny < 60,1,iif(horny < 80,2,4))]
	else
		orgasm += 1
		you_orgasm = 1
		horny = 0
		gs 'zz_render','','qwest/gadukino/new/mirafinger_dog_cum.jpg'
		$textdog[4]+$textfinger[rand(1,3)]+$text_emotions[4]+$text_cum[1]
		gs 'zz_render','','','Тело вас не слушается и вы плюхаетесь на живот, не в состоянии пошевелиться. Мира ехидно взглянула на вас и дав облизать свои пальцы, чтобы очистить их от ваших же выделений, пошла в сторону дома. Через некоторое время, отойдя от оргазма, вы привели себя в порядок и отправились домой.'
	end
	killvar '$textdog'
	killvar '$textlick'
	killvar '$text_emotions'
...
if $ARGS[0] = 'mirafinger_dog':
	*clr & cla
	...
	gs'time'
	if horny < 100:
		gs 'zz_render','','qwest/gadukino/new/mirafinger_dog'+rand(1,4)
