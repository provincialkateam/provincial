...
if cloth[0] > 0:
	if func('zz_clothing2','is_skirt') = 0:
		...
		gs 'zz_render', 'Лес', 'qwest/gadukino/forest/gadforestrelax_ski'+iif(bonfire = 0,1,iif(cloth[1] = 1,2,3))+'1.jpg'
	end
else
...
if cloth[0] > 2 and bonfire = 1:
	...
	act 'Греться у костра':
		*clr & cla
		...
		frost = 0
		manna += 10
		gs 'zz_render', 'Лес', 'qwest/gadukino/forest/gadforestrelax_bonfire'+iif(func('zz_weather','is_day')=1,2,3)+'.jpg','В течение часа вы сидели молча, глядя на костер.Вы согрелись и отдохнули, наверное пора двигаться дальше.'
		act 'Далее':gt 'gadforestrelax'
	end
		...
		*clr & cla
		...
		if cloth[0] > 0:
			if func('zz_clothing2','is_skirt') = 0:
				...
				gs 'zz_render', 'Лес', 'qwest/gadukino/forest/gadforestrelax_ski'+iif(cloth[1] = 1,4,5)+'.jpg'
			end
		else
		...
		if son < 24:
			...
			if cloth[0] > 0:
				if func('zz_clothing2','is_skirt') = 0:
					...
					gs 'zz_render', 'Лес', 'qwest/gadukino/forest/gadforestrelax_sleepski'+iif(cloth[1] = 1,'',1)+'.jpg'
				end
			else
