if $ARGS[0] = 'start':
	...
	*clr & cla
	...
	if cloth[1] = 0 and i > 90:gt 'sexshop', 'shprod'
	minut += 5
	$metka = $ARGS[0]
	$loc = $CURLOC
	gs 'stat'
	...
	$_str += '<tr bgcolor=#f3f4ee><td>Лубрикант</td><td>300р.</td><td>'+iif(money >= 200,'<a href="exec: gs''sexshop'',''buy_item'',7, 200">Купить</a>','Купить')+'</td></tr>'
	if pornMagazine = 0: $_str += '<tr bgcolor=#f3f4ee><td>Порножурнал</td><td>400р.</td><td>'+iif((money or karta) >= 400,'<a href="exec: gs''sexshop'',''buy_item'',8, 400">Купить</a>','')+'</td></tr>'
	if strapon = 0: $_str += '<tr bgcolor=#f3f4ee><td>Страпон</td><td>1200р.</td><td>'+iif((money or karta) >= 1200,'<a href="exec: gs''sexshop'',''buy_item'',9, 1200">Купить</a>','')+'</td></tr>'
	if analplug = 0: $_str += '<tr bgcolor=#f3f4ee><td>Анальная пробка</td><td>1200р.</td><td>'+iif((money or karta) >= 1200,'<a href="exec: gs''sexshop'',''buy_item'',10, 1200">Купить</a>','')+'</td></tr>'
	if vibrator = 0: $_str += '<tr bgcolor=#f3f4ee><td>Вибратор</td><td>1300р.</td><td>'+iif((money or karta) >= 1300,'<a href="exec: gs''sexshop'',''buy_item'',11, 1300">Купить</a>','')+'</td></tr>'
	if dildo = 0: $_str += '<tr bgcolor=#f3f4ee><td>Маленький фалоимитатор</td><td>800р.</td><td>'+iif((money or karta) >= 800,'<a href="exec: gs''sexshop'',''buy_item'',0, 800">Купить</a>','')+'</td></tr>'
	if middildo = 0: $_str += '<tr bgcolor=#f3f4ee><td>Фалоимитатор, 15 см</td><td>1500р.</td><td>'+iif((money or karta) >= 1500,'<a href="exec: gs''sexshop'',''buy_item'',1, 1500">Купить</a>','')+'</td></tr>'
	if largedildo = 0: $_str += '<tr bgcolor=#f3f4ee><td>Фалоимитатор, 20 см</td><td>2500р.</td><td>'+iif((money or karta) >= 2500,'<a href="exec: gs''sexshop'',''buy_item'',2, 2500">Купить</a>','')+'</td></tr>'
	if bigdildo = 0: $_str += '<tr bgcolor=#f3f4ee><td>Фалоимитатор, 25 см</td><td>4500р.</td><td>'+iif((money or karta) >= 4500,'<a href="exec: gs''sexshop'',''buy_item'',3, 4500">Купить</a>','')+'</td></tr>'
	if extradildo = 0: $_str += '<tr bgcolor=#f3f4ee><td>Фалоимитатор, 30 см</td><td>6500р.</td><td>'+iif((money or karta) >= 6500,'<a href="exec: gs''sexshop'',''buy_item'',4, 6500">Купить</a>','')+'</td></tr>'
	if superdildo = 0: $_str += '<tr bgcolor=#f3f4ee><td>Фалоимитатор, 35 см</td><td>8500р.</td><td>'+iif((money or karta) >= 8500,'<a href="exec: gs''sexshop'',''buy_item'',5, 8500">Купить</a>','')+'</td></tr>'
	if maddildo = 0: $_str += '<tr bgcolor=#f3f4ee><td>Фалоимитатор, 40 см</td><td>12000р.</td><td>'+iif((money or karta) >= 12000  ,'<a href="exec: gs''sexshop'',''buy_item'',6, 12000">Купить</a>','')+'</td></tr>'
	gs 'zz_render','','','<center><table width=600>'+$_str+'</table></center>'
	killvar '$_str'
	gs 'zz_clothing2','make_shop',7
end
! args[1] - id
! args[2] - price
if $ARGS[0] = 'buy_item':
	*clr & cla
	minut += rand(5,10)
	local item_price = args[2]
	local $_buy_item = ''
	local $message = ''
	if money >= item_price:
		money -= item_price
		gs 'stat'
	elseif karta >= item_price:
		karta -= item_price
		gs 'stat'
	else
	...
	if args[1] = 0: dildo = 1
	if args[1] = 1: middildo = 1
	if args[1] = 2: largedildo = 1
	if args[1] = 3: bigdildo = 1
	if args[1] = 4: extradildo = 1
	if args[1] = 5: superdildo = 1
	if args[1] = 6: maddildo = 1
	if args[1] = 7: lubri += 5 & $_buy_item += 'лубрикант'
	if args[1] = 8: pornMagazine = 50 & $_buy_item += 'порножурнал'
	if args[1] = 9: strapon = 1 & $_buy_item += 'страпон'
	if args[1] = 10: analplug = 1 & $_buy_item += 'анальная пробка'
	if args[1] = 11: vibrator = 1 & $_buy_item += 'вибратор'
	if args[1] <= 6: $_buy_item += 'фалоимитатор'
	$message += '<green><b>Вы купили '+$_buy_item+'</b></green>'
	if len($_buy_item) > 0: gs 'zz_funcs','message','', $message
	gt 'sexshop','start'
end
if $ARGS[0] = 'podval':
	*clr & cla
