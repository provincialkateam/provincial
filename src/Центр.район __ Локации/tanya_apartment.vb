...
if $args[0] = '' or $args[0] = 'main':
	*clr & cla
	...
	minut += 1
	gs 'stat'
	! переход на забавы со свечкой
	if $npc['31,candle_day'] = 1: gt 'tanya_events','tanya_candle'
	!---
	...
	act 'Ванная': gt 'zz_bathroom','init','tanya_apartment'
	gs 'tanya_events','check_room'
	if hour > 8 and hour < 19 and week < 7 and $npc['31,clean_day'] ! day: act 'Убираться': gt 'tanya_events','clean_room'
	if week < 6 and hour >= 22:
		if $npc['31,punish_day'] + 14 <= daystart:
			act 'Наказать Таню': gt 'tanya_events','punish2'
		elseif $npc['31,punish_day'] + 7 <= daystart:
