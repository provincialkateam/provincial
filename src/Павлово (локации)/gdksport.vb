if $args[0] = '' or $args[0] = 'main':
	...
	if hour >= 15 and hour < 20:
		if func('zz_clothing2','get_group') = 1:
			...
			if ivan_trainer_qw = 1 and week mod 2 ! 0 and week < 6 and hour = 18 and minut < 20 and ivan_trainer_qw_day ! day and sport = 0:
				act 'Идти на тренировку к Ивану':
					*clr & cla
					...
					gs 'zz_render','','images/qwest/alter/boxing/box'+$_img+'.jpg','На протяжении часа вы занимались под присмотром Ивана. **'+$_str
					killvar '$_img'
					killvar '$_str'
					gs 'stat'
				...
				if week ! 6 and volkomday ! day:
					act 'Заниматься с командой':
						*clr & cla
						...
						act 'Уйти': gt'gdksport'
						act 'Зайти в тренерскую':gt'Voltrener'
						if cloth[1] = 0 and volleytanga = 1:
							delact 'Зайти в тренерскую'
							act 'Поправить одежду':
								*clr & cla
								Voltrenersee += 1
								gs 'zz_render', '', 'images/pavlovo/dk/volley/game'+rand(1,2)+'.jpg','Похоже тренер заметил ваши манипуляции и вашу молодую письку, глаза его зажглись и он с трудом отворачиваясь произнес: "Лебедева зайдешь ко мне после тренировки. Мне нужно показать тебе ЭЭЭ ... пару приемов" '
								act 'Уйти': gt 'gdksport'
		...
		elseif func('zz_clothing2','get_group') = 1 and ($npc['23,qwMain'] + gsAboDance + gsAboBeg + gsAboVolley > 0):
...
if $args[0] = 'buy':
	if args[1] = 0 :
		if func('zz_funcs','payment',0,2000,1) = 1:
			gsAboBeg += 30
		end
	elseif args[1] = 1:
		if func('zz_funcs','payment',0,3000,1) = 1:
			gsAboDance += 30
		end
	else
		if func('zz_funcs','payment',0,1500,1) = 1:
			gsAboVolley += 30
		end
	end
	gt 'gdksport'
