if $args[0] = '' or $args[0] = 'main':
	...
	gs 'stat'
	gs 'time'
	gs 'zz_render','Рынок','pavlovo/market/market.jpg','Маленький рынок расположен возле <a href="exec: gt ''vokzalG''">вокзала</a>, но с него легко можно попасть в <a href="exec: gt''gorodok''">городок</a>.'
	act '<b>В городок</b>': gt 'gorodok'
	act '<b>Вокзал</b>': gt 'vokzalG'
	if hour >= 8 and hour <= 16:
		gs 'zz_render','','','На рынке продавцы выставили товары и зазывают покупателей.'
		act 'Ателье по пошиву и ремонту одежды': gt 'tailor'
			...
			act 'Продать гобелен':
				if gobQW = 0:
					...
					*clr & cla
					...
					elseif gobQW = 1:
						gobQW = 2
						gs 'zz_render','','','Мужчина приветствует вас и представляется Романом, вы так же отвечаете, что вас зовут Света. После приветствия Роман деловито интересуется, что у вас есть на продажу.'
					end
	...
	*clr & cla
	...
	if args[1] = 12:
		if arturKnow = 0:
			...
			gs 'zz_render','','pavlovo/market/market.jpg','Внезапно вас окликают:**\\\- Эй Светик, иди сюда, скюшай мандаринку.///'
...
if $args[0] = 'porn_market':
	*clr & cla
	...
	gs 'zz_render','','','Продавец спрашивает вас:**\\\- Желаете приобрести журнальчик?///'
	act 'Спросить цену':
		cla
		minut += 5
		if shameless['flag'] = 0:
			gs 'zz_render','','','Отчаянно краснея до кончиков волос осипшим голосом вы тихонько спросили у него цену на "эти вот журналы", указав кивком на торчащие из развала уголки "18+". Продавец с каменным лицом сказал: **\\\- Все журналы по 500 рублей.///'
		else
			gs 'zz_render','','','\\\- Все журналы по 500 рублей.///'
		end
		gs 'grinok','buy_pornMagazine'
		act 'Уйти': gt 'grinok'
	end
