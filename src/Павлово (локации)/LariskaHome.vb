...
if (hour >= 16 and hour < 19 and week < 6) or (hour >= 9 and hour < 19 and (week >= 6 or school['vacation'] > 0)):
	gs 'zz_render','','images/qwest/alter/Lariska/Home/home_1.jpg'
	...
	gs 'zz_render','','images/qwest/alter/Lariska/LariskaMother.jpg'
	if lariskaMotherKnown = 0:
		minut += 5
		gs 'stat'
		if hour >= 9:$text_home = 'Только, Света, Ларисы нет дома.'
		if hour < 9:$text_home = 'Света. Ночь на улице, приходи не раньше девяти.'
		gs 'zz_render', '', '', func('LariskaHome_strings', 'local_str4')
		lariskaMotherKnown = 1
		act 'Идти домой':gt'gorodok'
		exit
	end
	if hour >= 9:'Дверь вам открыла мама Ларисы "Привет, Света. А Ларисы нет дома."'
	if hour < 9:'Дверь вам открыла мама Ларисы "Света. Ночь на улице, приходи не раньше девяти."'
	act 'Идти домой':gt'gorodok'
end
if $ARGS[0] = 'in':
	*clr & cla
		...
		act 'Отойти в туалет':
			*clr & cla
			...
			elseif week >= 6:
				...
				gs 'zz_render','','images/qwest/alter/Lariska/Home/home_2.jpg'
