if $args[0] = 'main' or $args[0] = '':
	...
	gs 'zz_render','Дискотека','pavlovo/dk/disco/disco',func('gdkin_strings',0)+'<table><tr><td width=250 valign=top>На танцполе танцуют:**'+func('npc_editor','make_npc_group',3,1)+'</td><td width=250 valign=top>В углу стоят:**'+func('npc_editor','make_npc_group',4,1)+'</td></tr></table>'
		if $npc['38,wedding'] = 1 and hour >= 20 and hour <= 22 and rand(1,10) = 1: gs 'zz_render','','pavlovo/family/incest/mini/17.jpg','На дискотеке вы замечаете <a href="exec: gt ''gdkin'',''anya_misha''">Аню и Мишку</a>.'
...
if $args[0] = 'dancing':
	*clr & cla
	...
	elseif alko > 0 and alko < 4:
		if cloth[1] = 0 and func('zz_clothing2','is_skirt') = 1:
			gs 'zz_render','','pavlovo/dk/disco/dancing/1.webm',func('gdkin_strings',5)
		else
...
if $args[0] = 'gop_drink':
	*clr & cla
	...
	gs 'zz_render', '','','Распихивая танцующих на танцполе к вам протискивается <<$npc[''<<numnpc>>,surname'']>>:**\\\- Эй, Светик, давай подбухнем, а то ваще чёта стрёмно.///'
	gs 'gnpc_events','drink'
end
if $args[0] = 'kotov_dance':
	$npc['9,dance'] = day
	gs 'npc_editor','get_npc_profile',9
	gs 'zz_render', '','','Нагло распихивая людей на танцполе, к вам подруливает Витёк Котов:**\\\- Эй, Светик, слышь, давай потанцуем чё-ли?///'
	act 'Танцевать': gt 'kotovtalker','kotov_dance'
	if dom >= 10 and alko < 6: act 'Отказаться': gs 'npc_editor','change_rep','-',9,5 & gt $loc, $metka
end
if $args[0] = 'dima_dance':
	*clr & cla
	...
	gs 'zz_render', '', '', 'На танцполе к вам подошёл Дима Носов:**\\\- Светуля, может, потанцуем?///'
	act 'Танцевать': gt 'gnpc_events','krutishi_dance'
	if $npc['1,qwDimaRude'] = 0 and alko < 6 and dom > -25: act 'Отказаться': gs 'npc_editor','change_rep','-',1,5 & gt $loc, $metka
end
if $args[0] = 'igor_dance':
	*clr & cla
	...
	gs 'zz_render', '', '', 'На танцполе к вам подошёл Игорь Круглов:**\\\- Светуля, может, потанцуем?///'
	act 'Танцевать': gt 'gnpc_events','krutishi_dance'
	if $npc['1,qwDimaRude'] = 0 and alko < 6 and dom > -50: act 'Отказаться': gs 'npc_editor','change_rep','-',4,5 & gt $loc, $metka
end
if $args[0] = 'anya_misha':
    *clr & cla
    gs 'zz_render','','pavlovo/family/incest/mini/17.jpg','На дискотеке вы замечаете Аню и Мишку. Они не обращают внимания ни на что вокруг и поглощены только друг другом.'
    act 'Далее': gt 'gdkin'
end