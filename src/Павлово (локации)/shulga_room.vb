...
sveta_rnd_name = RAND(0,3)
	if sveta_rnd_name = 0:$sveta_rnd_name = 'Света'
	if sveta_rnd_name = 1:$sveta_rnd_name = 'Светик'
	if sveta_rnd_name = 2:$sveta_rnd_name = 'Светик'
	if sveta_rnd_name = 3:$sveta_rnd_name = 'Света'
!отношения
$vasya_shulgin_rep = {
...
if i = 7 and sh_hooker_count > 0 and week < 6 and hour >= 0 and hour <= 2 and func('zz_reputation','get') >=3:
	dynamic $sh_hooker
end
if i = 7 and sh_hooker_count > 0 and week >= 6 and hour >= 0 and hour <= 2:
	dynamic $sh_hooker
end
if i = 7 and sh_hooker_count > 0 and hour >= 10 and hour <= 16:
	dynamic $sh_hooker
end
										...
										gs 'zz_render', '', '', func('shulga_room_strings', 'local_str32')
											act 'Далее':
												...
												*clr & cla
												...
												anal += 1
												gs 'stat'
												gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/sex/1_12.jpg'
												if analplugin = 1:
													analplugin = 0
														...
														*clr & cla
														...
														act 'Далее':
															*clr & cla
															...
															act 'Уйти': gt 'zz_bathroom','init','shulgahome'
														end
													end
												end
											end
	...
	*clr & cla
	...
	gs 'zz_render', '', 'images/zsoft/pod_ezd/shulgahome/bigroom/drunk_ev/sex/bj'+rand(1,13)+'.jpg', func('shulga_room_strings', 'local_str47')
		...
		*clr & cla
		...
		sex += 1
		gs 'stat'
		gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/drunk_ev/sex/sex'+rand(1,22)+'.jpg'
	...
	gs 'zz_render', '', 'images/zsoft/pod_ezd/shulgahome/tv/tvporn'+rand(1,5)+'.gif"></center>', func('shulga_room_strings', 'local_str56')
		...
		act 'Сосать':
			*clr & cla
			...
			if $npc['9,sex'] = 0: $npc['9,sex'] = 1 & guy += 1
			gs 'stat'
			gs 'zz_render','','common/sex/bj/<<rand(0,4)>>.gif', func('shulga_room_strings', 'local_str57')
				...
				act 'Далее':
					*clr & cla
					...
					gs 'zz_funcs', 'cum', 'face'
					gs 'stat'
					gs 'zz_render','','common/sex/facial/<<rand(0,4)>>.jpg', func('shulga_room_strings', 'local_str58')
						...
						*clr & cla
						...
						if horny >=70:
								...
								*clr & cla
								...
								act 'Далее':
									*clr & cla
									...
									swallow += 1
									gs 'stat'
									gs 'zz_render', '','common/sex/facial/<<rand(0,4)>>.gif', func('shulga_room_strings', 'local_str63')
									act 'Закончить': gt'shulga_room'
								end
							end
						else
							...
							*clr & cla
							...
							cumfrot += 1
							gs 'stat'
							gs 'zz_render','','common/sex/facial/<<rand(0,4)>>.gif', func('shulga_room_strings', 'local_str64')
							act 'Закончить': gt'shulga_room'
						end
					end
				end
...
$view_pornotv_vasya = {
	act 'Смотреть порно канал':
		*clr & cla
		...
		horny += 15
		gs 'stat'
		gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/tv/tvporn'+rand(1,5)+'.gif'
		...
		if $npc['11,relation'] >=70 and func('zz_reputation','get') < 3:
			act 'Сосать':
				*clr & cla
				...
				horny += 10
				gs 'stat'
				gs 'zz_render','','common/sex/bj/<<rand(0,4)>>.gif',func('shulga_room_strings', 'local_str65')
				act 'Далее':
					*clr & cla
					minut += 10
					gs 'zz_funcs', 'cum', 'face'
					cumfrot += 1
					gs 'stat'
					gs 'zz_render','','common/sex/bj_cum/<<rand(0,4)>>.gif', func('shulga_room_strings', 'local_str66')
		...
		if $npc['11,relation'] >=20 and $npc['11,relation'] < 50:
			act 'Дрочить':
				*clr & cla
				...
				cumfrot += 1
				gs 'stat'
				gs 'zz_render','','common/sex/hj/<<rand(0,4)>>.gif', func('shulga_room_strings', 'local_str67')
				act 'Уйти': gt'shulga_room'
			end
		end
		if func('zz_reputation','get') >= 3 and $npc['11,relation'] >=70:
			act 'Сосать':
				*clr & cla
				...
				horny += 10
				gs 'stat'
				gs 'zz_render','','common/sex/bj/<<rand(0,4)>>.gif', func('shulga_room_strings', 'local_str68')
				act 'Далее':
					*clr & cla
					minut += 10
					horny += 15
					...
					act 'Далее':
						*clr & cla
						...
						gs 'zz_funcs', 'cum', 'face'
						gs 'stat'
						gs 'zz_render', '','common/sex/bj_cum/<<rand(0,4)>>.gif', func('shulga_room_strings', 'local_str70')
						act 'Уйти': gt 'shulga_room'
					end
				end
			end
...
if rndtvs>10:gs 'zz_render', '', '','<<$shulga_tv_rand>> ** Вы промастриваете телевизор около получаса.'
	...
	*clr & cla
	...
	act 'Поговорить':
		*clr & cla
		...
		if vasyan_textb = 5:$vasyan_textb = 'Рассказывает анекдот:**Разговор по Skуpе: — Люся, покажи сиськи!** — Нет, ты что, мы же друзья.** — Будь другом, покажи сиськи!'
		if vasyan_textb = 6:$vasyan_textb = 'Рассказывает анекдот:**- Вовочка, ты когда с дискотеки вернешься?**- Рано. Вы ещё спать будете...'
		if vasyan_textb = 7:$vasyan_textb = 'Рассказывает анекдот:**"Стоит Вовочка на улице курит сигарету. Незнакомая женщина делает ему замечание:**- Мальчик, а твои родители знают, что ты куришь?**- Тётенька, а ваш муж знает, что вы пристаёте к незнакомым мужчинам на улицах?'
		if vasyan_textb = 8 and $npc['11,relation'] < 30:$vasyan_textb = 'Вы болтаете минут десять на разные темы.'
		if vasyan_textb = 8 and $npc['11,relation'] >= 30: $vasyan_textb = 'Вы беседуете на разные темы.'
		if vasyan_textb = 8 and $npc['11,relation'] >= 30 and $npc['11,relation'] < 50: $vasyan_textb = '<<$shulga_name_rnd>> говорит: "Тут тема новая появилась, можно нормально деньжат поднять в интернете. Про вебкам-чаты слышала?"**-"Нет, но интересно. Рассказывай!" **- Васян продолжает: -"Ну так вот: короче, нужна вебкамера и компьютер, с интернетом, разумеется. Регишься на вебкам-сайте и можешь транслировать себя через интернет на камеру, а всякие лохи платят за просмотр денежку.**И, говорят, - чем откровеннее шоу, - тем больше бабла можно заработать."**-"Ну я не знаю, неправильно продавать своё тело" - в сомнениях мнётесь вы.**Вася парирует: -"Да чего ты там себе выдумываешь? - кому чего продавать то? Всего-то пару часиков перед камерой жопой повертеть - и можно нормально срубить денег! Это тебе не на дороге стоять, пиздой торговать!"'
		if vasyan_textb = 8 and $npc['11,relation'] >= 50 and $npc['11,relation'] < 70: $vasyan_textb = '<<$shulga_name_rnd>> Спрашивает: -"Помнишь, я тебе говорил про вебкам-чаты, ну там где бабло можно заработать, вертя жопой?"**-"Ага, помню" - отвечаете вы.**<<$shulga_name_rnd>> продолжает разговор: - "Если хочешь - можешь воспользоваться моим ноутом, мне не жалко."**-"Спасибо, я подумаю..." - отвечаете вы.'
		if vasyan_textb = 8 and $npc['11,relation'] >= 100:$vasyan_textb = '<<$shulga_name_rnd>> Спрашивает: -"Чо как, вебкой пользовалась? - вертела уже жопой в онлайн-чате?"**-"Да, было дело" - не смущаясь отвечаете вы.**<<$shulga_name_rnd>> продолжает разговор: - "Ты, если что, по выходным можешь меня звать, вместе шоу будем делать: задроты больше денег заплатят, если увидят как тебя шпилят в онлайне!"**-"Идея неплохая, я подумаю..." - говорите вы.'
		!базар когда гг с репой gorslut>=3:
		vasyan_textb_gorslut = RAND(0,12)
		if vasyan_textb_gorslut = 0 and $npc['25,qwSoniaSlut'] = 1:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> рассказывает как он с корешами драл Соньку: "Прямо в парке эту шлюшку разложили на лавочке, вертели как хотели пока мы с пацанами не устали, а она, блядина такая, ещё требовала!"** <<$rnd_rebyata>> подтвердил: "Ага, у меня после пары палок уже не стоял, а она всё ещё не унималась, ртом пытаясь поднять мой хер, требуя продолжения!"'
		if vasyan_textb_gorslut = 0 and $npc['25,qwSoniaSlut'] <=0:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> мечтательно говорит: -"Сонька на дискаче, бывает, так набухивается!.. Надо бы её как-нибудь поймать и выебать: пьяная баба пизде же не хозяйка, ведь так?"** Пацаны хором ржут.'
		if vasyan_textb_gorslut = 1:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> спрашивает: "<<$sveta_rnd_name>>, хорошо ли мы вчера в парке тебя по кругу пустили? - Я ж тебя давно знаю, нравится тебе это дело, ведь так?"** - "Да, понравилось", - соглашаетесь вы.** <<$rnd_rebyata>> намекает: "Надо бы повторить!" **Вы просто по-блядски улыбаетесь в ответ.'
		if vasyan_textb_gorslut = 2 and $npc['25,qwSoniaSlut'] = 1:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> спрашивает: "<<$sveta_rnd_name>> а как тебе Соня? Я бы посмотрел на вас вместе, может как-нибудь нам с пацанами шоу устроите?"**<<$rnd_rebyata>> добавляет: "Да, две наши нимфоманки могли бы такое показать!" ** - "Всё может быть..." - улыбаясь, отвечаете вы.'
		if vasyan_textb_gorslut = 2 and $npc['25,qwSoniaSlut'] <=0:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> спрашивает: "<<$sveta_rnd_name>> а как тебе Соня? Я бы посмотрел на вас вместе, может как-нибудь нам с пацанами шоу устроите?"**<<$rnd_rebyata>> добавляет: "Да, Соньку бы раскрутить... Надо на дискаче её напоить и выебать." ** - "Всё может быть..." - улыбаясь, отвечаете вы.'
		if vasyan_textb_gorslut = 3 and school['certificate'] = 0:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> мечтательно говорит: "Эх сейчас бы нашу Англичанку выебать, на уроках покоя не даёт ее одежда, словно специально миниюбку надевает, чтобы пацанов подразнить!"** - "Согласна, вертит жопой как специально! - я бы посмотрела как её дрючат, а может и сама бы поруководила процессом!" - киваете вы.'
		if vasyan_textb_gorslut = 3 and school['certificate'] = 1:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> мечтательно говорит: "Эх сейчас бы Юльку выебать, покоя не даёт её тело! Словно специально лёгкие платья надевает чтобы подразнить!"** - "Согласна, вертит попкой как специально! Я бы посмотрела как её дрючат, а может и сама бы поруководила процессом!" - ухмыляетесь вы.'
		if vasyan_textb_gorslut = 4:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> рассказывает: "<<$sveta_rnd_name>>, говорят, твоя мамка - та ещё шлюшка: я тут не так давно слышал, что в кафешке, где она работает, хозяин её пялит почти каждый день. Хоть и не молодуха, но ещё ничего так: ухаживает за собой, фигурка хорошая, да и опыта вагон наверное... Я бы сам вдул." - Пацаны одобрительно соглашаются со словами Шульги.** "Мечтать не вредно!" - вертите вы кукиш у них перед носами.'
		if vasyan_textb_gorslut = 5 and school['certificate'] = 0 and $npc['25,qwSoniaSlut'] = 1:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> говорит: "Недавно в школе такое видал - аж ахуел... Соньку в туалете по кругу какие-то сопляки пускали. Ей уже, по-моему, похуй - где и с кем ебаться, главное чтобы побольше хуёв и покрепче."** - "Ага, сделали из неё нимфоманку, теперь без хуя и дня не протянет" - соглашаетесь вы. ** <<$rnd_rebyata>> говорит: "Да мы её и так почти каждый день подкармливаем". - Пацаны ржут.'
		if vasyan_textb_gorslut = 5 and school['certificate'] = 1 and $npc['25,qwSoniaSlut'] = 1:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> говорит: "На рынке случайно такое видал - аж ахуел. Сонька, похоже, за шмотками пришла на рынок, а её там по кругу пустили Артур и его компания. Ей уже, по-моему, похуй - где и с кем ебаться, лишь бы много хуёв было. А на рынке ещё и скидку сделают." ** - "Сделали из неё общую дырку - теперь ей и прохода нету, да и сама она без хуя уже и дня не протянет" - соглашаетесь вы. ** <<$rnd_rebyata>> говорит: "Да мы её и так почти каждый день подкармливаем, но согласен - с нас скидку на шмотки не выбить!" Пацаны ржут.'
		...
		if vasyan_textb_gorslut = 6:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> намекает: "Что, <<$sveta_rnd_name>>, - когда ебаться будем?" - Пацаны переглядываются и уставляются на вас, ожидая ответа.** - "Как надумаю - так сразу" - спокойно отвечаете вы.** <<$rnd_rebyata>>: - "Может тебе выпить нужно, для расслабления организма, - быстрее надумаешь?". Пацаны ржут. ** Вы бросаете недовольный взгляд на компашку: "Не в этом дело. Хотя выпить идея хорошая."'
		if vasyan_textb_gorslut = 7:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> возбуждённо расспрашивает: "<<$sveta_rnd_name>>, а ты в попку даёшь с удовольствием или это для тебя мучение?"** - "Сначала не нравилось, но потом привыкла и теперь даже иногда кайф от этого ловлю" - хитро прищуриваетесь вы.** <<$rnd_rebyata>> хохмит: "Ну конечно: уже нравится, когда дупло не закрывается!" Пацаны ржут.'
		if vasyan_textb_gorslut = 8:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> расспрашивает: "<<$sveta_rnd_name>>, а какая поза тебе больше всего нравится в сексе?"** - "Удобная!" - смеясь, отвечаете вы.** Пацаны хором ржут.'
		if vasyan_textb_gorslut = 9:$vasyan_textb_gorslut = 'Рассказывает пошлый анекдот:**"Пацан провожает деваху до дома. Заходят в подъезд, останавливаются на лестничной площадке.**Она: — Выкрути лампочку.**Он выкручивает. В полной темноте она говорит:** — Хочешь, в рот возьму?** — Ты что? Она же горячая!"'
		if vasyan_textb_gorslut = 10:$vasyan_textb_gorslut = 'Рассказывает анекдот:**"Удав, глядя на слона:** — Ха-ха, член на морде!** Слон, глядя на удава:** — Ха-ха морда на члене!"'
		...
		if vasyan_textb_gorslut = 12 and $npc['11,relation'] >= 30: $vasyan_textb_gorslut = 'Вы беседуете на разные темы.'
		if vasyan_textb_gorslut = 12 and $npc['11,relation'] >= 30 and $npc['11,relation'] < 50: $vasyan_textb_gorslut = '<<$shulga_name_rnd>> говорит: "Тут тема новая появилась, можно нормально деньжат поднять в интернете. Про вебкам-чаты слышала?"**-"Нет, но интересно. Рассказывай." - говорите вы.**Васян продолжает: -"Ну так вот: нужна вебкамера и компьютер. Регишься на вебкам-сайте и можешь транслировать себя через интернет на камеру, а всякие лохи платят за просмотр денежку.**И, говорят, - чем откровеннее шоу, - тем больше бабла можно заработать.**-"Ну я не знаю, неправильно продавать своё тело" - задумываетесь вы.**Вася парирует: -"Да чего ты там себе выдумываешь - кому чего продавать-то? Всего-то пару часиков перед камерой жопой повертеть - и можно нормально срубить денег! Это тебе не на дороге стоять, пиздой торговать."'
		if vasyan_textb = 12 and $npc['11,relation'] >=50 and $npc['11,relation'] < 70:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> Спрашивает: -"Помнишь, я тебе говорил про вебкам-чаты, ну там, где бабло можно заработать, вертя жопой?"**-"Ага, помню" - отвечаете вы.**<<$shulga_name_rnd>> продолжает разговор: - "Если хочешь, можешь пользоваться моим ноутом - мне не жалко."**- Спасибо, я подумаю - отвечаете вы.'
		if vasyan_textb = 12 and $npc['11,relation'] >=100:$vasyan_textb_gorslut = '<<$shulga_name_rnd>> Спрашивает: -"Чо, как - вебкой пользовалась, вертела уже жопой в онлайн-чате?"**-"Да, было дело" - не смущаясь, отвечаете вы.**<<$shulga_name_rnd>> продолжает разговор: - "Ты, если что, по выходным можешь меня звать - вместе шоу будем делать: задроты больше денег заплатят, если увидят как тебя шпилят в онлайне."**- Идея неплохая, я подумаю... - говорите вы.'
		dynamic $vasya_shulgin_rep
...
den_textb = RAND(0,6)
			...
			if den_textb = 6:$den_textb = 'Рассказывает анекдот:**"Мариванна отчитывает Вовочку:**- Вовочка, добрая половина девочек нашей школы жалуется, что ты постоянно пристаёшь к ним с неприличными предложениями!**- Клевета, Мариванна! - отвечает Вовочка. - Это не добрая, а злая половина. Добрая отвечает на мои предложения согласием."'
!базар когда гг с репой gorslut>=3:
den_textb_gorslut = RAND(0,7)
			if den_textb_gorslut = 0:$den_textb_gorslut = 'Денис спрашиает вас: - <<$sveta_rnd_name>>, как у тебя с деньгами? Если мало - могу подработку для тебя найти: тело у тебя ничего так, рот рабочий, да и другие дырки разработаны. Можно нормально так бабло зашибать.**-"Прекращай фантазировать, я тебе что - шлюха какая!?" - зло отвечаете вы.**<<$rnd_rebyata2>> защищая друга говорит: - Да ты не кипишуй, у него всего лишь мысли вслух...'
			if den_textb_gorslut = 1 and $npc['25,qwSoniaSlut'] = 1:$den_textb_gorslut = 'Дэн спрашиает: - <<$sveta_rnd_name>>, когда ты нам какую-нибудь свою подружку подгонишь?**- А я что - одна уже не справляюсь? - корча обиженную мину, спрашиваете вы.**Дэн отвечает: - Да не, я не в претензиях, просто сразу с двумя бабами охота попробовать!**<<$rnd_rebyata2>> заголосил: - Да Соньку можно позвать, ей вообще похуй с кем и где. Её и Свету вместе разложить было бы супер."**Все соглашаются, что идея хорошая.'
			if den_textb_gorslut = 1 and $npc['25,qwSoniaSlut'] <=0:$den_textb_gorslut = 'Дэн спрашиает: - <<$sveta_rnd_name>>, когда ты нам какую-нибудь свою подружку подгонишь?**- А я, что разве одна не справляюсь? - корча обиженную мину спрашиваете вы.**Дэн отвечает: - Да не, я не в претензиях, просто сразу с двумя бабами охота попробовать!**<<$rnd_rebyata2>> заголосил: - Отличная идея! Осталось найти подходящую кандидатуру.'
			...
			if den_textb_gorslut = 3:$den_textb_gorslut = 'Дэн говорит: - Прикиньте пацаны, я тут такое узнал! Аптекарша-то наша, Людка, с негром живёт. Говорят, у чёрных хуи огромные, небось с твою руку Светик!**Пацаны ржут, вы усмехаетесь.**- Людка баба хоть и в возрасте, но еще многим молодухам фору даст.'
			if den_textb_gorslut = 4:$den_textb_gorslut = 'Рассказывает анекдот:**Жена говорит мужу:** — Я когда-то в институте встречалась с одним парнем... Пошли мы как-то в кино, и посреди фильма он меня спрашивает: "Хочешь пососать?" Я, естественно, начала возмущаться, а он достал из кармана чупа-чупс и протянул мне.**Муж, задумчиво:** — У этого парня, походу, был просто гениальный план отступления.'
			if den_textb_gorslut = 5:$den_textb_gorslut = 'Дэн расспрашивает: - <<$sveta_rnd_name>>, как сеструха твоя поживает? Парень-то у неё есть? Она же в магазине продавщицей работает, небось от грузчиков отбоя нет, не мучают её там? Может помощь ей какая нужна? Девка-то она видная, да и я парень ходь куда: Яйца, табак, перегар и щетина - чем хуже грузчиков? **- Вроде есть, но я в её личную жизнь стараюсь не лезть - отвечаете вы.'
			if den_textb_gorslut = 6:$den_textb_gorslut = 'Рассказывает анекдот:**Мужик "снял" девушку в ресторане, шикарный стол, шампанское, икра и т. д.**Потом говорит: — Поехали ко мне! Едут (ночь, зима), он сворачивает в парк, останавливается, достаёт пистолет — Раздевайся догола, вылезай и лепи снеговика. - Она вся в слезах, разделась, вылезла, слепила.**— Залезай в машину. Одевайся. Приехали на "хату", офигенный стол, потом такой же ceкc, потом он отвозит ее куда она просит.**Она: — Зачем?! Зачем?! Я бы и без снеговика дала...**Он: — Понимаешь... шампанское... икра, ceкc — все это ты скоро забудешь, а вот "снеговика" — вряд ли...'
			if den_textb_gorslut = 7:$den_textb_gorslut = 'Рассказывает анекдот:**Приходит учительница на урок, надела на шею на цепочку, новый золотой кулончик в виде самолётика. Вовочка весь урок не сводит с него глаз. В конце урока учительница спрашивает:**"Что, Вовочка, тебе так самолётик понравился?"**- "Нет, аэродромик."'
...
vitek_textb = RAND(0,7)
			...
			if vitek_textb = 5:$vitek_textb = '— Правда, что минута смеха продлевает жизнь на пять минут?**— Ну, это смотря над кем смееться… Может и сократить…'
			if vitek_textb = 6:$vitek_textb = 'Рассказывает анекдот:**- Привет, Люся! Как выходные?**- Удались!**- Сама удались, овца!'
...
vitek_textb_gorslut = RAND(0,7)
			...
			if vitek_textb_gorslut = 3 and school['certificate'] = 0 and $npc['25,qwSoniaSlut'] = 1:$vitek_textb_gorslut = '<<$rnd_vitek_name>> рассказывает: - Соньку вчера прижал в школьном туалете, но потом она забралась на меня сверху, и я вдруг понял, что изнасилование у меня как-то не задалось. **Все присутствующие в комнате заржали.'
			if vitek_textb_gorslut = 3 and school['certificate'] = 1 and $npc['25,qwSoniaSlut'] = 1:$vitek_textb_gorslut = '<<$rnd_vitek_name>> рассказывает: -"Соньку вчера прижал в парке, но потом она забралась на меня сверху, и я вдруг понял, что изнасилование у меня как-то не задалось."**Все присутствующие в комнате заржали.'
			if vitek_textb_gorslut = 3 and $npc['25,qwSoniaSlut'] <= 0:$vitek_textb_gorslut = 'Вы слушаете как Витёк рассказывает пошлую историю'
			if vitek_textb_gorslut = 4:$vitek_textb_gorslut = 'Друзья пьют пиво и беседуют о бабах и cекcе, вы влезаете в разговор и спрашиваете Витька:**— А как ты думаешь, кому cекc приятнее - мужчине или женщине?**<<$rnd_vitek_name>> задумывается, но тут влезает <<$rnd_rebyata3>>:**— Ежу понятно, женщине! Вот у тебя ухо зачесалось, так какой части тела приятнее, пальцу, которым ты ухо почесала, или самому уху!'
			if vitek_textb_gorslut = 5:$vitek_textb_gorslut = '<<$rnd_vitek_name>> спрашивает: -"<<$sveta_rnd_name>>, а ты знаешь - какое главное качество пениса?**- это его воспитанность. Он обязан встать при виде обнажённой дамы, чтобы даме было на что сесть."**Комната наполнилась смехом.'
			if vitek_textb_gorslut = 6:$vitek_textb_gorslut = '<<$rnd_vitek_name>> говорит: - Пьяная девушка своей пизде не хозяйка.**Вы парируете: - Даааа. Как будто пьяный мужик своему хую великий командир".**Друзья ржут.'
			if vitek_textb_gorslut = 7:$vitek_textb_gorslut = 'Рассказывает анекдот:**Мужик заходит в аптеку и громко заявляет:**- Мне пачку презервативов, пожалуйста!**Молоденькая продавщица покраснела и шепчет:**- Такое надо на ухо...**Мужик:**- А зачем мне на ухо?! Мне на хуй нужно!!!'
	...
	*clr & cla
	...
	act 'Далее':
		*clr & cla
		...
		horny += 10
		gs 'stat'
		gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/lern/ev<<lern_imgset>>_3.jpg'
		if lern_imgset <= 3:
		...
		act 'Далее':
			*clr & cla
			...
			horny += 15
			gs 'stat'
			gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/lern/ev<<lern_imgset>>_4.jpg'
			!dynamic $gg_sex_orgasm
			if lern_imgset <= 2:
				gs 'zz_dynamic_sex', 'sex_start', 1
				gs 'zz_dynamic_sex', 'vaginal', 'dick', 1
			...
			act 'Далее':
				*clr & cla
				...
				horny += 15
				gs 'stat'
				gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/lern/ev<<lern_imgset>>_5.jpg'
				if lern_imgset < 7:
					gs 'zz_funcs', 'cum', 'face'
...
vasyan_textb_day = RAND(0,8)
			...
			if vasyan_textb_day = 5:$vasyan_textb_day = 'Рассказывает анекдот:**Весь Советский Союз смеялся над Вовочкой. А Вовочка вырос, стал президентом! И тебе один ржет над всей страной!'
			if vasyan_textb_day = 6:$vasyan_textb_day = 'Рассказывает анекдот:**— Вовочка, ты почему так мелко пишешь?**— Чтобы ошибки были не так заметны, Марь Иванна.'
			if vasyan_textb_day = 7:$vasyan_textb_day = 'Рассказывает анекдот:**Что у пьяной женщины на уме, то у трезвого мужчины вообще никогда не получится!!!'
			if vasyan_textb_day = 8 and $npc['11,relation'] < 30:$vasyan_textb = 'Вы беседуете минут десять на разные темы.'
			if vasyan_textb_day = 8 and $npc['11,relation'] >= 30:$vasyan_textb_day = 'Вы беседуете на разные темы.'
			if vasyan_textb_day = 8 and $npc['11,relation'] >= 30 and $npc['11,relation'] < 50: $vasyan_textb_day = '<<$shulga_name_rnd>> говорит: - Тут тема новая появилась, можно нормально деньжат поднять в интернете: про вебкам-чаты слышала?"**- Нет, но интересно, рассказывай подробнее! - интересуетесь вы.**Васян продолжает: - Ну так вот: нужна вебкамера и компьютер. Регишься на вебкам-сайте и можешь транслировать себя через интернет на камеру, а всякие лохи платят за просмотр денежку.**И, говорят, - чем откровеннее шоу, - тем больше бабла можно заработать!**- Ну я не знаю, неправильно продавать своё тело - в сомнении мнётесь вы.**Вася парирует: - Да чего ты там себе выдумываешь - кому чего продавать то? Всего-то пару часиков перед камерой жопой повертеть - и можно нормально срубить денег! Это тебе не на дороге стоять, пиздой торговать.'
			if vasyan_textb_day = 8 and $npc['11,relation'] >= 50 and $npc['11,relation'] < 70: $vasyan_textb_day = '<<$shulga_name_rnd>> Спрашивает: - Помнишь я тебе говорил про вебкам-чаты, ну там где бабло можно заработать, вертя жопой?"**- Ага, помню, - отвечаете вы.**<<$shulga_name_rnd>> продолжает: - Если хочешь, - можешь пользоваться моим ноутом, мне не жалко.**- Спасибо, я подумаю... - чешете задумчиво голову вы.'
			if vasyan_textb_day = 8 and $npc['11,relation'] >= 100:$vasyan_textb_day = '<<$shulga_name_rnd>> Спрашивает: - Чо как, вебкой пользовалась, вертела уже жопой в онлайн-чате?**- Тебе-то какое дело? - поднимаете бровь вы.**<<$shulga_name_rnd>> продолжает разговор: - Ты, если что, по выходным можешь меня звать - вместе шоу будем делать: задроты больше денег заплатят, если увидят как тебя шпилят в онлайне"**- Идея неплохая, я подумаю, - говорите вы.'
dynamic $vasya_shulgin_rep
