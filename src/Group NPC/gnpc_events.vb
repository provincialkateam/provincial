...
if $args[0] = 'krutishi_dance':
	*clr & cla
	...
	elseif numnpc = 1 and $npc['<<numnpc>>,relation'] >= 80 and $npc['1,qwDimaRude'] > 0:
		...
		if alko < 3 and dom > -10: act 'Отказаться': gs 'npc_editor','change_rep','-',numnpc,5 & gt $loc, $metka
		act 'Идти с Димой на улицу':
			*clr & cla
			...
			if DimaRudeBlock = 1:act 'Отказаться': gs 'npc_editor','change_rep','-',numnpc,5 & gt $loc, $metka
	...
	elseif numnpc = 1 and $npc['<<numnpc>>,relation'] >= 80 and $npc['1,qwDimaRude'] = 0 and $npc['4,qwIgorLove'] = 0:
		...
		if alko < 3 and dom > -25: act 'Отказаться': gs 'npc_editor','change_rep','-',numnpc,5 & gt $loc, $metka
		act 'Идти с Димой на улицу':
			*clr & cla
			minut += 5
			gs 'stat'
			...
			if shameless['flag'] = 1 and dom > -10:
				act 'А ты никому не скажешь?':
					*clr & cla
					...
					act 'Сесть на корточки': gt 'gnpc_events','predlog_dima_bj'
					if dom >= 0: act 'Передумать': gs 'npc_editor','change_rep','-',numnpc,10  & gt 'gdk'
				end
			else
				act 'Сесть на корточки': gt 'gnpc_events','predlog_dima_bj'
			end
...
if $args[0] = 'igor_disco':
	act 'Идти на улицу':
		*clr & cla
		...
		act 'Отказаться':gs 'npc_editor','change_rep','-',numnpc,10 & gt 'gdk'
	end
	if alko < 6: act 'Отказаться': gt $loc, $metka
end
...
if $args[0] = 'drink':
	...
	if alko < 3 or alko >= 6:
		act 'Отказаться':
			*clr & cla
			if alko < 3: gs 'npc_editor','change_rep','-',numnpc,1
			gs 'npc_editor','get_npc_profile',numnpc
