...
if cloth[0] > 2 and clothesbackwater = 0 and temper >= 20:
	act'Снять одежду':
		*clr & cla
		if hantersKnowSlut > 0 or hanters = 0 or hour < 8 or hour >= 20 or shameless['flag'] > 0:
			...
			gs 'zz_render', 'Затон', 'qwest/gadukino/Swamp/backwaterundress'+iif(func('zz_clothing2','is_skirt') = 0,'','_ski')+'.jpg','Вы не стеснясь разделись, оставшись в чем мать родила.'
	...
	if cloth[0] > 0 and (cumpussy > 0 or cumbelly > 0 or cumass > 0 or cumanus > 0):
		act 'Подмыться':
			*clr & cla
...
if dirtyclothes = 1 and cloth[0] < 3:
	act 'Стирать одежду':
		*clr & cla
		if hantersRape = 1: hantersRape = 2
		minut += 60
...
if dirtyclothes = 1 and cloth[0] > 2 and temper >= 10:
...
if cloth[0] < 3 or swamp_clothes = 1:
	act 'Помыться':
		*clr & cla
		gs 'zz_bathroom','clean_me'
		manna += 10
		if hantersRape = 1: hantersRape = 2
		gs 'zz_render', 'Затон', 'qwest/gadukino/Swamp/backwaterswim'+iif(cloth[1] = 0,2,3)+'.jpg','Вы помылись в затоне.'
		act 'Далее':gt 'backwater'
	end
end
act 'Уйти':minut += 5 & gt'swamp_yard'
if hantersRape = 2: gt 'swamp_events', 'backwater_ralax'
if cloth[0] = 0 and hantersKnowSlut = 0 and hanters = 1 and hour > 8 and hour < 20 and hantersday ! day: gt 'swamp_events', 'hunter_looks'