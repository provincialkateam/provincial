if $ARGS[0] = '1':
	...
	*clr & cla
	...
	if goforestday ! daystart:
		if func('zz_clothing2','get_group') ! 2: gs 'zz_clothing2', 'decrease_condition'
		if func('zz_clothing2','is_glamour') = 1: gs 'zz_clothing2', 'decrease_condition',4
	end
	...
	$_str = 'Окраина леса'
	if snow = 0:
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest'+iif(func('zz_weather','is_day')=1,'','_night')+'1.jpg', iif(func('zz_weather','is_day')=1,'Окраина леса. Здесь мало грибов и часто можно встретить грибников.', 'Ночью в лесу страшно и неуютно.')
	else
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest_winter'+iif(func('zz_weather','is_day')=1,'','_night')+'1.jpg',iif(func('zz_weather','is_day')=1,'Окраина леса. Зимний лес очень красив.','Ночью в зимнем лесу очень страшно и неуютно.')
	end
	...
	if clothesforest = 1 and cloth[0] <= 1:
		act 'Одеться':
			*clr & cla
			clothesforest = 0
			swamp_clothes = 0
	...
	if clothforest = 1 and cloth[0] > 1:
		act 'Забрать брошеную одежду':
			cla
			clothesforest = 0
			gs 'zz_render','','','Вы забрали свою одежду.'
			act 'Назад': gt 'gadforest','1'
		end
	end
	if hour >= func('zz_weather','sunrise') and hour < func('zz_weather','sunset'):
		if cloth[0] > 2 and clothforest = 0 and temper >= 20 and sunWeather >= 0 and miratalkforest > 1 and mirainforest = 0:
		...
		if sunWeather >= 0:
			act 'Гулять по лесу':
				*clr & cla
				minut += 30
				manna += iif(cloth[0] = 0,30,20)
				if cloth[0] = 0:health += 20 & horny += 5
				gs 'stat'
				if month >= 4 and month <= 10:
					if mirainforest = 0:
						if cloth[0] > 2 and func('zz_clothing2','is_skirt') = 0: gs 'zz_render', $_str, 'qwest/gadukino/forest/gulforest0.'+rand(1,2)+'.jpg'
						if cloth[0] > 2 and cloth[1] = 1 and func('zz_clothing2','is_skirt') = 1: gs 'zz_render', $_str, 'qwest/gadukino/forest/gulforest1.'+rand(1,3)+'.jpg'
						if cloth[0] > 2 and cloth[1] = 0 and func('zz_clothing2','is_skirt') = 1: gs 'zz_render', $_str, 'qwest/gadukino/forest/gulforest_tanga0.'+rand(1,2)+'.jpg'
						if cloth[0] = 0: gs 'zz_render', $_str, 'qwest/gadukino/forest/gulforest_nude0.'+rand(1,4)+'.jpg'
						gs 'zz_render', '', '', 'Вы гуляете по лесу, наслаждаясь шумом деревьев и свежим воздухом.'
					else
						if cloth[0] > 0: gs 'zz_render', $_str, 'qwest/gadukino/forest/miraforest'+rand(1,3)+'.jpg'
						if cloth[0] = 0: gs 'zz_render', $_str, 'qwest/gadukino/forest/miraforest_nude'+rand(1,3)+'.jpg'
...
if $ARGS[0] = '2':
	...
	*clr & cla
	...
	$_str = 'Лес'
	if snow = 0:
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest'+iif(func('zz_weather','is_day')=1,'','_night')+'2.jpg', iif(func('zz_weather','is_day')=1,'От опьяняюще свежего воздуха и пения птиц на душе у вас светло и радостно.', 'Ночью в лесу страшно и неуютно.')
	else
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest_winter'+iif(func('zz_weather','is_day')=1,'','_night')+'2.jpg',iif(func('zz_weather','is_day')=1,'Зимний лес очень красив, свежий воздух поднимает вам настроение.','Ночью в зимнем лесу очень страшно и неуютно.')
	end
...
if $ARGS[0] = '3':
	...
	*clr & cla
	...
	$_str = 'Чаща'
	if snow = 0:
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest'+iif(func('zz_weather','is_day')=1,'','_night')+'3.jpg', iif(func('zz_weather','is_day')=1,'От опьяняюще свежего воздуха и пения птиц на душе у вас светло и радостно.', 'Ночью в лесу страшно и неуютно.')
	else
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest_winter'+iif(func('zz_weather','is_day')=1,'','_night')+'3.jpg',iif(func('zz_weather','is_day')=1,'Зимний лес очень красив, свежий воздух поднимает вам настроение.','Ночью в зимнем лесу очень страшно и неуютно.')
	end
...
if $ARGS[0] = '4':
	...
	*clr & cla
	...
	if snow = 0:
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest'+iif(func('zz_weather','is_day')=1,'','_night')+'4.jpg', iif(func('zz_weather','is_day')=1,'От опьяняюще свежего воздуха и пения птиц на душе у вас светло и радостно.', 'Ночью в лесу страшно и неуютно.')
	else
		gs 'zz_render', $_str, 'qwest/gadukino/forest/gadforest_winter'+iif(func('zz_weather','is_day')=1,'','_night')+'4.jpg',iif(func('zz_weather','is_day')=1,'Зимний лес очень красив, свежий воздух поднимает вам настроение.','Ночью в зимнем лесу очень страшно и неуютно.')
	end
