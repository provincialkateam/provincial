...
!	$metka = $ARGS[0]
	...
	gs 'zz_render', 'Офисное здание', 'images/city/south/south_office/south_office.jpg','На стене висит доска <a href="exec:gt ''south_office'',''ads''">объявлений</a>'+'.'
	if svQW < 1:
		gs 'zz_render', '', '', 'Офис '+func('zz_funcs','mk_link',8,17,'торгового представительства ','south_office','torgpred',1)+func('south_office_strings','hall',1)
	end
	if clener = 1:
		act 'Поговорить о работе уборщицей офиса':
			*clr & cla
			gs 'zz_render', '', 'images/city/south/south_office/administrator.jpg',+func ('south_office_strings','clener_1',10)
	...
	if clener >= 10 and clenerT = 0:
		act 'Спросить нет ли еще работы для уборщицы':
			*clr & cla
			gs 'zz_render', '', 'images/city/south/south_office/administrator.jpg',func ('south_office_strings','clener_1',11)
			act 'Отказаться и уйти': gt'south_office','start'
			act 'Согласиться на работу':
				cla
				clenerT = 1
	...
	if jouryQWNo = 1 and hour = 17:
		gs 'zz_render', '', '',func('south_office_strings','clener_2',1)
...
if $ARGS[0] = 'ads':
	*clr & cla
	minut += 5
	gs'stat'
	gs 'zz_render', 'Доска объявлений', 'images/city/south/south_office/ads.jpg','Вы подошли к доске, где расклеено множество различных объявлений'+iif(clener = 0 or logist=0,', среди которых бросаются в глаза объявления: '+'- <b>Требуются уборщицы</b>. Обращаться к администратору.','. Но ничего интересного вы не нашли.')
	if clener = 0: clener = 1
	act 'Уйти': gt'south_office',''
end
if $ARGS[0] = 'torgpred':
	gs'stat'
