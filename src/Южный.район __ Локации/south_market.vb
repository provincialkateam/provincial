...
if $ARGS[0] = 'start':
	*clr & cla
	...
	gs 'stat'
	gs 'time'
	gs 'zz_render', 'Рынок', 'images/city/south/south_market/market'+iif(month >= 4 and month <= 10,'','_winter')+'.jpg', iif(hour >= 8 and hour <= 14,'Большой и шумный рынок.**При входе на рынок можно купить <a href="exec: gt ''south_market'',''book''">книги</a>.**На самом рынке есть продуктовые ряды <a href="exec: gt ''south_market'',''food''">продуктовые ряды</a>.**Пройдя немного дальше можно купить <a href="exec: gt ''south_market'',''cosm''">бельё и косметику</a>.**Там же находятся и <a href="exec: gt ''south_market'',''hos''">хозяйственные</a> ряды.**В самом конце есть <a href="exec: gt ''south_market'',''clo''">вещевой рынок</a>.'+iif(BuyHous = 1 and (remkorr = 0 or remvanr = 0 or remkuhr = 0 or remsitr = 0 or rembedr = 0),'** На самой окраине расположилась стихийная <a href="exec: gt ''south_market'',''flea_market''">барахолка</a>.',''),'Рынок закрыт закрыт.')
	...
	if workrin = 1 and (week = 2 or week = 4 or week = 6):
		if workrin_hooky < iif(market_sex < 10,4,iif(rinslut = 0,6,10)):
			if hour < 9:
				...
				act '<b>Принимать товар</b>':
					*clr & cla
					...
					prodpayum = 1200
					workrin_hooky -= 1
					gs 'zz_render', 'Рустам', 'images/city/south/south_market/market_work/accept_goods.jpg', iif(workrin_hooky > rustam_know_hooky,'\\\ - Вай Свэта, зачэм прогуливаешь?, -/// накинулся на вас Рустам, когда вы подошли к нему.\\\ - Я за тэбя работать должэн что-ли? Смотри, дограешься, уволю'+iif(market_sex < 10,'',iif(rinslut = 0,'не смотря на твои ... эээ... качэства','не глядя на все твои "заслуги"'))+'...///**','')+'Рустам выгрузил барахло из своей ушатанной шохи к лотку и отдал вам. Вы начали развешивать одежду по палатке.**Перед тем как уйти Артур дает вам инструкции:**\\\- Продавай как хочешь, но чтобы вэчером мнэ отдала с каждой проданной вэщи по 1000. И чтоб наторговала нэ мэньше чэм <<rustam_plan>> рублей.///'
			...
			workrin = 0
			work = 0
			gs 'zz_render', '', '','Разгневаный Рустам ждёт вас у входа:**\\\- Опять прогуляла, Свэта. Маё тэрпенье лопнуло, вот твоя трудовая, вали давай!\\\**Вы забрали у него из рук трудовую и он пошёл прочь.'
		end
	...
	if analplugIN = 1 and cloth[1] = 0 and func('zz_clothing2','is_skirt') = 1 and temper > 20 and rand(1,10) = 10:
		horny += 10
...
if $ARGS[0] = 'analplug':
	*clr & cla
	gs 'stat'
	gs 'zz_render', 'Рынок', 'images/city/south/south_market/event/rinokplug.jpg','Вы почувствовали приятный, даже пьянящий, запах дорогой, мужской, туалетной воды. Внизу живота моментально "зазудело", по телу прошёл жар возбуждения.**Сзади стоял парень с определённой целью - он был любитель пощупать девушек в толпе, и сейчас намеревался пощупать вашу попку.**А рука тем временем развернулась и легла ладонью на одну из половинок вашей попки.'
	act 'Отойти':gt'south_market','start'
	act 'Далее':
		*clr & cla
		minut += 10
		gs 'stat'
		gs 'zz_render', 'Рынок', 'images/city/south/south_market/event/butplag.jpg','Парню фартило: "жертва" не выказывала неудовольствия, а его действия были закрыты от окружающих. Народу было плевать на него, все изнывали от жары.**Он передвинул руку на другую "булочку", остановил посередине, спустился чуть ниже. Начал понемногу сжимать попку в ладони. Вы не реагировали. Тогда он начал продвигать пальцы через ткань юбки вглубь, к бугорочку между ножек.**Он почувствовал, как вы вздрогнули, но не от неожиданности. Он тоже был опытен, он понял, что вы возбудились и уже "плывёте".**Вы действительно плыли. Вы были уже неподвластны себе. Вы хотели, чтобы парень забрался под юбку и вставил член... Ощущения захватывали вас, возбуждение переполняло. Вы еле сдерживались, чтобы не начать охать и стонать...'
		act 'Стоять':
			*clr & cla
			minut += 10
			orgasm += 1
			horny = 0
			gs 'zz_render', 'Рынок', 'images/city/south/south_market/event/finger.jpg','А парень, тем временем, потянул подол юбки вверх, добрался рукой до разреза, и умелым движением залез под юбку. Его рука легла на низ, влажной от испарины попки, и пальцы коснулись анальной пробки между ног.**Вы даже чуть пошатнулась, но никто этого в давке не заметил, кроме этого искусителя. Он немного замер, похоже в шоке от того, что девушка вот так вот может разгуливать по рынку без трусов и с пробкой в попе.**В голове у вас всё кружилось, вы были на пике возбуждения, ещё немного - и вы кончите.**Парню конечно было мало. Он проник во влагалище пальцами и начал их вводить, и выводить. И тут же вы задрожали, сделали несколько шумных, глубоких вздохов, а на пальцы парню брызнула влага.**\\\- Ничего себе!... -/// парень не ожидал такой реакции от вас, и даже струхнул, что его могут "запалить".**И действительно, одна женщина предпенсионного возраста спросила у вас:**\\\- Девонька, тебе плохо?///**\\- Нет, спасибо, всё уже прошло? -// собравшись с силами ответили вы.**Видимо женщина спугнула извращенца. Так как за вами уже никто не стоял.'
...
if $ARGS[0] = 'book':
	*clr & cla
	...
	if ero_books_view = 5:
		...
		ero_books_view = 6
		gs 'stat'
		gs 'zz_render', '', '','Вы рассматривали очередную книгу, выбирая, что же вам купить почитать, когда продавщица, понизив голос, спросила:**\\\- Не можете выбрать, девушка? Может вас заинтересуют более пикантные журналы? Всего по 400 рублей.///'
		...
		act 'Не интересует':
			...
			minut += 1
			gs 'stat'
			gs 'zz_render', '', '','\\- Нет, знаете, меня это не интересует, -\\ ответили вы.**\\\- Ну, сейчас может и не интересует, -/// улыбнулась вам продавщица.\\\ - А потом может заинтересовать. В любом случае, вы знаете где искать. Просто подойдите и спросите.///'
			act 'Назад': gt 'south_market', 'book'
		end
	end
end
...
if $args[0] = 'buy':
	...
	if zz_count > 0 and zz_count <= 100:
		if money >= zz_price * zz_count:
			...
			if zz_id = 2: $inventory[7+'_count'] += zz_count*10
			if zz_id = 3: panties += zz_count*500
...
if $args[0] = 'closed':
	cla
	gs 'zz_render', '', '','<red>Рынок закрыт, рабочие часы: 08.00-15.00</red>'
