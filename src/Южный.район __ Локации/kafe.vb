if $ARGS[0] = 'start':
	...
	*clr & cla
	...
	!Условия старта ивента с Самантой
	if hour = 17 and banda['madam'] = 2 and rand(1,2) = 1:	gt 'rb_madam', 'eventmadam3'
	if money >= 1500:
		act 'Поесть 500 руб':
			*clr & cla
			money -= 500
	...
	*clr & cla
	...
	if work = 0 and workKafe = 0:
		act 'Спросить возьмет ли он вас на работу':
			...
			if vnesh < 20:
				gs 'zz_render','','','Вай, вай, вай, ты мне всех клиентов распугаешь. Сходи, что ли приоденься, косметику используй, причешись. А ты выглядишь просто страшно.'
			else
...
if $ARGS[0] = 'razvod':
	...
	if deyst = 0:
		...
		if horny < 75: act 'Отшить': gt'south'
	elseif deyst = 1:
