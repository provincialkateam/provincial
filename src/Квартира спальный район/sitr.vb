...
$zz_str[4] = iif(my_house = 1,'письменный',iif(remsitr = 0,'добитый',iif(remsitr = 1,'старый',iif(remsitr = 2,'удобный','роскошный')))) + ' стол'
...
if university['student'] > 0 and (month <= 6 or month >= 9):
	if university['course'] = 1:
		act 'Писать курсовую работу': gt'uni_events','course_work'
	else
		act 'Писать дипломную работу': gt'uni_events','graduate_work'
	end
end
