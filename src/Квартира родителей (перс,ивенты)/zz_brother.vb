...
!	if $brother['location'] = 'sitrPar' and $npc['39,qwKolkaPrince'] = 1: gs 'KolkaPrince', 'main'
	...
	if spanked >= 25 and cloth[0] = 0 and cloth[1] = 0: gs 'zz_brother', 'spank_reaction' & exit
	! рандомные диалоги с братиком
!	msg $brother['location']
	if ($brother['location'] = 'kuhrPar' and hour = 7):
			gs 'zz_brother', 'smalltalk', 'kitchen_talk'
	elseif ($brother['location'] = 'sitrPar' and ((hour => 18 and hour <= 23) or (hour > 7 and hour < 11))):
		if rand(0) = 1:
			gs 'zz_brother', 'smalltalk', 'tv_talk'
		else
			gs 'zz_brother', 'smalltalk', 'hobby_talk'
		end
		exit
	end
...
if $args[0] = 'smalltalk':
	...
	elseif $npc['39,relation'] >= 40 and $npc['39,relation'] < 60:
		maxval = 4
	elseif $npc['39,relation'] >= 60 and $npc['39,relation'] < 80:
		maxval = 9
	elseif $npc['39,relation'] >= 80  and ($npc['39,sex'] = 1 or $npc['39,qwPornoFootball'] > 10):
		!убрал  пошлые под условие "был секс"
		! можно выделить в отдельную категорию
		maxval = 12
	else
		exit
	end
	gs 'zz_render', '', '', func('brother_smalltalks_data', $args[1], rand(maxval))
end
if $args[0] = 'get_off':
	if $npc['39,relation'] < 40:
		gs 'npc_editor','change_rep','-', 39
		gs 'zz_render', '', '', '\\\- Отвали, дура, я спать хочу!///'
	else
		gs 'zz_render', '', '', '\\\- Света, не мешай спать!///'
	end
	if school['vacation'] = 0 and (week < 5 or week = 7) and brotherAge < 18: gs 'zz_render', '', '', '\\\- Мне с утра вообще-то в школу!///'
end
if $args[0] = 'check_naked':
	if cloth[0] = 1:
		if $npc['39,sex'] < 1:
			gs 'zz_render','','','Ваш брат ехидно комментирует коротенькое полотенце на вас:\\\- Светик, ты уверена что это не полотенце для рук?///'
		else
			gs 'zz_render','','','Ваш брат ехидно комментирует коротенькое полотенце на вас:\\\- Сестричка, ты специально для меня выбрала полотенце покороче?///'
		end
	elseif cloth[0] = 0 and lobok > 2 and cloth[1] = 0:
		gs 'zz_render','','','\\\- Светик, ты не думала сбрить эти свои заросли? Или хоть подстичь свои джунгли?///'
	elseif cloth[0] = 0:
		if $npc['39,sex'] >= 1:
			gs 'zz_render','','','\\\Светик, у тебя потрясающее тело!///'
		else
			gs 'zz_render','','','\\\Классно выглядишь. Ты что, нудизмом решила заняться?///'
			gs 'npc_editor','change_rep','+', 39
		end
	end
end
if $args[0] = 'cum_reaction':
	if cumface + cumlip > 0 :
		...
		gs 'npc_editor','change_rep','-', 39
		gs 'zz_render','','','\\\- Вымой лицо, смотреть противно на всю эту кончу.///'
		exit
	end
	if cumfrot > 0 and cloth[0] > 2:
		gs 'zz_render','','','\\\- У тебя вся одежда перепачкалась. Постирай, или мать попроси.///'
		exit
	end
	if cumbelly > 0 and cloth[0] = 0:
		if brotherknowslut < 1: brotherknowslut = 1
		gs 'npc_editor','change_rep','-', 39
		gs 'zz_render','','','\\\- Иди помойся, дура - смотреть противно на всю эту кончу!///'
		exit
	end
	if cumass > 0 and cloth[0] <= 2 and cloth[1] = 0:
		gs 'zz_render','','','\\\- Иди помойся - а то вся жопа в конче! И барахло своё постирай обшликанное.///'
		if brotherknowslut < 1: brotherknowslut = 1
		gs 'npc_editor','change_rep','-', 39
		exit
	end
...
if $args[0] = 'spank_reaction':
	if $npc['39,relation'] < 40:
		gs 'zz_render', '', '', '\\\- Чё, дуреха, получила по заслугам?/// - не скрывая радости выдал Колька.'
	else
		gs 'zz_render', '', '', '\\\- Что у тебя с попой? Наказали за что-то?///'
	end
end
if $args[0] = 'check_pregnant':
	if cloth[0] <= 2 or (cloth[0] > 2 and pregnancy > 150):
		gs 'zz_render', '', '', 'Брат удивлённо смотрит на ваш животик: \\\- Светик, ты залетела что-ли?///'
		pregTalkBrother = 1
	end
end
if $args[0] = 'football':
	gs 'npc_editor','get_npc_profile',39
	if $npc['39,relation'] < 40:
		gs 'zz_render','','','Брат хмурится: \\\Отвали от меня, придурошная!///'
		act '<b>Уйти</b>': gt 'gschool'
		exit
	else
		if sunWeather >= 0 and month > 2 and month < 12:
			gs 'zz_render','','','Брат приветственно машет рукой: \\\- Светик, давай к нам! Может, и с нами поиграешь?///'
			act '<b>Уйти</b>': gt 'gschool'
	...
	if $npc['39,qwPornoFootball'] = 0:
		*clr & cla
		...
		act 'Вы тут о чем говорите?':
			...
			gs 'zz_render','','','Пацаны замялись: \\\- Да так, ни о чём...///'
			if dom > 0:
				gs 'zz_render','','','Вы немного надавили на них:**\\- Вы что как малышня? Секретики какие-то у вас! - ехидно ухмыльнулись вы.//**Один из пацанов почти сразу "раскололся":**\\\- Да мы, это, того... мы хотим сходить порнуху поглядеть...///'
				act 'Возьмите меня':
					cla
		...
		*clr & cla
		gs 'zz_render','','images/pavlovo/family/brother/boys_talk.jpg','Пацаны, пошушукавшись промеж собой, предлагают вам: '+iif($npc['39,qwPornoFootball']<5,'\\\Светик, пойдешь порнушку смотреть?///','\\\Да ну его, этот футбол, пойдем лучше порнушку смотреть!///')
...
if $args[0] = 'check_piercing':
	if cloth[0] = 0:
		...
		*clr & cla
		...
		brother_pirsF = 1
		if $npc['39,relation'] < 40:
			gs 'zz_render','','','\\\- Ты это всем подряд показываешь, шлюха пришибленная?/// - заходясь от смеха еле выдавил с себя Колька.'
		elseif $npc['39,relation'] >= 40 and $npc['39,relation'] < 60:
			gs 'zz_render','','','\\\- Не понимаю, зачем так издеваться над собой?///- Колька заметил ваши проколотые соски.'
		else
			horny += 3
			gs 'npc_editor','change_rep','+', 39
			if $npc['39,sex'] = 0:
				gs 'zz_render','','','\\\- Тебе идёт///, - брат, подмигнув, кивает на ваши проколотые соски.'
			else
				gs 'zz_render','','','\\\- Какая прелесть!/// - Колька играет с вашими проколотыми сосками.'
			end
		end
		act '<B>Отойти</B>': gt $loc, $metka
	else
		act 'Показать пирсинг сосков':
			*clr & cla
			minut += 1
			brother_pirsF = 1
			gs 'zz_render','','pavlovo/family/brother/pirsf.jpg','\\- Смотри, что у меня есть//, - вы подходите к Кольке поближе и оголяете грудь, показывая брату украшение в вашем соске,- \\не стесняйся, можешь потрогать//.'
			if $npc['39,relation'] < 40:
				cla
				gs 'zz_render','','','\\\- Ты это всем подряд показываешь, шлюха пришибленная?/// - заходясь от смеха еле выдавил с себя Колька.'
				act '<B>Отойти</B>': gt $loc, $metka
...
if $args[0] = 'homework_init':
	if $npc['39,qwPornoFootball'] >= 7 or $npc['39,qwBrotherNude'] >= 2:
		...
		*clr & cla
		gs 'npc_editor','get_npc_profile',39
		gs 'zz_render','','','\\\- Светик, поможешь мне с домашкой, а то у меня тут задачка не получается?///'
		act 'Конечно помогу': gt 'zz_brother', 'homework'
		act 'Потом, сейчас не могу':
			cla
			gs 'zz_render','','','\\- Извини, я сейчас занята. Потом помогу.//'
			act 'Отойти': gt $loc, $metka
		end
	else
		act 'Предложить помощь с домашкой':
			cla
			gs 'zz_render','','','\\- Коль, с уроками помощь не нужна?//'
			if $npc['39,relation'] < 40:
				gs 'zz_render','','','\\\Сам разберусь, отвали.///'
				act 'Отойти': gt $loc, $metka
			else
				gs 'npc_editor','change_rep','+', 39
				gs 'zz_render','','','\\\Да, не помешала бы.///'
	...
	elseif $npc['39,qwPornoFootball'] >= 7 or $npc['39,qwBrotherNude'] >= 3:
		...
		act 'Игнорировать и продолжить':
			*clr & cla
			...
			if hour = 21:
				...
				! ---
				gs 'zz_render','','','Вы закончили с домашкой, когда уже родители пошли спать, оставив вас с братцем самих. Колька c красным от возбуждения лицом спрашивает: \\\- Светик, может немного пошалим?///'
		...
		act 'Убрать руку и продолжить':
			*clr & cla
			horny += RAND(1,5)
			gs 'npc_editor','change_rep','-', 39
			gs 'zz_render','','images/qwest/alter/ev2/brother/homework1.jpg','\\- Не надо, вдруг родители увидят// - шепчете вы, убирая его руку с ножки.**\\\- Тю.../// - буркнул себе под нос Колька и с гримассой обиды на лице продолжил заниматься.'
			act 'Закончить': gt $loc, $metka
		end
		exit
	end
...
if $args[0] = 'show_body':
	gs 'zz_render','','images/qwest/alter/ev2/brother/homework1.jpg','\\\- Светик, у меня скоро контрольная по анатомии будет, а я всё никак выучить её не могу. Не поможешь мне?///**\\- В смысле?...// - в недоумении переспрашиваете вы.**\\\- Ну там.../// - брат немного замешкался - \\\посмотреть как у девочек всё там устроено, объяснить...///'
	if $npc['39,qwBrotherNude'] <= 3:
		cla
		if (sex + bj + hj + anal + kuni) = 0 and $npc['39,qwPornoFootball'] < 12:
			act 'Ты что совсем офигел?':
				cla
				gs 'zz_render','','','\\- Коля, ты ничего не путаешь? Я ведь твоя сестра! Ты что совсем офигел, как ты себе это представляешь?//**\\\- Ну да, ты права. Глупость ляпнул...///'
				act 'Закончить': gt $loc, $metka
			end
		else
		...
		act 'Хм...':
			...
			if ($npc['39,qwPornoFootball'] >= 12 and (sex + bj + hj + anal + kuni) = 0) or ($npc['39,qwPornoFootball'] >= 7 and (sex + bj + hj + anal + kuni) > 0):
				gs 'zz_render','','','\\- Как будто ты не видел// - хихикнули вы, вспоминая совместные просмотры порно с компанией Колькиных друганов.**\\\- Ну это не то.../// - начал мямлить он.'
			else
				gs 'zz_render','','','\\- И как ты себе это представляешь? Мне перед тобой раздеться?//**\\\- Ну... Было бы неплохо еще и рассказать/// - добавил он.'
				gs 'zz_render','','','\\- А в книжке у тебя что картинок нет?//**\\\- Есть, но там всё так коряво описано, а учительница пропустила этот урок, мол "вы и так все знаете", а контрольную ей напиши/// - начал он вас уговаривать с разгоряченным лицом.'
			end
			gs 'zz_render','','','\\- Ладно, помогу тебе, но только если никому не скажешь. Договорились? Я серьёзно: "никому" - значит вообще никому!// - строго говорите вы.**\\\- Слово пионера!///**\\- Ты не пионер...//**\\\- А... ну тогда просто честное слово!///**\\- Ладно, я поняла. Не продолжай.//'
...
if $ARGS[0] = 'brother_go_away':
	*clr & cla
	...
	dom += 1
	gs 'stat'
	gs 'zz_render','','images/qwest/alter/ev2/brother/b13.jpg','Вы прикрываете двумя руками свою киску и выкрикиваете брату:**\\- Коля, какого хрена!? Быстро свалил отсюда, мелюзга!//**Пойманный на горячем, Колька хлопает дверью и убегает прочь. Вы спокойно заканчиваете мыться и начинаете вытираться.'
	act '...': gt 'zz_bathroom'
end
if $ARGS[0] = 'brother_voyeur_end':
	*clr & cla
	...
	manna += 10
	gs 'stat'
	gs 'zz_render','','images/qwest/alter/ev2/brother/b_end'+RAND(1,2)+'.jpg','Смущённо, исподтишка глядя на брата, вы берёте полотенце и оборачиваете вокруг вашего разгорячённого, распаренного тела.**Вас бьёт дрожь от осознания неправильности и наказуемости ваших действий. Колька аккуратно прикрывает дверь и на цыпочках уходит.'
	act 'Выйти из ванной': gt 'zz_bathroom'
end
...
if $ARGS[0] = 'brother_voyeur_ev4':
	*clr & cla
	...
	if $npc['39,qwBrotherNude'] = 4:
		gs 'zz_render','','','Вы проверяете закрыта ли дверь в зал, затем молча быстро снимаете с себя всю одежду и усаживаетесь в кресло рядом с диваном, немного смущенно прикрывая свою грудь руками:**\\- Ну что "пионер", что тебе объяснять?// - спрашиваете вы, глядя на офигевшего от реальности происходящего брата.**\\\- Охренеть.../// - единственное, что смог выдавить из себя Колька, глазея на вас выпученными глазами.'
		gs 'zz_render','','','\\- Коля, ты просто полюбоваться захотел или тебе действительно нужна помощь?// - строго говорите вы, замечая увеличивающийся бугорок у него в штанах.**\\\- Нет-нет, нужна-нужна!///**\\- Так может, быстрее начнем - быстрее закончим?//**\\\- Эм-да, хорошо...///'
		act 'Далее': gt 'zz_brother','brother_voyeur_ev5'
	elseif $npc['39,qwBrotherNude'] > 4:
		gs 'zz_render','','','Вы проверяете закрыта ли дверь в зал, затем молча быстро снимаете с себя всю одежду и усаживаетесь в кресло рядом с диваном:**\\\- Ну что, повторение - мать учения?/// - спрашивает брат.**\\- Вроде того// - с улыбкой отвечаете вы.'
		act 'Далее': gt 'zz_brother','brother_voyeur_ev5'
	end
end
if $ARGS[0] = 'brother_voyeur_ev5':
	*clr & cla
	...
	$_str += '**\\\-'+iif($npc['39,qwBrotherNude'] = 4,'Блин, офигеть','Крутой вид!')+'/// - после небольшой паузы говорит брат.'
	if analplugIN = 1:
		if brother_saw_analplug = 0:
			brother_saw_analplug = 1
			$_str += '**\\\- Ого, а это еще зачем?/// - в недоумении заёрзав, спрашивает Коля, явно имея ввиду секс-игрушку в вашей попе.'
			$_str += '**\\- Мне так нравится.//'
			$_str += '**\\\- Вау! Серьёзно?///'
			$_str += '**\\- Ну да...// - смутившись, отвечаете вы.'
			$_str += '**\\\- И как ощущения?///'
			$_str += '**\\- Хочешь попробовать?//'
			$_str += '**\\\- Эм... Нет, спасибо. А, всем девчонкам такие вещи нравятся?///'
			$_str += '**\\- Не знаю... Я у всех не спрашивала// - краснея, отвечаете вы.'
		else
			$_str += '**\\\- Опять ты с этой штукой в заднице?/// - спрашивает Коля, явно имея ввиду секс-игрушку в вашей попе.'
			$_str += '**\\- Ага...// - кокетливо отвечаете вы.'
			if $npc['39,sex'] = 1:
				$_str += '**\\\- Может лучше туда что-нибудь другое вставлять?/// - спрашивает он, явно намекая.'
				$_str += '**\\- В другой раз, Коля//'
				$_str += '**\\\- Облом...///'
			else
				$_str += '**\\\- Извращенка!///'
				$_str += '**\\- Ну разве что совсем чуть-чуть//'
			end
		end
	else
		if brother_anus_mem < anpic:
			brother_anus_mem = anpic
			$_str += '**\\\- Светик, мне кажется или у тебя очко шире стало?///'
			$_str += '**\\- Чего?//'
			$_str += '**\\\- Я серьезно, раньше меньше было!///'
			$_str += '**\\- Не выдумывай, ничего там не изменилось и не надо на него пялиться!//'
			if $npc['39,qwPornoFootball'] < 22:
				$_str += '**\\\- Изменилось, точно говорю! Раньше меньше было! Светик, ты даешь в жопу?///'
				$_str += '**\\- Охренел? Нет конечно!// - краснеете вы.'
				$_str += '**\\\- Точно даёшь!///'
				$_str += '**\\- Коля, иди нахрен!//'
				$_str += '**\\\- Ладно-ладно, молчу...///'
			end
		end
	end
	$_str += '**\\- Ты так и будешь таращиться?//'
	$_str += '**\\\- Ладно, давай начнём.///'
	$_str += '**Вы начинаете объяснять брату всю разницу между мужским и женским телом, зачем женщине грудь и тому подобное, попутно борясь с возбуждением вперемешку со стыдом от осознания того, что вы стоите раком голая перед своим родным братом.'
	$_str += '**\\\- А это больно?/// - перебивает вас брат.'
	$_str += '**\\- Что?//'
	$_str += '**\\\- Ну, когда в тебя пихают член?///'
	if vagina > 0:
		$_str += '**\\- Только в первый раз или два.//'
		if brother_knows_not_virgin = 0:
			brother_knows_not_virgin = 1
			$_str += '**\\\- Ты уже пробовала?///'
			$_str += '**\\- Неважно...//'
			$_str += '**\\\- Ну, скажи!///'
			if sex = 0:
				$_str += '**\\- Нет, не пробовала.//'
			elseif sex >= 1 and sex < 10:
				$_str += '**\\- Ну, было несколько раз// - отвечаете вы.'
			else
				$_str += '**\\- Ну было дело, пробовала// - отвечаете вы.'
				$_str += '**\\\- А много раз уже?///'
				$_str += '**\\- '+iif(sex >= 20,'Много','Достаточно')+'.//'
			end
		end
	else
		$_str += '**\\- Не знаю, я не пробовала.//'
		$_str += '**\\\- Понятно...///'
	end
...
if $ARGS[0] = 'brother_voyeur_ev6':
	*clr & cla
	...
	if $npc['39,qwBrotherNude'] = 4:
		$_str += '**\\\- Света, а можешь поближе показать?///**'
		$_str += '\\- Что показать?//**'
		$_str += '\\\- Ну ты поняла...///**'
		$_str += '\\- Тебе мало что ли? И так далеко зашло...// - возмущённо отвечаете вы.**'
		$_str += '\\\- Ну покажи!///**'
	elseif $npc['39,qwBrotherNude'] > 4:
		$_str += '\\\- Светик, а покажи поближе?///**'
		$_str += '\\- Опять?//**'
		$_str += '\\\- Снова/// - улыбается он - \\\ну давай, чё ты ломаешься?///**'
	end
	if brother_knows_not_virgin = 1: $_str += '\\- Коль, ты же всё это уже видел...//**'
...
if $ARGS[0] = 'brother_voyeur_ev7':
	*clr & cla
	...
	if $npc['39,qwBrotherNude'] = 4:
		...
		$_str += '**\\\- Охренеть!.../// - вырывается дрожащим голосом у вашего брата.'
		$_str += '**\\- Всё? Доволен?//'
		$_str += '**\\\- Подожди-подожди, дай рассмотреть!///'
		$_str += '**\\- Всё, хватит.// - вы убираете руки и встаёте.'
		$_str += '**\\\- Ну, Света!///'
		$_str += '**\\- Я сказала всё!// - строго отрезаете вы и одеваетесь.'
		gs 'zz_render','','',$_str
		killvar '$_str'
		act 'Отойти': gt $loc, $metka
	elseif $npc['39,qwBrotherNude'] > 4:
		...
		$_str += '**\\\- Охуенно!/// - вырывается у вашего брата.'
		$_str += '**\\- Доволен?// - спрашиваете вы.'
		$_str += '**\\\- Почти...///'
		if brother_knows_not_virgin = 1 and vagina > 0 and rand(1,2) = 1:
			$_str += '**\\- Что еще?//'
			$_str += '**\\\- Светик, а можешь пальчик внутрь засунуть?///'
			$_str += '**\\- Зачем?//'
			$_str += '**\\\- Ну, мне интересно посмотреть.///'
			act 'Отказать':
				cla
				$_str += '**\\- Нет, конечно!</h>'
				$_str += '**\\\- Почему?/// - в недоуменнии спрашивает он.'
				$_str += '**\\- Сам как думаешь?// - строго говорите вы и встаёте, начиная одеваться.'
				gs 'zz_render','','',$_str
				killvar '$_str'
				act 'Отойти': gt $loc, $metka
			end
			act 'Засунуть':
				*clr & cla
				minut += RAND(5,10)
				$_str += '**Вы, не говоря ни слова, вводите палец во влагалище и затем ещё один. Вторая рука машинально потянулась к клитору.'
				if vagina < 5:
					horny += RAND(2,5)
					$_str += '**Скользкие стенки вашей киски очень плотно обхватили пальчики.'
				elseif vagina >= 5 and vagina < 10:
					horny += RAND(5,10)
					$_str += '**Вы чувствуете скользкие стенки вашей киски вокруг собственных пальчиков.'
				elseif vagina >= 10:
					horny += RAND(2,5)
					$_str += '**Ваша киска довольно просторна для пары пальцев, и вы, двигая пальцами, чувствуете скользкие стенки вашей киски.'
				end
				$_str += '**Сгорая от желания, вы, сами того не понимая, начинаете мастурбировать прямо перед лицом вашего брата.'
				$_str += '**Накопившееся возбуждение полностью затуманило ваш рассудок и вы даже не обратили внимание на то, что Колька, глядя на вас, занялся тем же самым прямо напротив.'
			...
			if $npc['39,qwBrotherNude'] = 5:
				$_str += '**\\- Что еще?//'
				$_str += '**\\\- Помоги мне сбросить напряжение/// - выдаёт Колька.'
				$_str += '**\\- Ты охуел?//'
				$_str += '**\\\- Просто не одевайся и посиди на месте - я сам всё сделаю!///'
			elseif $npc['39,qwBrotherNude'] > 5:
				$_str += '**\\- Что еще?//'
				$_str += '**\\\- Давай как в прошлый раз? Мне очень понравилось/// - выдаёт Колька.'
			end
			gs 'zz_render','','',$_str
			killvar '$_str'
			if dom > -30: act 'Отказать и одеваться': gt $loc, $metka
	...
	act 'Далее':
		*clr & cla
		...
		if $npc['39,qwBrotherNude'] = 5:
			...
			$_str += '**\\- Офигеть!// - взволнованно почти выкрикиваете вы, глядя на капли спермы у себя на животе.'
			$_str += '**\\\- Ага.../// - Колька тяжело переводит дыхание.'
			$_str += '**\\- Блядь!.. Ты... НЕТ СЛОВ!//'
			$_str += '**\\\- А мне понравилось.../// - отвечает он.'
			$_str += '**\\- Никому! Слышышь?! НИКОМУ НИ СЛОВА!//'
			$_str += '**\\\- Да понял я, понял. Не кричи, а то мама услышит!///'
		elseif $npc['39,qwBrotherNude'] >= 6:
			$_str += 'Придя в себя, вы поднимаете голову на улыбающегося брата:'
			$_str += '**\\- Опять всю меня забрызгал...//'
			$_str += '**\\\- Ага.../// - Колька тяжело переводит дыхание.'
			$_str += '**\\- Засранец! Ладно, пойду, пока никто нас не застукал.//'
			$_str += '**\\\- Давай... Было круто, Светик!///'
		end
...
if $ARGS[0] = 'brother_voyeur_ev9':
	*clr & cla
	...
	$_str += '**Вы терпеливо ждёте, бросая взляды то на него, то на дверь в комнату, чем еще больше смущаете братца.'
	if $npc['39,qwPornoFootball'] <= 10:
		$_str += '**Наконец, спустя несколько минут, он кончает, снова попадая на вас. Не говоря ни слова, вы одеваетесь и уходите.'
		gs 'zz_funcs', 'cum', 'belly'
		gs 'zz_render','','',$_str
		killvar '$_str'
		act 'Уйти': gt $loc, $metka
	else
		$_str += '**\\\- Светик, помоги мне/// - прошептал Колька.'
		gs 'zz_render','','',$_str
		killvar '$_str'
		cla
		gs 'zz_brother','brotherSexStart'
	...
	if $npc['39,sex'] > 4:
		act 'Сосать у брата':
			*clr & cla
			...
			gs 'zz_render','','','Вскоре Колька застонал: "Светик, я сейчас кончу!"'
			act 'Сосать дальше':
				*clr & cla
				minut += 5
				horny += 10
			...
			act 'Вынуть изо рта':
				*clr & cla
				...
				gs 'zz_render','','common/sex/facial/<<rand(0,4)>>.jpg','Вы вынули изо рта член и тут же вам в лицо ударила струя горячей спермы, потом еще и еще. Наконец брат прекратил кончать на ваше лицо.'
				gs 'zz_brother','brotherSexEnd'
			end
...
if $ARGS[0] = 'brotherSexEnd':
	...
	gs 'stat'
	if $loc = 'sitrPar':
		gs 'zz_render','','','Колька дал вам немного передохнуть, после чего сказав: \\\- '+iif(dom<0,'Молодец, Светик, хорошо поработала, а теперь вали давай!','Спасибо, Светик, ты мне очень помогла')+'///, продолжил заниматься своими делами.'
	else
		gs 'zz_render','','','Колька сказав: \\\- '+iif(dom<0,'Молодец, Светик, хорошо постаралась, а теперь вали давай!','Спасибо, сеструх. Ну я пошёл.')+'///'
	end
	act 'Уйти': gt $loc, $metka
end
