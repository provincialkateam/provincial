...
if $sister['location'] = 'kuhrPar':
	minut += 1
	gs 'zz_render','','','\\\- Света, некогда болтать, я спешу!///'
	exit
end
...
if $npc['38,qwKolka'] = 10 and $npc['38,qwmisha'] = 0:	act '<B>Спросить про Михаила</B>' : gt 'sisterQW','incest_event10sub'
if $npc['38,wedding'] = 1 and family_trip = 0 :	act 'Спросить о вечеринке' : gt 'sisterQW','incest_dialog1'
if $npc['38,anayslut'] = 1 and family_trip = 0 : act 'Спросить о вечеринке' : gt 'sisterQW','incest_dialog2'
if anya_event['init_relocation_day'] >= daystart and $npc['38,wedding'] = 2: act 'Поговорить о переезде' : gt 'sisterQW','incest_dialog3'
! walk_prepeare
if (hour = 10 or hour = 11) and week >= 6:
	*clr
	gs 'zz_render','','pavlovo/family/sister/progulka.jpg','По комнате ходит, собираясь на прогулку, ваша сестра Аня. Она любуется собой в зеркале и вертит задом, пытаясь разглядеть себя получше. Аня, не поворачиваясь к вам, говорит: **\\\- Светик, если надумаешь погулять, то я после работы возле клуба с друзьями или на пляже. Или в кафешку в парке пойдём если будет дождь. С пяти до девяти вечера - я у своего.///'
	exit
end
! ----
...
if hour >= 7:
	if cloth[0] = 0:
		gs 'zz_render','','',func('sister_strings',iif($npc['38,qwSisterLesbi'] = 0,'local_str4','local_str5'))
	end
	if cloth[0] <= 2 and pregTalkSister = 0 and pregnancy > 90:
		gs 'zz_render', '', '', func('sister_strings', 'local_str6')
		pregTalkSister = 1
	end
	if cloth[0] > 2 and pregTalkSister = 0 and pregnancy > 150:
		gs 'zz_render', '', '', func('sister_strings', 'local_str7')
		pregTalkSister = 1
	end
	if cumlip > 1 or cumface > 0 or cumfrot > 0 and func('zz_reputation','get') < 3:
		sisterknowslut = 1
		gs 'zz_render', '', '', func('sister_strings', 'local_str8')
	elseif cumlip > 1 or cumface > 0 or cumfrot > 0 and func('zz_reputation','get') >= 3 and sisterknowslut = 0:
		sisterknowslut = 1
	...
	if $npc['38,relation'] > 65 and mesec > 0 and pregnancy = 0 and tampon = 0:
