if $ARGS[0] = 'start':
	cla
	if health <= 0:
		gs 'zz_render','','','<b><font color = red>Вы проиграли техническим нокаутом</font></b>'
		xgt'enderKik','loss'
		exit
	elseif healthV <= 0:
		gs 'zz_render','','','<b><font color = green><<$boy>> проиграла техническим нокаутом</font></b>'
		xgt'enderKik','win'
		exit
	end
	if round > 0:
	...
	gs 'zz_render','','','<b><<$boy>></b> жизнь <b><font color = red><<healthV>></font></b>'
	gs 'zz_render','','','Ваша жизнь <b><font color = red><<health>></font></b>'
	gs 'zz_render','','','<b>= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =</b>'
	gs 'zz_render','','','Ваши очки <<pointKik>>, очки соперника <<pointKikV>>'
	gs 'zz_render','','','<b>= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =</b>'
	gs'initKik'
	if hod = 0:
		!игрок атакует
		gs 'zz_render','','','<font color = green>Вы можете атаковать</font>'
		act 'Нанести быстрый Джеб':
			...
			gs 'zz_render','','','<font color = green>Вы наносите быстрый джеб с шагом вперед.</font>'
			gs'Kikatak','player'
			xgt'kikbox','sta'
		end
		act 'Нанести силовой удар':
			...
			koefpop = koefatak*20/100
			popad = RAND(koefatak - koefpop,koefatak + koefpop)
			gs 'zz_render','','','<font color = green>Вы наносите мощный удар рукой.</font>'
			gs'Kikatak','player'
			xgt'kikbox','sta'
		end
		act 'Нанести удар ногой':
			...
			koefpop = koefatak*20/100
			popad = RAND(koefatak - koefpop,koefatak + koefpop)
			gs 'zz_render','','','<font color = green>Вы наносите мощный пинок ногой.</font>'
			gs'Kikatak','player'
			xgt'kikbox','sta'
		end
	elseif hod = 1:
		!противник атакует
		gs 'zz_render','','','<font color = red><<$boy>> атакует</font>'
		tiprand = RAND(0,2)
		if tiprand = 0:
			...
			koefpop = koefatak*20/100
			popad = RAND(koefatak - koefpop,koefatak + koefpop)
			gs 'zz_render','','','<font color = red><<$boy>> наносит быстрый джеб с шагом вперед.</font>'
			gs'Kikatak','enemy'
			xgt'kikbox','sta'
		elseif tiprand = 1:
			...
			koefpop = koefatak*20/100
			popad = RAND(koefatak - koefpop,koefatak + koefpop)
			gs 'zz_render','','','<font color = red><<$boy>> наносит силовой удар рукой.</font>'
			gs'Kikatak','enemy'
			xgt'kikbox','sta'
		elseif tiprand = 2:
			damkoef = strenV*20/100
			...
			koefpop = koefatak*20/100
			popad = RAND(koefatak - koefpop,koefatak + koefpop)
			gs 'zz_render','','','<font color = red><<$boy>> наносит мощный удар ногой.</font>'
			gs'Kikatak','enemy'
			xgt'kikbox','sta'
		end
	end
...
if $ARGS[0] = 'sta':
	...
	if minround = 5:
		...
		minround = 0
		round += 1
		gs 'zz_render','','','Прозвенел гонг обозначающий конец раунда'
		if round <= formula:
			act 'Отдыхать в углу':
				cla
				gs 'zz_render','','','Вы отдохнули в своем углу.'
		...
		elseif round > formula and sparing = 1:
			act 'Уйти с ринга':gt 'fitness','dressroom'
		end
	end
end