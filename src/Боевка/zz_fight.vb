...
if _finish = 1:
	$_str += '**Удар получился хлестким, но точным и весьма сильным - ваш соперник заточился, а потом и вовсе упал на задницу.'
elseif _finish = 2:
	$_str += '**Удар получился точным и сильным - <<$enemy[_enemy]>> в ступоре, а вы нанесли еще парочку ударов, а потом еще и еще. Наконец, <<$enemy[_enemy]>> упал'+iif(_enemy>=4,'а','')+' на земь и заумолял'+iif(_enemy>=4,'а','')+' больше '+iif(_enemy>=4,'ее','его')+' не бить.'
elseif _finish = 3:
	health -= 20
	$_str += '**Удар получился точным и сильным, но <<$enemy[_enemy]>> тоже не промах. Обмениваясь ударами, вы таки посадили '+iif(_enemy>=4,'ее','его')+' на жопу, но в подарок после драки вам остался разбитый нос.'
elseif _finish = 4:
	health -= 40
	$_str += '**То ли удар не получился, то ли <<$enemy[_enemy]>> половчее, но последнее, что вы запомнили, перед тем как рухнуть на земь - это вспышку в глазах и пронзительный свист в левом ухе.'
elseif _finish = 5:
	health -= 60
	$_str += '**Но <<$enemy[_enemy]>> явно сильнее вас в уличных драках и, несмотря на все ваше сопротивление, без особых усилий сбивает вас с ног, заставляя признать поражение и умолять о пощаде.'
else
	health -= 80
	$_str += '**Но <<$enemy[_enemy]>> явно сильнее вас в уличных драках и несколькими ударами умудряется разбить вам нос, заставляя признать поражение и умолять о пощаде.'
end
	...
	*clr & cla
	...
	if _finish >= 4:
		...
		elseif _enemy = 3:
			...
			money = 0
			gs 'stat'
			gs 'zz_render','','common/fight/loss_enemy3.jpg','Вы упали на землю. Бандит закурил и обыскал вас, забрал все ваши деньги и сказал:**\\\- Надеюсь ты усвоила урок шлюха! В следующий раз я не буду таким добрым, накажу по настоящему.///'
			act 'Уйти': gt $loc, $metka
		elseif _enemy = 4:
			killvar '_enemy'
			money = 0
		...
		elseif _enemy = 5:
			...
			if zverRageQW = 10:
				gs 'zz_render','','','Кристина Зверева побила вас и вы упали на землю заплакав перед всей школой. Она злобно прошипела вам:**\\\- Теперь я превращу твою жизнь в ад, мразь!///**Она наклонилась к вашему лицу и сказала:**\\\- Открой рот падла и высунь язык.///**Вы послушались Кристину и открыли рот высунув язык. Кристина тут же смачно харкнула вам в рот:**\\\- Глотай падла!///**Вы перед всей школой униженно проглотили ее харчек, после чего Кристина еще раз плюнула, попав уже вам в лицо. Поиздевавшись над вами, Кристина пару раз пнув вам по ребрам развернулась, и пошла со школьной площадки.'
			else
				gs 'zz_render','','','Кристина Зверева побила вас и вы упали на землю заплакав перед всей школой. Она злобно прошипела вам:**\\\- Что, сука, одного раза мало было?///**Она наклонилась к вашему лицу и сказала:**\\\- Открой рот падла и высунь язык.///**Вы послушались Кристину и открыли рот высунув язык. Кристина тут же смачно харкнула вам в рот:**\\\- Глотай падла!///**Вы перед всей школой униженно проглотили ее харчек, после чего Кристина еще раз плюнула, попав уже вам в лицо. Поиздевавшись над вами, Кристина пару раз пнув вам по ребрам развернулась, и пошла со школьной площадки.'
			end
			act 'Уйти': gt 'gschool'
		end
	else
		...
		elseif _enemy = 4 or _enemy = 5:
			...
			$npc['<<numnpc>>,group'] = 0
			gs 'stat'
			gs 'zz_render','','common/fight/win_enemy6.jpg','Кристина Зверева упала на землю, вы схватили ее за волосы:**\\- Ну что, сучка, говори, сосала ты у физрука, или нет?//**Кристина, заревев, призналась перед всей школой в том, что она сосала хуй у физрука.'
			act 'Уйти': gt 'gschool'
		end
	end
end