...
if $ARGS[0] = 'start':
	*clr & cla
	...
	anuta['horny'] = 70
	gs 'stat'
	gs 'zz_render', '','images/city/south/lake/anuta/walk'+iif(vnesh < 40,1,2)+'.jpg','Загорая, вы '+iif(anutaQW = 1,'опять ','')+'заметили бредущую по пляжу подвыпившую девушку с бутылкой вина в руке, которая лениво осматривает пляж, как будто кого-то ищет среди отдыхающих.**'+iif(vnesh < 40,'Она медленно прошла мимо, даже не взглянув на вас. Видимо вы не та, кого она высматривает','Заметив ваш взгляд, она, улыбнувшись в ответ, подходит к вам:**<do>- Привет! Не против если я рядом присяду?</do>')
	if vnesh < 40:
		...
		act 'Не против, присаживайся':
			anutaQW = 2
			*clr & cla
			gs 'zz_render', '','images/city/south/lake/anuta/talk1.jpg','Девушка опускается на песок рядом с вами:**<do>- Меня Аня зовут. Но мне больше нравится когда меня Анютой называют...</do> - широко улыбается она.**<dh>- <<$name[1]>>, -</dh> в ответ представляетесь вы.**<do>- <<$name[1]>>... -</do> тянет она, словно пробуя на вкус и смакуя имя.<do> - Значит <<$name[2]>>, - можно я так тебя буду называть?</do>'
			act 'Можно':
				*clr & cla
				gs 'zz_render', '','images/city/south/lake/anuta/talk2.jpg','<dh>- Можно, конечно, -</dh> улыбаетесь вы в ответ.**<do>- Вот и отлично, -</do> просияла ваша новая знакомая.<do> - Тогда может отметим наше знакомство?</do>**Она кивает на бутылку вина, лежащую у неё в ногах.'
				act 'Давай':
					*clr & cla
					minut += 60
					gs 'zz_funcs', 'alko', 5
					gs 'stat'
					gs 'time'
					gs 'zz_render', '','images/city/south/lake/anuta/talk3.jpg','Вы с новой знакомой находите на пляже уединённое местечко подальше от посторонних глаз, и раскупориваете бутылочку. Стаканчиков нет - так что вы по очереди прикладываетесь к бутылке, передавая её друг дружке, весело болтая между собой.**Распив бутылку, вы расслаблено облокотились на новую подругу. И вдруг почувствовали её руку, нежно поглаживающую вашу спину. И это весьма приятно!'
					act 'Не реагировать':
						*clr & cla
						gs 'zz_render', '','images/city/south/lake/anuta/kiss1.jpg','Видя, что вы не против, Анюта медленно приблизила лицо к вашему, ваши глаза встретились. Ещё мгновение - и ваши губы сливаются в жарком поцелуе.'
						act 'Целоваться': gt 'anuta_events','kiss'
					end
					if horny < 70 and shameless['flag'] < 2:
						act 'Убрать руку':
							*clr & cla
							gs 'zz_render', '','images/city/south/lake/anuta/talk4.jpg','<dh>- Анют, ты только не обижайся, но я '+iif(lesbian = 0,'не такая, мне мальчики нравятся','не хочу сейчас')+'.</dh>**<do>- Ну ладно, нет так нет, -</do> улыбнулась Анюта, подмигнув вам.<do> - Может в другой раз... Я тут, на пляже, часто бываю в хорошую погоду, приходи как-нибудь.</do>**Девушка поднялась, и махнув на прощание рукой ушла, оставив вас в одиночестве.'
			...
			act 'Нельзя':
				*clr & cla
				gs 'zz_render', '','images/city/south/lake/anuta/talk5.jpg','<do>- Ну ты и бука, -</do> пьяненько ухмыльнулась Анюта.<do> - Если оттаешь - присоединяйся: я тут часто бываю в хорошую погоду.</do>**Девушка, покачнувшись, поднялась, и махнув на прощание рукой, побрела к выходу с пляжа.'
				act 'Далее': gt'nudelake'
			end
		end
		act 'Против':
...
if $ARGS[0] = 'alko':
	*clr & cla
	minut += 5
	gs 'stat'
	gs 'zz_render', '','images/city/south/lake/anuta/walk2.jpg','Загорая, вы заметили лениво шатающуюся по пляжу подвыпившую Аню с бутылкой вина в руке. Увидев вас, она обрадовалась:**<do>- О, привет, <<$name[2]>>, скучаешь?</do>И не дожидаясь приглашения, плюхнулась на песок рядом с вами.'
	act 'Далее':
		*clr & cla
		gs 'zz_render', '','images/city/south/lake/anuta/talk2.jpg','- Что-то скучно сегодня, -</do> вздыхает Анюта и поглядывает на вас:<do> - <<$name[2]>>, может составишь мне компанию?</do>**Она кивает на бутылку вина, лежащую у неё в ногах. '
		act 'Давай':
			*clr & cla
			minut += 60
			gs 'zz_funcs', 'alko', 5
			...
			if horny < 70 and shameless['flag'] < 2:
				act 'Убрать руку':
					*clr & cla
					gs 'zz_render', '','images/city/south/lake/anuta/talk4.jpg','<dh>- Анют, ты только не обижайся, но я не хочу сейчас.</dh>**<do>- Ой, ну ты и зануда, -</do> разачарованно буркает Анюта.<do> - Ну ладно, может в другой раз...</do>**Девушка поднялась, и прихватив бутылку с вином ушла, оставив вас в одиночестве.'
					act 'Уйти': gt'nudelake'
				end
			end
		end
		act 'Нет, неохота':
			*clr & cla
			minut += 10
			gs 'stat'
			gs 'zz_render', '','images/city/south/lake/anuta/talk1.jpg','<dh>- Нет, неохота, Анют -</dh> отказываетесь вы. - Ты только не обижайся, ...**<do>- Да ладно, нет так нет, -</do> улыбается разочарованно Анюта.<do> - Может в другой раз... </do>**Поболтав с вами ещё минут десять, девушка поднялась, и махнув на прощание рукой, побрела к выходу с пляжа.'
			act 'Уйти': gt'nudelake'
		end
	end
end
if $ARGS[0] = 'talk':
	*clr & cla
	...
	gs 'stat'
	gs 'time'
	gs 'zz_render', '','images/city/south/lake/anuta/talk'+rand(8,9)+'.jpg','В течении часа вы с Анютой сидите, обнявшись, на берегу, просто болтая о всяких пустяках.**<do>- Ой, <<$name[2]>>, заболталась я с тобой, -</do> взглянув на часы, спохватывается Аня.<do>- Идти мне пора...</do>** Быстро одевшись и махнув на прощание рукой, она направилась к выходу с пляжа.'
...
if $ARGS[0] = 'kiss':
	*clr & cla
	...
	protect = 1
	gs 'stat'
	gs 'zz_render', '','images/city/south/lake/anuta/tits'+iif(horny < 50,1,2)+'.jpg','Вы с Аней страстно целуетесь, забыв обо всём на свете. Её руки блуждают по вашему '+iif(horny < 50,'','возбуждённому ')+'телу, доставляя наслаждение. Оторвавшись от ваших губ, она опускается ниже и впивается губами в вашу грудь, лаская и дразня язычком сосок, '+iif(horny < 50,'и одновременно запустив руку к вашей киске, возбуждая вас не на шутку','сорвав с ваших уст сладострастный стон')+'.'
...
if $ARGS[0] = 'kiss2':
	*clr & cla
	...
	protect = 1
	gs 'stat'
	gs 'zz_render','','images/city/south/lake/anuta/tits'+iif(horny < 90,3,4)+'.jpg','Вы отрываетесь от Аниных губ и спускаетесь к её груди.'+iif(horny < 90,' Вскользь поцеловав сосок, вы принимаетесь играться с ним, то игриво щекоча его кончиком языка, то нежно облизывая круговыми движениями',' Вы буквально впились в её сосок, обхватив его губами, и страстно ласкаете его, то посасывая, то теребя кончиком языка')+', вырывая из груди девушки стоны наслаждения.'
...
if $ARGS[0] = 'variant':
	...
	if you_orgasm = 0 or girl_orgasm = 0:
		...
		elseif horny < anuta['horny'] and you_orgasm = 0 or horny > anuta['horny'] and girl_orgasm = 1:
			...
			if stat_vgape < 2:act 'Попросить вставить бутылку вам в киску': gt'anuta_events','ubvag'
			if stat_agape < 2:act 'Попросить вставить бутылку вам в попу': gt'anuta_events','ubanal'
			if stat_agape < 2:act 'Сесть попой на бутылку': gt'anuta_events','ubsit'
		else
...
if $ARGS[0] = 'end':
	*clr & cla
	...
	protect = 0
	gs 'stat'
	gs 'zz_render', '', 'images/city/south/lake/anuta/end.jpg', 'Вы с Анютой уставшие, но весьма довольные, ещё какое-то время сидите рядом, отдыхая и лениво болтая о всяких пустяках. Наконец Анюта, глянув на часы, сказала что ей пора уходить и стала одеваться.**<do>- '+iif(anuta['sex'] = 1,'Я тут, на пляже, часто бываю в хорошую погоду, приходи как-нибудь','До скорой встречи, <<$name[2]>>')+', -</do> сказала она вам и улыбнувшись на последок ушла.'
	act 'Уйти': gt 'nudelake'
end