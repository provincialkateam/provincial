if $ARGS[0] = 'youry_start':
	*clr & cla
	...
	act 'Далее':
		*clr & cla
		gs 'zz_render', '', 'images/city/north/youry/youry.jpg','Он подошел к вам и заметно нервничая сказал вам тихо:**\\\- Привет, меня зовут Юра. Ты мне очень понравилась. Может сходим куда-нибудь после работы?///'
		act 'Нет':
			*clr & cla
			jouryQWNo = 1
		...
		act 'Да':
			...
			minut += 15
			gs 'stat'
			gs 'zz_render', '', '','Юрий сказал что он сейчас закончит работу и встретит вас на проходной в здание.**После выполненной работы вы зашли к коменданту здания Борису Ивановичу. Он дал вам 100 рублей за работу.**Когда вы вышли от коменданта, на проходной вас уже ждал Юрий. Он довольно сильно взволнован и кажется, как будто он лом проглотил. Не зная что сказать он выпалил:**\\\- Может поедем ко мне? Кино посмотрим, шампанского выпьем, поговорим...///'
...
if $ARGS[0] = 'youry_talk':
	*clr & cla
	minut += 15
	gs 'stat'
	gs 'zz_render', '', 'images/city/north/youry/youry.jpg','Юрий подошел к вам и сказал:**\\\- Света, может сходим попьем кофе?///'
...
if $ARGS[0] = 'youry_talk2':
	*clr & cla
	...
	act 'Далее':
		*clr & cla
		gs 'zz_render', '', 'images/city/north/youry/youry.jpg','Увидев вас он подошел и сказал:**\\\- Света, не подходи ко мне в офисе. Ну ты же понимаешь, коллектив не поймет, если узнает, что я встречаюсь с уборщицей...///'
		act 'Ты меня стесняешься?':
			*clr & cla
			gs 'zz_render', '', 'images/city/north/youry/youry.jpg','Вы оторвались от унитаза и взглянув Юрию в глаза спросили:**\\- Ты меня стесняешься?//**Юрий замялся**\\\- Ну не то что бы стесняюсь, но ты пойми, я менеджер, а ты уборщица. Нас не должны видеть вместе. Но я хочу с тобой продолжать дальше встречаться. Ты знаешь где я живу, заезжай ко мне после четырёх вечера...///'
			act 'Да пошел ты':
				*clr & cla
				dom += 1
				jouryResult = 1
...
if $ARGS[0] = 'stasik_work2':
	*clr & cla
	gs 'stat'
	gs 'zz_render', '', 'images/city/south/stas/stas.jpg', 'Вы подошли к сидевшему за столом Стасу.**\\- Стас, я '+iif(svQW = 1,'передумала','подумала')+' и решила принять твоё предложение, -// сказали вы ему.**\\\- '+iif(svQW = 1,'Хорошо','Отлично!')+', -/// заулыбался Стасик.\\\ - '+iif(svQW = 1,'Тогда давай сходим в кафе, обсудим наши дальнейшие перспективы','Напомню правила. В северном, центральном и Южном районах находятся супермаркеты. Ты будешь заходить в наш офис, брать задание на работу, потом ехать во все эти супермаркеты и расставлять там нашу продукцию согласно заданию. На кассе возьмешь чек подтверждающий то, что ты была на работе. После этого ты возвращаешься в офис и мы выплачиваем тебе вознаграждение')+'.///'
...
if $ARGS[0] = 'stasik_talk':
	act 'Отлично!':
		*clr & cla
		...
		TPmainQW = 1
		gs 'stat'
		gs 'zz_render', '', 'images/city/south/stas/stas.jpg', 'Вы ответили Стасу, что все отлично, и он довольно улыбнулся:**\\\- Наша фирма только развивается, и для умного человека тут найдется место и получше. Кстати, давай сходим в кафе, ты еще не проставлялась за работу.///'
	...
	act 'Так себе':
		*clr & cla
		minut += 5
		gs 'stat'
		gs 'zz_render', '', 'images/city/south/stas/stas.jpg', 'Вы ответили Стасу, что так себе, и он расстроенно нахмурился:**\\\- Светик пойми, наша фирма только развивается, но для умного человека тут может найтись место и получше. Может сходим в кафе, обсудим дальнейшие перспективы?///'
		act 'Согласиться': gt 'south_office_events','stasik_yes'
		act 'Отказаться': gt 'south_office_events','stasik_no'
	end
end
...
if $ARGS[0] = 'stasik_no':
	*clr & cla
	minut += 15
	gs 'stat'
	gs 'zz_render', '', 'images/city/south/stas/stas.jpg', 'Вы ответили отказом и Стас '+iif(TPmainQW = 1,'заметно расстроился:**\\\- Ну ладно, я хотел лишь помочь тебе...///','нахмурился:**\\\- Как хочешь, я хотел лишь помочь тебе, но раз тебя эта работа не устраивает, считай, что ты уволена...///')
...
if $ARGS[0] = 'stasik_bf':
	*clr & cla
	...
	act 'Отказать':
		...
		TPmainQW = 3
		gs 'stat'
		gs 'zz_render', '', '', 'Вы довольно резко отказали Стасу, на что он обиделся:**\\\- Ну и ладно! Я то думал, что у нас что-то большее, чем случайный перепих...///'
		act 'Далее': gt'south_office','torgpred'
	end
end
if $ARGS[0] = 'security_start':
	*clr & cla
	borodachQW = 1
	borodachSexDay = day
	gs 'zz_render', '', 'images/city/south/south_office/security.jpg', '\\- Фиаско, -// ответили вы.**\\\- Точно, -/// почесал затылок охранник.\\\ - То-то я думаю ПИЗДЕЦ не может быть ответом!///'
	act 'Уйти': gt'south_office','start'
	act 'Засмеяться':
		*clr & cla
		gs 'zz_render', '', 'images/city/south/south_office/security.jpg', 'Вы засмеялись над охранником, он заулыбался и отложил кроссворд в сторону.**\\\- Я тебя тут уже не раз видел. Уборщицей работаешь?, -/// спросил он вас.**\\- Да, подрабатываю, -// кивнули вы в ответ.\\ - Тут такие деньги платят, что работой это не назовешь...//**\\\- Понятно, -/// сказал охранник.\\\ - Меня кстати Саша зовут, а тебя?///**\\- Меня Света, -// ответили вы.**\\\- Очень приятно, Света, -/// улыбнувшись сказал Саша.\\\ - У меня тут бутылка есть и закусь, составишь компанию?///'
	...
	*clr & cla
	...
	act 'Выпить':
		*clr & cla
		...
		energy += 2
		gs 'stat'
		gs 'zz_render', '', 'images/city/south/south_office/drink.jpg', 'Вы выпили водки, дыханье у вас спёрло, и вы схватив скорей '+iif(i = 1,'сырок','огурец')+', стали занюхивать. Сашка привычно хлопнул стакан и даже не поморщился:**\\\- Эх, хорошо пошла!///**Пока вы, наклонившись к столу, занюхивали и закусывали, вы почувствовали как рука охранника,'+iif(borodachSex = 0,' как бы случайно,','')+' легкими прикосновениями, гладит вас по попе.'
		if borodachSex = 0:
			act 'Эй! Ты чего задумал?':
				...
				horny += 5
				gs 'stat'
				gs 'zz_render', '', '', '\\- Эй! Ты чего задумал?, -// воскликнули вы.**\\\- Да ладно, я случайно... -/// начал бормотать Сашка, наливая ещё водки.\\\ - Давай лучше выпьем!///'
		...
		act iif(borodachSex = 0,'Сделать вид, что не заметили','Улыбаться'):
			...
			horny += 5
			gs 'stat'
			gs 'zz_render', '', '', iif(borodachSex = 0,'Вы закусили, стараясь не обращать внимания на, как бы, случайные прикосновения','Вы улыбнулись и Сашка налил еще водки, не отрывая руку ласкающую вас от вашей попы.')+'. Сашка налил еще водки**\\\- Между первой и второй, перерывчик не большой, -/// сказал он и опрокинул стакан.'
			if horny < 70: act 'Мне хватит': gt'south_office_events','security_no'
			act 'Выпить еще': gt'south_office_events','security_drink2'
		end
				...
				*clr & cla
				...
				act 'Дрочить ему':
					*clr & cla
					...
					gs 'stat'
					hj += 1
					gs 'zz_render', '', 'images/city/south/south_office/security_hj<<i>>.jpg', 'Вы начали ласкать его орган руками. Вскоре Сашка закряхтел:**\\\- Я так кончу скоро!///'
					act 'Дрочить ему дальше': gt'south_office_events','security_hjcum'
					act 'Взять в рот': gt'south_office_events','security_bjcum','bj'
					if stat_vgape < 2:act 'Секс': gt'south_office_events','security_sex'
				end
				act 'Взять в рот':
					*clr & cla
					i = rand(1,5)
					...
					gs 'zz_render', '', '','Сашка закряхтел:**\\\- Я сейчас кончу!///'
