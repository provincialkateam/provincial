if $ARGS[0] = 'start':
	*clr & cla
	gs 'stat'
	gs 'time'
	gs 'zz_render', 'Рынок', 'images/city/south/south_market/market_work/trade'+iif(month >= 4 and month <= 10,'','_winter')+'.jpg','Вы стоите за лотком с одеждой в ожидании покупателей.**В кассе <<rinmon>> рублей, продано <<rinvesh>> вещей.**Средняя цена на рынке <<prodpayum>>.'
	...
	if hour = 14:
		...
		act 'Отдать Рустаму деньги':
			*clr & cla
			...
			money += rinmon
			gs 'stat'
			gs 'zz_render', 'Рустам', 'images/city/south/south_market/market_work/rustam.jpg','Вы отдали Рустаму <<rinrust>> рублей за проданные <<rinvesh>> вещей.**Сегодня вы заработали <<rinmon>> рублей.'
			if rinrust < rustam_plan:
				bad_trade += 1
				gs 'zz_render', '', '','\\\- Эээ, Свэта, плохо работаешь, дэнег мало. Ты так разоришь мэня совсэм, -/// сказал он.'
			else
				bad_trade -= 1
				gs 'zz_render', '', '','\\\- Маладэц Свэта, хорошо поработала, -/// сказал он вам.'
			end
			rinmon = 0
			rinvesh = 0
			if bad_trade > 10:
				work = 0
				workrin = 0
				gs 'zz_render', '', '','\\\- Ээ, заебала ты уже савсэм, -/// сказал вам Руслан чуть помедлив.\\\ - Вот твоя трудовая. Проваливай, чтоб я тэбя больше нэ видел!///'
				act 'Уйти':gt'south'
				exit
			elseif bad_trade = 9:
				work = 0
				workrin = 0
				gs 'zz_render', '', '','\\\- Ээ, Свэта, -/// сказал вам Руслан чуть помедлив.\\\ - Если в слэдующий раз будэшь так плохо работать, выгоню!///'
				act 'Уйти':gt'south'
				exit
			end
			rinrand = RAND(1,60)
			if rinrand < vnesh:
				gs 'zz_render', '', '','\\\- Вай, Свэтик!, -/// продолжил Рустам.\\\ -'+iif(market_sex = 0,' Ми с тобой ещё даже нэ познакомились хорошенько','')+'. Пойдем в контейнэр, покушаем шашлыка, выпьем'+ iif(market_sex = 0,',познакомимся',',развлечемся')+'.///'
				act 'Отказаться':
					cla
					gs 'zz_render', '', '','Вы стали отнекиваться, но Рустам вам сказал:**\\\- Вай, зачэм оскорбляешь,! У тэбя вот лоток нэ убран, пыль, грязь. Я вэдь и штраф могу с тэбя взять!///'
					...
					act 'Нет':
						...
						if money >= 1000 and bad_trade >= 0:
							money -= 1000
							gs 'zz_render', '', '','Вы всё равно решили отказаться, тогда Рустам сказал:**\\\- Вай, у тэбя вещи грязные, отдавай тысячу рублей, это штраф, -/// вы отдали Рустаму деньги.'
						else
							bad_trade += 1
							gs 'zz_render', '', '','Вы всё равно решили отказаться, тогда Рустам сказал:**\\\- Ладно, иды уже.///'
						end
						act 'Уйти':gt'south'
					end
				end
		...
		if hour = 9:
			...
			if rintakrand = 0:
				...
				act 'Смотреть на Хасана и Надю':
					...
					minut += 15
					talkrand = RAND(0,2)
					if talkrand = 0:gs 'zz_render', '', '','Хасан выговаривает Наде:**\\\- Сэгодня продашь большэ, а то штраф дам. Мнэ деньги нужны!///'
			...
			elseif rintakrand = 1:
				...
				act 'Смотреть на Абдулу и Анну':
					...
					minut += 15
					talkrand = RAND(0,2)
					if talkrand = 0:gs 'zz_render', '', '','Абдула ругается:**\\\- Ты уже заебала! Нычего продать нэ можешь!///**Анна парирует:**\\\- Так ты мне трухлявое дерьмо на продажу даешь, его брать никто не хочет!///'
			...
			elseif rintakrand = 2:
				...
				act 'Смотреть на кавказцев':
					...
					if talkrand = 2:gs 'zz_render', '', '','Абдула смотрит на вас и говорит Хасану:**\\\- Гляды какая дэвочка, губки пэрсик, вах!///'
					act 'Далее': gt 'market_work','start'
				end
			elseif rintakrand = 3:
				...
				act 'Болтать с девчонками':
					...
					if rinslut = 0:
						if talkrand = 0:
							gs 'zz_render', '', '','Аня рассказывает, как заходила недавно в офисное здание и видела там объявление, что им требуется уборщица.**\\\- Думаю неплохой способ подзаработать, если деньги нужны, -/// добавляет она задумчиво.'
			...
			elseif rintakrand = 4:
				...
				act 'Смотреть на кавказцев':
					...
					if talkrand = 2:gs 'zz_render', '', '','Абдула глядит на вас и говорит Рустаму:**\\\- Гляды какая дэвочка, губки пэрсик, вах!///'
					act 'Далее': gt 'market_work','start'
				end
			end
			if rintakrand = 5:
				...
				act 'Смотреть на кавказцев':
					...
					if talkrand = 2:gs 'zz_render', '', '','Абдула глядит на вас и говорит другим кавказцам:**\\\- Гляды какая дэвочка, губки пэрсик, вах!///'
					act 'Далее': gt 'market_work','start'
				end
			end
			gs 'zz_render', '', '','<<$rintak>>'
		elseif hour = 10:
			...
			if rintakrand = 0:
				...
				act 'Смотреть на Хасана':
					cla
					minut += 15
					gs 'zz_render', '', '','Хасан говорит Наде:**\\\- Надэнька, зачэм ти отказываешься? Покушаем шашлыков, выпьем вина...///'
					act 'Далее': gt 'market_work','start'
				end
			elseif rintakrand = 1:
			...
			elseif rintakrand = 2:
				...
				act 'Болтать с Надей':
					...
					minut += 15
					manna -= 5
					gs 'zz_render', '', '','Надя недовольна:**\\\- Света, не отвлекай, у меня покупатели.///','Иди ты в жопу со своими разговорами!///'
					act 'Далее': gt 'market_work','start'
				end
			elseif rintakrand = 3:
			...
			elseif rintakrand = 4:
				...
				act 'Болтать с Аней':
					...
					minut += 15
					manna -= 5
					gs 'zz_render', '', '','Аня недовольна:**\\\- Света, не отвлекай, у меня покупатели.///','Иди ты в задницу со своими разговорами!///'
					act 'Далее': gt 'market_work','start'
				end
			elseif rintakrand = 5:
		...
		elseif hour = 11:
			...
			elseif rintakrand = 4:
				...
				act 'Болтать с Аней':
					...
					minut += 15
					manna -= 5
					gs 'zz_render', '', '','Аня недовольна:**\\\- Света, у меня покупатели.///','Иди ты в задницу со своими разговорами!///'
					act 'Далее': gt 'market_work','start'
				end
			elseif rintakrand = 5:
	...
	act 'Назвать цену':
		...
		if prodpay <= pokpay:
			...
			if poknorand >= 10:
				...
				gs 'zz_render', '', '','Покупатель вертит вещь в руках:**\\\- <<prodpay>> рублей - это слишком дорого...///'
