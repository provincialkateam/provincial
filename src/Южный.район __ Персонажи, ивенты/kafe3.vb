$cafe_suck_start = {
	...
	act 'Вынуть член изо рта':
		*clr & cla
		...
		gs 'zz_render','','common/sex/facial/<<rand(0,4)>>.jpg', '<<$boy>> кончает вам на лицо.'
		act 'Уйти': gt'south'
	end
	act 'Сосать дальше':
		*clr & cla
		gs 'zz_render', '','common/sex/bj_cum/<<rand(0,4)>>.gif', func('kafe3_strings', 'local_str2')
		act 'Проглотить':
			*clr & cla
			gs 'zz_funcs', 'cum', 'lip'
			swallow += 1
			gs 'zz_render', '', 'common/sex/bj_cum/<<rand(0,4)>>.gif', func('kafe3_strings', 'local_str3')
			act 'Уйти': gt'south'
		end
		act 'Выплюнуть':
			cla
			...
			if RAND(0,100) >= 90:
				...
				act 'Взять в рот':
					*clr & cla
					...
					guy += 1
					bj += 1
					gs 'zz_render', '', 'common/sex/bj_cum<<rand(0,1)>>.gif', func('kafe3_strings', 'local_str10')
					swallow += 1
					gs 'zz_funcs', 'cum', 'lip'
					gs 'stat'
	...
	*clr & cla
	...
	elseif deyst >= 70 and deyst < 85:
		...
		act 'Согласиться':
			...
			if lubri > 0:lubri -= 1
			gs 'zz_funcs', 'cum', 'ass'
			gs 'zz_render', '', 'common/sex/anal/<<rand(0,4)>>.gif', func('kafe3_strings', 'local_str18')
			act 'Уйти': gt 'south'
		end
	end
end
...
if $ARGS[0] = 'компашка':
	cla
	dynamic $venerasiak
	gs 'zz_render','','common/sex/dp/<<rand(0,4)>>', func('kafe3_strings', 'local_str21')
	guy += 3
	sex += 1
	bj += 1
	anal += 1
