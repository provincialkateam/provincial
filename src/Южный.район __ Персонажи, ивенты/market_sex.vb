...
if $ARGS[0] = 'refuse':
	*clr & cla
	...
	cumlip = 0
	gs 'stat'
	gs 'zz_render', '', 'images/city/south/south_market/market_work/refuse.jpg',iif(args[1] > 0,'Вы, с красным от стыда лицом, отрицательно поматали головой'+iif(args[1] = 1,'стёрли остатки спермы с губ',''),'Вы начали сопротивляться, отталкивая Рустама руками. Вырвавшись, наконец, из его объятий, вы поправили одежду и прическу')+', и молча отправились к выходу из подсобки.**\\\- Вай, Свэтик, нэ хорошо поступаешь! Я вэдь и обидыться могу.///'
	act 'Передумать':
		if args[1] = 1:
			gang += 1
	...
	act 'Уйти':minut += 15 & rustam_plan += 2000 & gt'south'
end
if $ARGS[0] = 'oral':
	*clr & cla
	...
	gs 'stat'
	gs 'zz_render', '', 'images/city/south/south_market/market_work/oral'+iif(args[1] = 0,rand(1,3),4)+'.jpg',iif(args[1] = 1,'Обернувшись, вы кивнули головой, в знак своего согласия. Абдула и Хасан тут же вынули свои члены, и подставили их к вашему рту. Вы принялись сосать сразу у обоих, меняя их члены у себя во рту.**Через некоторое время, Абдула и Хасан стали кончать, наполняя ваш рот своей спермой, а вы глотали и слизывали её до тех пор, пока их члены не стали опадать.**После того как парни кончили, к вам подошел довольный Рустам, и протянул 1000 рублей премиальных','Вы сели перед хозяином на колени и достали его член')+'.'
	if args[1] = 0:
		gs 'zz_dynamic_sex', 'bj'
		if market_sex < 20:
			act 'Сосать дальше':gt'market_sex','swallow'
			if stat_vgape < 2:act 'Отдаться':gt'market_sex','vaginal'
			if stat_agape < 2:act 'Подставить попу':gt'market_sex','anal'
		else
			temp = rand(1,3)
		...
		act 'Идти домой':minut += 15 & gt'south'
	end
end
if $ARGS[0] = 'swallow':
	*clr & cla
	...
	if temp < 95 and palevorin = 0 or temp < 70 and palevorin >= 1:
		...
		gs 'zz_render', '', '','Вы уже закончили делать минет и слизывали остатки спермы с члена Рустама, когда дверь открылась и в каморку вошло двое. В них узнали Абдулу и Хасана, владельцев соседних палаток.**'+iif(palevorin = 0,'Вы замерли от удивления и стыда, что вас застукали с полным ртом спермы облизывающей красную елду. ','')+'Мужики обрадовались и загоготали:**\\\- Вах, Свэтик, ми тоже хотим, нам отсосёшь?///**\\\- Свэтик, помоги моим братьям'+iif(palevorin = 0,', а то без женщины совсэм дыкие стали','')+', -/// сказал расслабленый Рустам, роясь у себя в бумажнике.\\\ - А за это двойную прэмию от мэня получишь!///'
...
if $ARGS[0] = 'vaginal':
	*clr & cla
	...
	if args[1] > 0: bj += 1
	gs 'stat'
	gs 'zz_render', '', 'images/city/south/south_market/market_work/vaginal'+iif(args[1] = 0,rand(1,3),rand(4,6))+'.jpg','<<$boy>> раздел вас и '+iif(args[1] = 0,'оперев руками о стену','поставив раком')+', начал пристраиваться к вашей  киске.'
...
if $ARGS[0] = 'anal':
	*clr & cla
	...
	if args[1] > 0: bj += 1
	gs 'stat'
	gs 'zz_render', '', 'images/city/south/south_market/market_work/anal'+iif(args[1] = 0,rand(1,3),rand(4,6))+'.jpg','<<$boy>> раздел вас и поставив раком, и начал пристраиваться к вашей  попке.'
	...
	*clr & cla
	...
	if args[1] = 0:
		if temp < 95 and palevorin = 0 or temp < 70 and palevorin >= 1:
			...
			gs 'zz_render', '', '','В это время дверь со скрипом открылась и в каморку вошло двое. Вы узнали Абдулу и Хасана, владельцев соседних палаток.**Вы замерли от удивления и стыда, что вас застукали голой, да еще и с членом в заднице. Мужики обрадовались и загоготали:**\\\- Вах, Светик, ми тоже хотим, нам дашь?///**- Светик, помоги моим братьям, а то без женщины совсэм дыкие стали, -/// сказал расслабленый Рустам, роясь у себя в бумажнике.\\\ - А за это двойную прэмию от мэня получишь!///'
			if palevorin = 0:palevorin = 1
				...
				*clr & cla
				...
				act 'Далее':
					*clr & cla
					...
					gs 'zz_render', '', 'images/city/south/south_market/market_work/group_swallow'+rand(1,3)+'.jpg','Наконец парни выдохлись, и посадив вас на пол, начали кончать, заливая ваше лицо своей спермой, и попадая на тело.**После Абдула и Хасан оделись у ушли, а Рустам выписал вам <<market_money>> рублей премиальных и завалился отдыхать.'
					act 'Идти домой':market_money = 0 & minut += 15 & gt'south'
				end
			end
		end
	end
