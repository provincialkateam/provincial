	...
	if school['certificate'] = 2:
		...
		act 'Узнать результат':
			*clr & cla
			...
			university['course'] = 1
			university['semester'] = 1
			if university['student'] = 2:university['scholarship'] = 5000
			gs'stat'
...
if $ARGS[0] = 'lecture':
	*clr & cla
	...
	if alko < 3:
		act 'Слушать лекцию':
			*clr & cla
			if rand(1,8) >= son: gt'uni_events','sleep',1
	...
	if alko < 6:
		act 'Болтать':
			*clr & cla
			if rand(1,iif(alko < 3,8,iif(alko < 6,12,16))) >= son: gt'uni_events','sleep',2
	...
	if son < 18 or alko >= 6: act 'Спать': gt'uni_events','sleep'
end
!сон на лекции
if $ARGS[0] = 'sleep':
	*clr & cla
...
if $args[0] = 'course_work':
	*clr & cla
	...
	gs 'zz_render', '', '', 'Вы, в течении часа, '+iif(university['work_memory'] = university['course_work'],'пытались писать курсовую работу, но так ничего нового и не написали.'+iif(alko = 0,' Вам просто не хватило знаний и интеллекта для этого.',iif(alko < 3,' Вам просто нехватило знаний и интеллекта, или лёгкое опьянение повлияло на это. А может и всё вместе взятое.','так как из-за опьянения ничего не соображали.')) ,'писали курсовую работу'+iif(university['course_work'] >= 100,' и наконец её закончили',' и продвинулись вперёд.'))+iif(university['course_work'] < 100,'**Курсовая работа готова <<university[''course_work'']>> процентов.','')
	act 'Закончить':gt $loc, $metka
end
!сдача курсовой
...
if $args[0] = 'graduate_work':
	*clr & cla
	...
	gs 'zz_render', '', '', 'Вы, в течении часа, '+iif(university['work_memory'] = university['graduate_work'],'пытались писать дипломную работу, но так ничего нового и не написали.'+iif(alko = 0,' Вам просто не хватило знаний и интеллекта для этого.',iif(alko < 3,' Вам просто нехватило знаний и интеллекта, или лёгкое опьянение повлияло на это. А может и всё вместе взятое.','так как из-за опьянения ничего не соображали.')) ,'писали дипломную работу'+iif(university['graduate_work'] >= 100,' и наконец её закончили',' и продвинулись вперёд.'))+iif(university['course_work'] < 100,'**Дипломная работа готова <<university[''graduate_work'']>> процентов.','')
	act 'Закончить': gt $loc, $metka
end
!защита диплома
...
if $args[0] = 'university_status':
	...
	if university['student'] > 0:
		...
		if (university['vacation'] = 0 or university['vacation'] = 4 and month > 6) and university['student'] = 1 and university['payment'] ! university['semester']:
			...
			gs 'zz_render', '', '', '<red><b>Вас отчислили из университета, так как вы не оплатили учёбу.</b></red>'
		end
		if month = 2 and university['semester'] = 1 or month = 7  and university['semester'] = 2:
			...
			university['graduate_work'] = 0
			university['semester'] = 0
			gs 'zz_render', '', '', '<red><b>Вы завалили сессию и вас отчислили из университета.</b></red>'
		end
	end
end
	...
	*clr & cla
	...
	act 'Ласкать клитор':
		*clr & cla
		...
		if horny >= 60:
			act'Передохнуть': gt'uni_events','selfplay'
		else
			act 'Закончить': gt'uni_events','toilet_cubicle'
		end
	end
	if vagina > 0:
			...
			act iif(dildohand = 0,'Взять в сумочке дилдо','Выбрать другой дилдо'):
				*clr & cla
				...
				if dildo = 1 and dildohand ! 10:
					...
					act 'Взять 10ти сантиметровый дилдо':dildohand = 10 & gt'uni_events','selfplay'
				end
				if middildo = 1 and dildohand ! 15:
					gs 'zz_render', '', '','У вас есть дилдо длинной 15 см.'
					act 'Взять 15ти сантиметровый дилдо':dildohand = 15 & gt'uni_events','selfplay'
				end
				if largedildo = 1 and dildohand ! 20:
					gs 'zz_render', '', '','У вас есть дилдо длинной 20 см.'
					act 'Взять 20ти сантиметровый дилдо':dildohand = 20 & gt'uni_events','selfplay'
				end
				if bigdildo = 1 and dildohand ! 25:
					gs 'zz_render', '', '','У вас есть дилдо длинной 25 см.'
					act 'Взять 25ти сантиметровый дилдо':dildohand = 25 & gt'uni_events','selfplay'
				end
				if extradildo = 1 and dildohand ! 30:
					gs 'zz_render', '', '','У вас есть дилдо длинной 30 см.'
					act 'Взять 30ти сантиметровый дилдо':dildohand = 30 & gt'uni_events','selfplay'
				end
				if superdildo = 1 and dildohand ! 35:
					gs 'zz_render', '', '','У вас есть дилдо длинной 35 см.'
					act 'Взять 35ти сантиметровый дилдо':dildohand = 35 & gt'uni_events','selfplay'
				end
				if maddildo = 1 and dildohand ! 40:
					gs 'zz_render', '', '','У вас есть дилдо длинной 40 см.'
					act 'Взять 40ти сантиметровый дилдо':dildohand = 40 & gt'uni_events','selfplay'
				end
				act'Передумать': gt'uni_events','selfplay'
			end
		end
	end
	act 'Отказаться от этой затеи': gt'uni_events','toilet_cubicle'
end