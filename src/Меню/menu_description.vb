...
if $args[0] = 'tab0':
	...
	!-----
	! текст описания
	$_hero_text = 'Вас зовут <b>Света Лебедева</b>.'
	$_hero_text += '**'+'Вам <<age>> <<$letgoda>>, на вид вам можно дать не больше ' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''age'',1"><<vidage>></a>',vidage) + ' <<$letgoda2>>.'
	!if birthday > 0 and birthmonth > 0:$_str += '**'+'Дата вашего рождения <<birthday>>.0<<birthmonth>>.<<birthyear>>г.'
	!$_hero_text += '**Рост '+iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''body_height'',1">'+body['height']+'</a>',body['height'])+' см, вес '+iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''body_weight'',1">'+body['weight']+'</a>',body['weight'])+' кг.'
	$_hero_text += '**' + $body
	!$_hero_text += '**' + body['breast']+' - '+body['waist']+' - '+body['hips']+', размер груди '+body['tits']+'.'
	$_hero_text += '**<<$vnesh>>'
	$_hero_text += '**' + $hair
	$_hero_text += '**' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''lip'',1"><<$lip>></a>',$lip)
	$_hero_text += '**' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''throat'',1"><<$throat>></a>',$throat)
	$_hero_text += '**' + $skin
	$_hero_text += '**' + $glaza
	$_hero_text += '**' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''mop'',1"><<$mop>></a>',$mop)
	$_hero_text += '**' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''leghair'',1"><<$leghair>></a>',$leghair)
	$_hero_text += '**' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''lobok'',1"><<$lobok>></a>',$lobok)
	$_hero_text += '**' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''vagina'',1"><<$vagina>></a>',$vagina)
	$_hero_text += '**' + iif($settings['cheats']=1,'<a class="cheat" href="exec:gs''zz_cheats'',''anus'',1"><<$anus>></a>',$anus)
	if curly > 0: $_hero_text += '**Завивки хватит еще на <<curly>> дней.'
	!-----
...
if $args[0] = 'tab1':
	...
	$rc_param += func('zz_funcs', 'scale', maidstat, 100, 0, iif($settings['cheats']= 1,'<a class="cheat" href="exec:gs''zz_cheats'',''maidstat'',2">Навыки горничной</a>','Навыки горничной'))
	...
	!---
	! здесь надо правильно указать переменную, отвечающую за прием в банду
	if $npc['agent_zeropussy'] = 1:
		$rc_param += func('zz_funcs', 'scale', criminal, 100,0,'Криминал')
		$rc_param += func('zz_funcs', 'scale', accuracy, 100,0,'Меткость')
	end
	!---
...
if $args[0] = 'tab2':
	...
	if rinslut > 1 and kavslut > 0:$_str += '**В Южном районе Сити вас знают как шлюху для кавказцев'
	if motherKnowSpravka > 0 and motherKnowWhore > 0:
		$_str += '**Ваша мать знает, что вы уже не девственица и считает вас шлюхой'
	elseif motherKnowWhore > 0:
		$_str += '**Ваша мать считает вас шлюхой'
	elseif motherKnowSpravka > 0:
		$_str += '**Ваша мать знает, что вы уже не девственица'
	else
		$_str += '**Ваша мать думает, что вы девственница'
	end
	if $npc['39,qwPornoFootball'] >= 30 and $npc['39,sex'] = 1:
		$_str += '**Ваш брат считает вас нимфоманкой.'
	elseif $npc['39,sex'] = 1 or ($npc['39,qwPornoFootball'] >= 16 and $npc['39,qwPornoFootball'] < 30):
		$_str += '**Ваш брат считает, что вы - удобная телка.'
	elseif brotherknowslut = 1:
		$_str += '**Ваш брат видел сперму на вашем теле и думает, что вы даете'
	elseif brotherknowslut = 2:
		$_str += '**Ваш брат видел как вы отсасывали гопникам'
	else
		$_str += '**Ваш брат о вас хорошего мнения'
	end
	if sisterknowslut > 1 and sisboypartyQW = 2:
		$_str += '**Ваша сестра считает вас отвязной шлюшкой'
	elseif sisterknowslut > 0:
		$_str += '**Ваша сестра знает, что вы даете'
	else
		$_str += '**Ваша сестра о вас хорошего мнения'
	end
	$_str += '</div>'
	gs 'zz_render','','',$_str
	killvar '$_str'
