...
if $args[0] = 'tamara':
	*clr & cla
	...
	if $curloc = 'mey_home' and $npc['45,qwTamara'] >= 3 and $npc['45,qwTamara'] < 50:
		if week >= 6 or school['vacation'] > 0: act 'Попросить отвезти в город': minut += 30 & gt 'center'
...
if $args[0] = 'dialog_start':
	*clr & cla
	gs 'npc_editor','get_npc_profile',45
	gs 'npc_editor','check_dialogs',45,'mey_tamara_events','dialogs'
	act '<b>Отойти</b>': gt 'mey_home',4
end
if $args[0] = 'dialogs':
	*clr & cla
	minut += rand(10,20)
	manna += rand(1,5)
	gs 'npc_editor','change_rep','+',45,1
	gs 'stat'
	gs 'zz_render','','pavlovo/meynolds/tamara/dialog.jpg','Тётьтамаре всегда есть, о чём рассказать - она просто неисчерпаемый кладезь интересных историй и полезных знаний: много поездила, много повидала и очень интересная рассказчица.'
	act '<b>Отойти</b>': gt 'mey_home',4
end
! первая встреча
	...
	*clr & cla
	...
	if zz_stage < 1:
		...
		if zz_stage < 8:
			...
			act 'Сесть в такси':
				*clr & cla
				gs 'zz_render','','common/transport/taxi/inner_night', 'Несмотря на близкое расстояние, вам всё-таки не пришлось переться по тёмным улочкам в поздний час. Таксист подвёз к подъезду и вы, поблагодарив, забежали к себе в подъезд.'
				act 'домой' : gt 'korrPar'
			end
		end
	end
end
	...
	*clr & cla
	...
	act '...':
		*clr & cla
		...
		gs 'zz_clothing2','add_rand_dress',4
		gs 'zz_render','','','<green>Повседневное платье</green>'
		act '...':
			*clr & cla
				...
				*clr & cla
				...
				act 'Идти дальше':
					*clr & cla
					minut += 20
					gs 'stat'
					gs 'zz_render', '', $_img + 21, func('mey_tamara_strings', 'learning24')
					act 'Я успею': $npc['45,qwTamara'] = 6 & gt 'center'
...
if $args[0] = 'check_look':
	*clr & cla
	...
	if func('zz_clothing2','is_glamour') = 1 and hapri = 1 and (mop = 2 or mop = 3) and sweat <= 0 and vnesh >= 60:
...
if $args[0] = 'party3':
	*clr & cla
	gs 'zz_render', '', $_img + 15, func('mey_tamara_strings', 'party15')+'**'+func('mey_tamara_strings', 'party31')
	$npc['45,qwTamara'] = 50
	hour = 23
	minut = 55
	manna -= 20
