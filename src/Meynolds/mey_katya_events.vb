...
! $npc['14,qwKatya_disco'] - добавляем проверку - ушла ли Катя с дискотеки с парнем
!---
! ветка Кати, стадии
! 0 - ветка не начата
...
if $args[0] = 'gossip':
	...
	$_str[1] = 'А ты слышала, что наш Царёк хорошо дерется? Пацаны хотели ему тёмную устроить, так он их как котят..'
	...
	$_str[3] = 'В школе тут судачат, что скоро форму школьную отменят и вообще можно будет ходить как хочешь. Лерка Царева, небось, в одних трусах припрётся!'
	...
	$_str[8] = 'По секрету только тебе: я тут услышала, что у нас в классе не осталось ни одной девушки, прикинь! Интересно, а как...'
...
if $args[0] = 'sleep_together':
	*clr & cla
	...
	$metka = '5'
	! --
	gs 'zz_render','','pavlovo/meynolds/katya/drunk_qw/drunk_sleep1',func('mey_katya_strings',14)
...
if $args[0] = 'homework':
	*clr & cla
	...
	!закончить учеба/ключа нет
	elseif $npc['0,meynold_key'] = 0:
		minut += rand(5,10)
		act 'Закончить с уроками' : gt 'mey_home',5
		act 'Идти домой' : minut += rand(5,10) & gt 'gorodok'
	!закончить учеба/ключ есть
	else
		act 'Идти домой' : minut += rand(5,10) & gt 'gorodok'
		act 'Выйти из комнаты' : gt 'mey_home',5
	end
end
...
if $args[0] = 'hotel_observing':
	*clr & cla
	_stage = args[1]
	gs 'zz_render','','pavlovo/meynolds/katya/hotel_sex/'+iif(_stage<9,_stage,'')+'.webm',func('mey_katya_strings',76+_stage)
...
if $args[0] = 'park_sex_prepeare':
	*clr & cla
	minut += rand(15,30)
	gs 'stat'
	gs 'zz_render','','pavlovo/meynolds/katya/park_walk/2',func('mey_katya_strings',91)
...
if $args[0] = 'park_sex':
	*clr & cla
	...
	gs 'stat'
	gs 'zz_render','','pavlovo/meynolds/katya/park_sex/' + iif(_stage < 7,_stage,'') + '.webm',func('mey_katya_strings',92+_stage)
...
if $args[0] = 'random_hotel_sex':
	*clr & cla
	...
	$npc['14,qwKatya_day'] = day
	$npc['14,qwKatya_disco'] = 1
	gs 'npc_editor','get_npc_profile',14
	gs 'zz_render','','',func('mey_katya_strings',100)
	if $npc['0,hotelwork'] = 1 or $npc['0,hotelwork'] = 3:
		act 'Проследить':
			*clr & cla
			minut += rand(40,60)
...
if $args[0] = 'hub_talk':
	*clr & cla
	minut += rand(1,3)
	gs 'npc_editor','get_npc_profile',14
	gs 'zz_render','','',func('mey_katya_strings',102)
	act 'Согласиться':
		*clr & cla
		$npc['14,qwKatya'] = 100
		gs 'npc_editor','get_npc_profile',14
		gs 'zz_render','','',func('mey_katya_strings',103)
	...
	act 'Настаивать':
		*clr & cla
		...
		gs 'npc_editor','get_npc_profile',14
		gs 'zz_render','','',func('mey_katya_strings',104)
		act 'Отойти': gt 'zz_school'
	end
end
...
if $args[0] = 'after_talk':
	*clr & cla
	...
	minut += rand(1,3)
	gs 'npc_editor','get_npc_profile',14
	gs 'zz_render','','',func('mey_katya_strings',105)
	act 'Отойти': gt 'zz_school'
end
! разговор о трио
if $args[0] = 'ask_about_trio':
	*clr & cla
	...
	gs 'npc_editor','get_npc_profile',14
	gs 'zz_render','','',func('mey_katya_strings',106)+func('mey_katya_strings',iif(month > 3 and month < 11,107,108))
	!act 'Отойти': gt 'gdkin'
	act 'Отойти': gt $loc
end
! трио в парке
