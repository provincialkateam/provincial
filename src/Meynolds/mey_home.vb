...
if $args[0] = 'main':
	...
	$_str[3] = 'После недолгого ожидания щёлкает замок, и дверь открывает мама Кати и Вики: **\\\- О, Светик, привет! Проходи, чувствуй себя как дома.///'
	...
	if $npc['0,meynold_key'] = 1:
		gs 'zz_render','','pavlovo/meynolds/home/mey_home',$_str[1]+$_str[4]
...
$mey_desc['1,text'] = 'Кухня огромна и роскошна. Отделана она в минималистическом хайтек-стиле. Вокруг стоящего посередь её обеденного стола можно, как и холле, ездить на велосипеде. Бытовая техника вся скрыта, встроенная. Кухня сияет чистотой.'
...
$mey_desc['5,text'] = 'В комнате Кати складывается ощущение, что она ненадолго сюда въехала и скоро съедет. Идеальный порядок, зеркальная чистота и ощущение "нежилого помещения". В углу стоит стол с компьютером, напротив - кровать. В другом углу просторной, но почему-то кажущейся тесной комнаты, спрятался стенной шкаф-гардероб. Напротив стоит книжный шкаф, забитый всякими умными книжками.'
...
$mey_desc['7,text'] = 'Комната Тамары Михайловны напоминала бы офисный рабочий кабинет, если бы не кровать в углу. Тут стоит стол с компьютером, кожаное офисное кресло у него, платяной шкаф и довольно скромная кровать. Комнатка невелика и тесновата. "Украшательств" в ней нет, кроме висящего на стенке зачем-то меча. Что он тут делает - совершенно непонятно.'
...
if mey_loc_id = 0:
	...
	if hour = 17 and $npc['45,qwTamara'] = 16:
		gs 'zz_render','','','**В холле вас ожидает <a href="exec: gt''mey_tamara_events'',''check_look''">Тамара Михайловна.</a>'
		exit
	end
	! надо трахнуть катю!
...
if mey_loc_id = 2:
	...
	elseif hour = 7 and minut < 30:
		*clr
		gs 'zz_render','','pavlovo/meynolds/vika/bath/11','Ванная занята, судя по времени - это стопудово засоня <a href="exec:gt''mey_vika_talk'',''main''">Вика</a>, а эта может долго лёпаться.'
	else
		$metka = 0
		gt 'zz_bathroom','init','mey_home','0'
	end
!	exit
end
! guestroom
if mey_loc_id = 3:
	...
	act '<b>Выйти</b>':
		if cloth[0] < 3:
			*clr
			gs 'zz_render','','images/clothing/'+iif(cloth[1]=0,'0','tanga')+'.jpg','<red>В таком виде в гостях лучше не ходить!</red>'
			exit
		end
		gt 'mey_home',10
	end
...
if mey_loc_id = 4:
	...
	if hour >= 18 and hour < 22:
		...
		gs 'zz_render','','','В зале на диванчике сидит <a href="exec:gt''mey_tamara_events'',''dialog_start''">Тамара Михайловна</a>.'
	end
...
if mey_loc_id = 9:
	cla
	act '<b>Уйти</b>':
		if cloth[0] <= 2: gs 'mey_home_events','cloth_on'
		gt $curloc, 0
	end
