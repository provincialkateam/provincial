...
if $ARGS[0] = 'kuh_buh':
	*clr & cla
	...
	serge_textbuhk = RAND(0,4)
			...
			if serge_textbuhk = 3:$serge_textbuhk = 'Сергей рассказывает вам анекдот:**Идут два алкаша:**- Лёха!.. Мы где?.. В джунглях?!..**- Сёма, твою мать!.. Убери укроп с очков...'
			if serge_textbuhk = 4:$serge_textbuhk = 'Света, а знаешь почему наш народ пить не умеет?**Это всё потому что у нас всего две стадии:**1.Чё-то вообще не взяло!**2.Блииин, я в гoвнoооо...'
	...
	if sergebazarkuh_count <= 0:'- "Света, я не хочу сейчас разговаривать, не мешай мне. - гонит вас Сергей.'
		...
		*clr & cla
		...
		act 'Согласиться':
			*clr & cla
			...
			gs 'stat'
			gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/kuh/event/alko2.jpg'
			if beer_count > 0:
...
*clr & cla
gs 'stat'
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/serge1.jpg'
if sergehelppos_count > 0 or sergehelppol_count > 0 : 'Света, если не сложно - помой посуду или пол протри.'
if sergehelppos_count <= 0 and sergehelppol_count <= 0 : 'Света, ты уже все сделала, больше на сегодня дел нет.'
...
$serge_kuh_izn[1] = {
	...
	*clr & cla
	...
	horny += 25
	sex += 1
	gs 'zz_funcs','set_gape','vagina',horny, 21, 2
	gs 'stat'
	...
	*clr & cla
	...
	horny += 20
	anal +=1
	gs 'zz_funcs','set_gape','anus', iif(lubri>0,10,0),21,2
	gs 'stat'
...
$serge_kuh_sex[3] = {
	...
	*clr & cla
	...
	gs 'zz_funcs','set_gape','anus', iif(lubri>0,10,0),21,2
	gs 'stat'
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/kuh/help/event/sex3-5.jpg'
		if analplugin = 1:
			analplugin = 0
