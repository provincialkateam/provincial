if $args[0] = '' or $args[0] = 'home':
	...
	cumlip = 0
	arturday = day
	if $npc['40,sex'] = 0 : guy+=1
	! счетчик количества трахов с Артуром
	$npc['40,sex'] += 1
	! локальный счетчик количества актов
	artsextimes = 0
	! локальные счетчики секса
	art_temp_vaginal=0
	art_temp_anal=0
	art_temp_oral=0
	gs 'stat'
	$_str = 'Артур отвёл вас на стоянку, где стояла его чёрная заниженная Приора. Вы сели в машину, и он куда-то повёз вас.**\\\- Дэвачка, я дэлаю настоящий, правильний шашлык, как у нас на Кавказе в горах готовят для сэбя, а не как для турыстов, и ни то щто дрянь из кошек здэсь!///**Через некоторое время машина остановилась возле небольшого, но хорошо отделанного домика, и горец, взяв вас за руку, проводил в дом.'
	$_str += 'Артур щедро накрыл на стол. Вы ели самый роскошный шашлык в своей жизни - горец не обманул. Готовил он его сам, добавляя травки и щедро поливая мясо вином из простой баклажки для аромата.**\\\- Это вино - ни тэ помои, что видают за наще! Это - виращено моим дэдом в горах!///**На столе появилась и обещанная чача. Вы быстро захмелели. Артур включил музыку и подошёл к вам:**\\\- Иди ка мнэ, дэвочка!///**'
	if $npc['40,sex'] = 0: $_str += 'Вы попросили его надеть презерватив, на что он возмутился:**\\\- Вай, это как цилаваться в пративагазе! Нэт!///**'
...
if $args[0] = 'sex_init':
	...
	if artsextimes > 0:
		*clr
		gs 'zz_render','','images/qwest/alter/artur/bedmiss.jpg'
...
if $args[0] = 'suck':
	*clr & cla
	...
	art_temp_oral += 1
	gs 'stat'
	gs 'zz_render','','common/sex/bj/<<rand(0,4)>>.gif','Вы сели на колени перед Артуром и стали дрочить его длинный и толстый член. Артур в нетерпении гладит вашу голову своей жилистой рукой.'
	act 'Сосать':
		*clr & cla
		minut += 5
		gs 'stat'
		gs 'zz_render','','common/sex/bj/<<rand(0,4)>>.gif','Вы открыли рот и обхватили своими губами здоровенный член и начали сосать, но Артур вскоре потерял терпение и схватил вас за волосы.'
...
if $args[0] = 'facefuck':
	*clr & cla
	...
	gs 'stat'
	gs 'zz_render','','common/sex/facefuck/<<rand(0,4)>>.gif','Артур схватил вас за волосы и просто насадил ваш рот на свой здоровенный член.'
	if throat < dick:
...
if $args[0] = 'ender':
	*clr & cla
	...
	pose = 0
	cumface = 0
	if art_temp_vaginal>0: killvar 'art_temp_vaginal' & sex += 1
	if art_temp_anal>0: killvar 'art_temp_anal' & anal += 1
	if art_temp_oral>0: killvar 'art_temp_oral' & bj += 1
	if RAND(0,5) = 0:
		kosmetica += 20
		$_str = 'Артур подарил вам набор косметики.'
	else
		money += 1000
		$_str = 'Артур сказал:**\\\- Эээ, у мэня времени нэ было тэбе подарок искать, на тысячу, сама купи, что тэбе нравится.///'
	end
	gs 'zz_render','','images/qwest/alter/artur/artur.jpg','Артур встал с вас и тут же, как ни в чем не бывало, занялся делами.**Вы, как могли, привели себя в порядок и взглянули на него, выразительно хлопая глазами, от чего тот подскочил, как будто что-то вспомнил, метнулся резвым архаром в соседнюю комнату, и, спустя миг, вернулся, пряча что-то за спиной. ' + $_str
	killvar '$_str'
	gs 'stat'
	act 'Уйти': gt 'gorodok'
end
...
if $args[0] = 'suck_behind_kiosk':
	dick = 26
	arturday = day
	gs 'zz_render','','images/qwest/alter/artur/artur.jpg','Вместе с Артуром вы зашли в контейнер который используется как склад. Он тут же расстегнул ширинку: **\\\- Давай Свэтик, поработай ротиком.///'
	act 'Опуститься на колени':
		*clr & cla
		...
		bj += 1
		gs 'stat'
		gs 'zz_render','','common/sex/bj/<<rand(0,4)>>.gif','Вы сели на колени перед Артуром и стали дрочить его длинный и толстый член.'
		...
		act 'Сосать':
			*clr & cla
			...
			gs 'zz_render','','common/sex/bj_cum/<<rand(0,4)>>.gif','После этого Артур ухмыльнулся, заправил член в штаны и сказал:**\\\- Вай Свэта, ты настоящая соска. Вот тебе от дяди Артурчика, купи сэбэ конфэток.///**Он протянул вам 300 рублей.'
