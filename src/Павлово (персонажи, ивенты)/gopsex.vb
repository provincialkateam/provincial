...
$ind_stamina_npc = {
	...
	gs 'zz_render','','','<center>Вас трахает <<$gopsex_name_npc>>, его выносливость: <<$ind_npc>></center>'
}
!групповой индикатор стамины
$ind_stamina_groupnpc = {
	$ind_npc = func('zz_funcs','scale',sh_ind,5)
	sh_ind = shgopsex_group_count
	gs 'zz_render','','','<center>Вас <<$gop_sextext3>> <<$gop_text_npcr>>, их выносливость: <<$ind_npc>></center>'
}
! ---
!Рандомизация текста!
...
$gg_devst_deflor = {
	if vagina <= 0:
		...
		manna -= 15
		vagina += 1
		gs 'zz_render','','','Вы предупреждаете что это ваш первый раз и просите быть поаккуратнее.'
		gs 'zz_render','','','<font color = red><b>Вы закусили губу, когда член вошёл в вас, разорвав девственную плеву. Это довольно болезненно в первое время.</b></font>'
	end
}
...
$gg_sex_orgasm = {
	if mesec > 0:
		...
		horny = 0
		manna -= 10
		gs 'zz_render','','','<font color = red><b>У вас месячные и в то время пока член таранит вас, из влагалища потихоньку вытекает кровь.</b></font>'
...
$masturb_gg = {
	...
	if mesec > 0:
		...
		horny = 0
		manna -= 10
		gs 'zz_render','','','<font color = red><b>Вас эта картина немного возбудила, но так как у вас месячные подрочить не получится...</b></font>'
	elseif mesec <= 0:
		!нет месячных, оргазм возможен
		if horny >=50:'**<<$rnd_masturb_text>>'
		...
		if horny >=70:
			...
			horny = 0
			mastr +=1
			gs 'zz_render','','','От обилия нахлынувших чувств вы с громким стоном кончили.'
		end
	end
}
...
$gop_sex_analplugin = {
	if analplugin = 1:
		analplugin = 0
		gs 'zz_render','','','- "Это что пробка у тебя в жопе?", удивленно спрашивает <<$gopsex_name_npc>>.'
		gs 'zz_render','','','- Ага, очко разрабатываю!'
		gs 'zz_render','','','- "Похвально, но она сейчас нам мешать будет", сказал он и вынул из вашей попки пробку.**'
	end
}
$gop_sex_lubri = {
	if gop_sex_lubri = 0:
...
$gop_groupsex_analplugin = {
	if analplugin = 1:
		analplugin = 0
		gs 'zz_render','','','- "Это что пробка у тебя в жопе?", удивленно спрашивает <<$gop_name_npc>>.'
		gs 'zz_render','','','- Ага, для вас очко разрабатываю!'
		gs 'zz_render','','','- "Похвально, но она сейчас нам мешать будет", сказал он и вынул из вашей попки пробку.**'
	end
}
$gop_groupsex_lubri = {
...
$accview_ggsex = {
	!дела домашние
	$_gopstr[0] = 'обед приготовить'
	$_gopstr[1] = 'уроки сделать'
	$_gopstr[2] = 'свою комнату убрать'
	$_gopstr[3] = 'посуду помыть'
	$_gopstr[4] = 'в магазин за хлебом сходить'
	$rnd_domdela_text = $_gopstr[rand(0,4)]
	!репа: давалка
	$_gopstr[0] = 'Не пизда а ведро'
	$_gopstr[1] = 'Совсем девку заебали'
	$_gopstr[2] = 'Бездонной дыркой стала наша <<$gop_name_gg>>'
	$_gopstr[3] = 'Опять <<$gop_name_gg>> ты тут отдаешся всем подряд...'
	$_gopstr[4] = 'Опять <<$gop_name_gg>> тебя <<gop_sextext3>>, сколько можно то?, не устала всем подряд дырки подставлять...'
	$rnd_telogg_text = $_gopstr[rand(0,4)]
	!репа: шлюха
	$_gopstr[0] = '<<$gop_name_gg>> и не стыдно тебе в таком грязном месте подрабатывать?'
	$_gopstr[1] = '<<$gop_name_gg>> уже и тут подрабатываешь?'
	$_gopstr[2] = '<<$gop_name_gg>> на каждом углу пиздой торгуешь?'
	$_gopstr[3] = 'Клиентов себе нашла, с полной отдачей работаешь <<$gop_name_gg>>'
	$_gopstr[4] = 'Быстро же ты <<$gop_name_gg>> клиентов себе находишь, я поражаюсь!'
	$rnd_telogg2_text = $_gopstr[rand(0,4)]
	!разный текст
	...
	if rnd_caught_mom = 4:$rnd_caught_mom = '<<$rnd_rodnya_text>> ваша <<$rnd_namemom_text>>, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text>>: — <<$gop_name_gg>>, вместо того, чтобы со всем городом ебаться, лучше бы шла уроки делать.** — "Да сделала я!"'
	...
	if rnd_caught_mom = 6:$rnd_caught_mom = '<<$rnd_rodnya_text>> ваша <<$rnd_namemom_text>>, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text>>: —Дочка, как же тебя разъебли… Ты скоро домой? ** <<$rnd_mom_txt1>>'
	!------
	!Сестра:
	...
	if rnd_caught_sister = 3:$rnd_caught_sister = '<<$rnd_rodnya_text>> ваша <<$rnd_namesis_text>>, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text>>: — Я, блин, бегаю, ищу её везде... А её, оказывается, тут ебут! Колокольчик что ли на шею повесь, чтобы в следующий раз легче найти тебя было. **Ключи давай, дырка подзаборная, а то я свои дома забыла... Ладно, не дёргайся, сама возьму. В сумочке? Да вынь ты хуй изо рта, когда тебя старшие спрашивают! В кармане? Всё, нашла уже... Пойдёшь в ванную - вычисти после себя всё с порошком. Не хватало мне ещё после тебя от Святого Духа аборты делать.'
	if rnd_caught_sister = 4 and func('zz_reputation','get') < 4:$rnd_caught_sister = '<<$rnd_rodnya_text>> ваша <<$rnd_namesis_text>>, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text>>: — <<$gop_name_gg>>, мама тебя обыскалась, а она тут ебётся! Ты долго ещё?**— Ох, сестрёнка, сладко очень… Ещё минут 10 поебут и приду.'
	if rnd_caught_sister = 4 and func('zz_reputation','get') = 4:$rnd_caught_sister = '<<$rnd_rodnya_text>> ваша <<$rnd_namesis_text>>, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text>>: — <<$gop_name_gg>>, ты с клиентами? Долго ещё? Тебе завтра вставать рано.**Сеструха обращается к стоящим рядом парням:**— Вы ебли уже её?**— Неа, первый только начал'
	!----
	!Брат:
	...
	if rnd_caught_brother = 3 and $npc['39,qwPornoFootball'] < 16:$rnd_caught_brother = '<<$rnd_rodnya_text2>> ваш Брат, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text2>>: — <<$gop_name_gg>>, моим корешам только сосёшь, значит, а чёрт знает кому в пизду даёшь?**— Так они и не просили. Попросят — дам хоть в пизду, хоть в жопу."'
	!-----
	!Отчим:
	...
	if rnd_caught_othernpc = 10:$rnd_caught_othernpc = 'Вас заметили: Мамашка, гуляющая с дочкой, хватая ту на руки — Отвернись, не смотри. Эта девочка просто маму не слушала, поэтому её и наказывают. Слышишь, как теперь маму зовёт? Слушайся маму, и никогда не станешь такой, как она. **Обращаясь к вам: — До дома дотерпеть не могла, шалашовка?! Обязательно надо именно здесь своей мандищей трясти?! Тут, между прочим, дети гуляют!'
	if rnd_caught_othernpc = 11 and func('zz_reputation','get') < 4:$rnd_caught_othernpc = '<<$rnd_rodnya_text2>> <<$shulga_name_rnd>>, остановился и спросил у вас: — <<$gop_name_gg>>, ты нахрена этим дрищам даёшь?**— Вообще-то я всем даю.'
	if rnd_caught_othernpc = 11 and func('zz_reputation','get') = 4:$rnd_caught_othernpc = '<<$rnd_rodnya_text2>> <<$shulga_name_rnd>>, остановился и спросил у вас: — <<$gop_name_gg>>, у тебя сегодня сеанс благотворительности что ли?**— С какого перепуга. Я незнакомым бесплатно не даю.'
	if rnd_caught_othernpc = 12 and func('zz_reputation','get') < 4:$rnd_caught_othernpc = '<<$rnd_rodnya_text2>> дядя Миша, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text2>>: — <<$gop_name_gg>>, тебя разогрели уже, может уважишь старика?**— дядь Миш, я как тут закончу к вам заскочу!'
	if rnd_caught_othernpc = 12 and func('zz_reputation','get') = 4:$rnd_caught_othernpc = '<<$rnd_rodnya_text2>> дядя Миша, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text2>>: — <<$gop_name_gg>>, тебя разогрели уже, может уважишь старика?**— Дядь Миш я ближе к вечеру к вам заскочу, но за бесплатно только отсосать могу, а в пизду или в попку за денежку.**— Хорошо я обжумаю твое предложение и буду ждать тебя вечером!'
	if rnd_caught_othernpc = 13 and func('zz_reputation','get') < 4:$rnd_caught_othernpc = '<<$rnd_rodnya_text>> тетя Люда, <<$rnd_viewgg_text>> <<$rnd_talkgg2_text>> <<$rnd_talkgg_text>>: — <<$gop_name_gg>>, опять своих ёбарей притащила! Тебя на весь подъезд слышно!**— Тёть Люб, я тихо не умею.'
	...
	rnd_caughtgg_view = rand(0,100)
		...
		if rnd_caughtgg_view >= 50 and hour >= 5 and hour <= 23:$rnd_caughtgg_view = '**<font color = red><b><<$rnd_caughtgg_npc>></b></font>'
	gs 'zz_render','','','<<$rnd_caughtgg_view>>'
}
!если гг шлюха
gg_whore_text = RAND(0,4)
...
gop_name_gg = RAND(0,3)
	if gop_name_gg = 0:$gop_name_gg = 'Светик'
	if gop_name_gg = 1:$gop_name_gg = 'Света'
	if gop_name_gg = 2:$gop_name_gg = 'Света'
	if gop_name_gg = 3:$gop_name_gg = 'Светуля'
gop_gg_name = RAND(0,4)
...
gop_gg_name2 = RAND(0,4)
	if gop_gg_name2 = 0:$gop_gg_name2 = 'шмара'
	if gop_gg_name2 = 1:$gop_gg_name2 = 'давалка'
	if gop_gg_name2 = 2:$gop_gg_name2 = 'шлюха'
	if gop_gg_name2 = 3:$gop_gg_name2 = 'сука'
	if gop_gg_name2 = 4:$gop_gg_name2 = 'проблядь'
...
!куни:
$text_sex_cun = {
	$_gopstr[0] = 'Вы запустили руку ему в шевелюру и потянула его голову вниз. Он поцеловал вашу киску.**Вы еще больше выпятили киску вперёд, а <<$gopsex_name_npc>> припал к ней губами.**— Ты знаешь, она у тебя красивая.**— Знаю, — улыбнулись вы.**Его язычок проникал вам в киску, раздвигая губки, а ваш стон постепенно переходил на крик.'
	$_gopstr[1] = '<<$gopsex_name_npc>> прислонил лицо к вашей киске.**— Нравится? — спросили вы.**— Очень, — и он прильнул к вашему клитору, лаская его языком. Лизал он хорошо. Тем более для его возраста. Видно было, что он не впервые опускает свой язык в лоно девушки.**— Ты хорошо лижешь. Ты тренируешься?**— Иногда, — сказал он с улыбкой.**— У тебя есть девушка?**— Нет, — он отвечал время от времени отрываясь от процесса.**— А с кем ты там научился?**— Есть знакомая.**— У-у. Я её знаю? Она со школы?**— Нет. Потом расскажу.'
	$_gopstr[2] = 'Вы легли на спину и раздвинули широко ноги. <<$gopsex_name_npc>> отодвинул край трусиков и прильнул к вашей пизде.**Он лизал и сосал губы, клитор, а иногда входил внутрь язычком, а вы стонали, обхватив его голову и прижимая сильнее к своей мокрой киске.'
	$_gopstr[3] = '<<$gopsex_name_npc>> положил вас на спину и припал к вашему сокровищу. Он вылизывал клитор, затем вставил сразу 2 пальца в вашу вагину и стал потрахивать.**Начали раздаваться хлюпающие звуки, вы застонали, а <<$gopsex_name_npc>> лишь ускорил движения.'
	$_gopstr[4] = 'Вы расставили ножки в стороны, предоставив его взору свою сокровенную киску. Не раздумывая, <<$gopsex_name_npc>> стал покрывать поцелуями ваше сокровище.**Он прошелся язычком по клитору, добрался до влагалища, оно оказалось вовсю влажным. Вы давно готовы к сношению. В общем трах можно было начинать прямо сейчас.**Но <<$gopsex_name_npc>> не спешил. Какое то время он ласкал вас языком, пальчиками, слушая ваши сладкие стоны. Вы вся извивались, но ни о чем не просили, просто наслаждались моментом.'
	gs 'zz_render','','',$_gopstr[rand(0,4)]
	killvar '$_gopstr'
}
!-------------------------
...
$text_sex_69 = {
	txt_sex_69 = RAND(0,1)
	if txt_sex_69 = 0:$txt_sex_69 = 'Ваши <<$liptalk3>> податливо принял его член, страстно обсасывая его со всех сторон, словно чупа-чупс, помогали себе язычком, который нежно щекотал уздечку.**<<$gopsex_name_npc>> тем временем обхватил кончиками пальцев клитор и нежно мял его между ними, языком тем временем обследуя вашу дырочку, которая уже давно была готова принять в себя нечто большее...'
	if txt_sex_69 = 1:$txt_sex_69 = '<<$gopsex_name_npc>> лёг на спину, взял вашу ногу и перекинул через себя так, чтобы ваша попка находилась прямо перед его лицом.**Запустив язычок между ваших половых губок, он почувствовал как вы в очередной раз глубоко заглотили его член. Потом чуть ослабив хватку начали его обсасывать своими <<$liptalk>>.'
	gs 'zz_render','','','<<$txt_sex_69>>'
}
!-------------------------
!дрочка руками
$text_sex_hj = {
	txt_sex_hj = RAND(0,3)
	if txt_sex_hj = 0:$txt_sex_hj = '<<$gopsex_name_npc>> без разговоров сунул свой член вам в лицо.**— Вот это по-мужски, — со смехом сказали вы, беря член в руку и принимаясь дрочить.'
	if txt_sex_hj = 1:$txt_sex_hj = 'Вы взяли его член своей рукой, он увеличивался на глазах. Вы начали надрачивать его, любуясь им.'
	if txt_sex_hj = 2:$txt_sex_hj = '<<$gopsex_name_npc>> сунул в вашу руку член. Вы неуверенно зажали упругий, толстенький ствол в маленькой руке. Он заставил вас сжать руку сильнее.**- "Не отпускай... Двигай рукой вверх-вниз...", распоряжался он, показывая, как надо делать.**- "Не так резко... Вот так. Не дергай... Да, вот так. Хорошо... Еще... Еще!"'
...
$text_sex_bj = {
	...
	if txt_sex_bj = 1:$txt_sex_bj = 'Вы взяли член в рот и принялись его сосать. <<$gopsex_name_npc>> схватился руками за ваши <<$hapri>> <<$hair2>> и насаживал вашу голову на свой хуй.**Вы заглатывали член, причмокивая, водили по нему рукой, ощущали своими <<$liptalk>> на нём каждую вену и каждый бугорок.'
	if txt_sex_bj = 2:$txt_sex_bj = 'Вы обхватили его член рукой и, надрачивая, взяли своими <<$liptalk>> головку в рот. Член ещё больше затвердел и принял внушительные размеры.**Вы уже давно научилась довольно неплохо заглатывать член, и с тех пор всегда это применяли.**<<$gopsex_name_npc>> был в восторге. Одной рукой вы ласкали его яички, другой — водили рукой по члену, время от времени отпуская, чтобы взять его в рот полностью.'
...
$text_sex_bjt = {
	...
	if txt_sex_bjt = 1:$txt_sex_bjt = 'Попробовали своими <<$liptalk>> заглотнуть его поглубже. И снова надрачивали рукой, и ласкали ртом. Потом с каждым разов все больше и больше вбирали его в рот по самое горло.**У вас на глазах появились слезы, но вы все равно старались взять глубже. <<$gopsex_name_npc>> взял вас за <<$hapri>> <<$hair2>> и стал сам наяривать, натягивая вашу голову себе на член.'
	if txt_sex_bjt = 2:$txt_sex_bjt = 'Головка упиралась в самое горло и проникала еще глубже, но вы не могли засунуть ее дальше — уж слишком толстой она была.**Однако с каждым разом на пару миллиметров, немного, но она пролизала все глубже и глубже. По вашим <<$liptalk2>> текла слюна капая на пол.'
	gs 'zz_render','','','<<$txt_sex_bjt>>'
}
!-------------------------
!раком в киску
$text_sex_doggy = {
	txt_sex_doggy = RAND(0,4)
	if txt_sex_doggy = 0:$txt_sex_doggy = '<<$gopsex_name_npc>> был в восторге от вас. Он вскочил, поставил вас раком, задрал юбку, открыв вашу прилесную попку, и вонзил член в киску сзади.**Вы не ожидали таких резких действий, однако он трахал вас минут десять, практически не сбавляя темпа, лишь изредка останавливаясь, чтобы перевести дыхание.**Вы думали, что он вот-вот выдохнется, но он оказался выносливее, чем можно было предположить. Последние минут пять вы забыли, где находитесь, и кричали во всё горло.**<<$gopsex_name_npc>> рычал, одной рукой шлёпая по заднице, другой взяв вас за <<$hapri>> <<$hair2>>.'
	if txt_sex_doggy = 1:$txt_sex_doggy = '<<$gopsex_name_npc>> надрочил свой член и насадил вас на него. Вы принялись подмахивать задом. Он скользил внутри вас, упирался вам в матку.**Вы прогибали спину, а он руками сильнее ухватился за вашу жопу и начал сильнее ебать.'
	if txt_sex_doggy = 2:$txt_sex_doggy = 'Вы тяжело дышали стоя раком и старательно подмахивали попочкой. <<$gopsex_name_npc>> стоял сзади и натягивал вас как кобель ссучку.**Грубые руки <<$gopsex_name_npc>> сжимали вашу попку помогая ей глубже насаживаться на его окаменевший ствол.**Вы стонали от удовольствия, ваша попка стремительно двигалась навстречу этому восхитительному и ненасытному стволу.**<<$gopsex_name_npc>> поднял руку и с оттяжкой шлепнул вас по заднице.**— "Молодец, шлюшка, ну ка еще наподдай попочкой. Давай лапочка, проворней работай", приговаривал <<$gopsex_name_npc>> шлепая вас то одной то другой рукой.'
	if txt_sex_doggy = 3:$txt_sex_doggy = '<<$gopsex_name_npc>> поставил вас в позу раком и встал сзади вас. Затем он одним резким движением засунул хуй, вошел грубо и резко одним толчком, в вашу мокрую пизду.**Вы дернулись и вскрикнули от пронизывающей и резкой боли, но остались покорно стоять раком.**Он начал жестко насаживать вас под самый корень, он вбивал в вас свой кол так, как будто пытался вогнать его глубже и глубже с каждым толчком.**Он яростно ебал свою одноклассницу, держа одной рукой за <<$hapri>> <<$hair2>>. Движения <<$gopsex_name_npc>> были резкими и сильными.**Тишину прорезали громкие шлепки. Его таз с силой ударялся об упругую задницу бляди, которая в этот момент стонала сквозь сжатые зубы.'
	if txt_sex_doggy = 4:$txt_sex_doggy = '— Спускайся на пол и встань на четвереньки, моя сладкая.**Вы безоговорочно подчинились, встали раком на четвереньки. Вам нравилось, когда вас трахают в этой позе.**— "Светик прогни спинку, а голову опусти ниже".**— "Да, <<$gopsex_name_npc>>", cказали возбужденным голосом и сильнее прогнули спину и опустили голову.**Ваша попка задралась еще выше, а киска стала ближе и доступнее, что очень понравилось ему.**<<$gopsex_name_npc>> опустился на пол и развел ее ноги в стороны, устраиваясь между ними на колени. Затем он широко раздвинул ее ягодицы, медленно провел членом по мокрой пизде, засадил глубоко.**— Ммм... ааааах... мммм, застонали вы.**<<$gopsex_name_npc>> положил обе руки на ее попу и с наслаждением скользил своим членом в ее пизде.'
	gs 'zz_render','','','<<$txt_sex_doggy>>'
}
!-------------------------
!раком в попу
$text_sex_doggy_anal = {
...
— У тебя большая и красивая задница, Светик..., не меняя темпа сказал он.
...
В глазах все поплыло, и проносились какие-то разноцветные тени и узоры. <<$gopsex_name_npc>> монотонно и сильно трахал вас, сжимая вашу попку.'
	gs 'zz_render','','','<<$txt_sex_doggy_anal>>'
}
!-------------------------
!сверху
$text_sex_cow = {
...
Он вoшeл дo упoрa и принялся oстoрoжнo двигaться, слoвнo бoясь причинить вам бoль. Ваша попка былa жaркaя, кaк пeчь. Вы с ним в унисoн принялись двигaться, зaбыв oбo всeм нa свeтe.'
	...
	gs 'zz_render','','','<<$txt_sex_miss_anal>>'
}
!-----------!
! окончание !
!===========!
...
$gopsex_select = {
	*clr & cla
	...
	gs 'stat'
	if horny <50 and func('zz_clothing2', 'get_group') ! 7: '<center><img src="images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/body/b'+rand(1,17)+'.jpg"></center> ** Вы хотите секса.'
	if horny >=50 and func('zz_clothing2', 'get_group') ! 7: '<center><img src="images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/body/g'+rand(1,20)+'.jpg"></center> ** Вы перевозбуждены и сильно хотите трахаться.'
	if func('zz_clothing2', 'get_group') = 7: '<center><img src="images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/body/h'+rand(1,20)+'.jpg"></center>'
	act 'Потрахаться с Васькой':
		if shgopsex_vasya_count = 0:
			cla
			gs 'zz_render','','','Вася говорит: -"<<$gop_name_gg>> заебала уже меня, на сегодня хватит секса, не приставай."'
	...
	if week < 6 and hour >= 0 and hour < 3:
		act 'Потрахаться с Дэном':
			if shgopsex_den_count = 0:
				cla
				gs 'zz_render','','','Дэн говорит: -"Вот ты <<$gop_gg_name>> заебала меня сегодня, отъебись."'
				act 'Уйти': gt'shulga_room'
			else
				$gopsex_name_npc = 'Дэн'
				$gopsex_n_npc = 0
		...
		act 'Потрахаться с Витьком':
			if shgopsex_vitya_count = 0:
				cla
				gs 'zz_render','','','Витек говорит: -"Извини <<$gop_name_gg>>, больше сегодня не смогу."'
				act 'Уйти': gt'shulga_room'
			else
...
$shgopsex = {
	cla
	gs 'stat'
	gs 'zz_render','','','- "Ну чего <<$gopsex_name_npc>> как меня <<$gop_sextext1>> будеш?. Что мне для тебя сделать?", игриво спрашиваете вы.'
	if mesec > 0 and vagina >=1:
		gs 'zz_render','','','<font color = red><b>Вы предупреждаете, что у вас месячные и в пизду доступ закрыт!</b></font>'
	elseif vagina = 0:
		gs 'zz_render','','','<font color = red><b>Вы предупреждаете, что вы бережете свою девственность, по этому в письку не даете!</b></font>'
	end
	...
	*clr & cla
	...
	if $gopsex_n_npc = 1:
		if func('zz_reputation','get') >= 3:
			gs 'zz_render','','','<<$gop_name_gg>> ты же у нас <<$gnikname>>, мне западло такой бабе пизду лизать, хуй знает сколько там за сегодня хуев побывало. Так что давай, подставляй свои блядские дырки, ебать буду!'
		else
			horny += 10
			minut += rand(3,7)
			dynamic $ind_stamina_npc
			gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/lick/lick'+rand(1,11)+'.jpg'
	...
	if $gopsex_n_npc = 2:
		if func('zz_reputation','get') >= 3:
			gs 'zz_render','','','<<$gop_name_gg>> ты же у нас <<$gnikname>>, мне западло такой бабе пизду лизать, хуй знает сколько там за сегодня хуев побывало. Так что давай, лучше подставляй свои блядские дырки!'
		else
			horny += 10
			minut += rand(3,7)
			dynamic $ind_stamina_npc
	...
	!Дэн (нпс 0)
	if $gopsex_n_npc = 0:
	gs 'zz_render','','','- "Дэн, полижи мне письку", ласково просите вы.'
	gs 'zz_render','','','- <<$gop_name_gg>>, а ты не ахуела ли, мне такое предлагать?'
	gs 'zz_render','','','- А что тут такого?, тебе западло сделать мне приятно?'
	gs 'zz_render','','','- Да, мне западло давалке пизду лизать, хуй знает сколько там за сегодня хуев побывало... Так что давай, не еби мне мозг, подставляй свои блядские дырки, ебать буду!'
	end
	...
	*clr & cla
	...
	if $gopsex_n_npc >= 1:
		if func('zz_reputation','get') >= 3:
			...
			sh_bjandhj_cum = 1
			dynamic $ind_stamina_npc
			gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/lick/69_'+rand(1,3)+'.jpg'
			dynamic $text_sex_69
			dynamic $gg_sex_orgasm
			dynamic $npc_orgasm
			if shgopsex_count > 0:
		...
		gs 'zz_render','','','- "Нет, <<$gop_name_gg>> лизать пизду я не буду"'
		act 'Выбор секса': dynamic $shgopsex
	end
}
!===============================================================!
...
$shgopsex_eattits = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/tits/eat'+rand(1,2)+'.jpg'
	gs 'zz_render','','','<<$gopsex_name_npc>> покусывает вашу сиську.'
...
$shgopsex_fucktits = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/tits/tj'+rand(1,9)+'.jpg'
	gs 'zz_render','','','<<$gopsex_name_npc>> хочет погонять хуй между ваших сисек, а вы и не против.'
	if shgopsex_count > 0:
		act 'Выбор секса': dynamic $shgopsex
		!act 'Закончить': dynamic $shgopsex_end
	else
...
$shgopsex_tits= {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/tits/tits'+rand(1,6)+'.jpg'
	gs 'zz_render','','','- "Ну как <<$gopsex_name_npc>> нравится мои сиськи <<$gop_text3>>?"'
	gs 'zz_render','','','- "Да <<$gop_name_gg>>, заебись сиськи у тебя!"'
...
$shgopsex_hj = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/hj'+rand(1,12)+'.jpg'
	dynamic $text_sex_hj
	dynamic $npc_orgasm
	if shgopsex_count > 0:
		act 'Выбор секса': dynamic $shgopsex
...
$shgopsex_bj = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/oral/bj'+rand(1,25)+'.jpg'
	dynamic $text_sex_bj
	dynamic $npc_orgasm
	if shgopsex_count > 0:
		act 'Выбор секса': dynamic $shgopsex
...
$shgopsex_throat = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/oral/throat'+rand(1,15)+'.jpg'
...
$shgopsex_vag = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/pussy/dog'+rand(1,20)+'.jpg'
...
$shgopsex_vag1 = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/pussy/miss'+rand(1,20)+'.jpg'
...
$shgopsex_vag2 = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/pussy/cow'+rand(1,20)+'.jpg'
...
$shgopsex_vag3 = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/pussy/bok'+rand(1,3)+'.jpg'
	dynamic $gg_sex_orgasm
	dynamic $npc_orgasm
	if shgopsex_count > 0:
		act 'Сменить позу: раком': dynamic $shgopsex_vag
...
$shgopsex_anal = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/anal/dog'+rand(1,20)+'.jpg'
...
$shgopsex_anal1 = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/anal/miss'+rand(1,20)+'.jpg'
	dynamic $gop_sex_analplugin
	dynamic $gop_sex_lubri
...
$shgopsex_anal2 = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/anal/cow'+rand(1,20)+'.jpg'
	dynamic $gop_sex_analplugin
	dynamic $gop_sex_lubri
...
$shgopsex_anal3 = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_npc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/sex/anal/bok'+rand(1,9)+'.jpg'
	dynamic $gop_sex_analplugin
	dynamic $gop_sex_lubri
	gs 'zz_render','','','на боку'
	dynamic $gg_sex_orgasm
	dynamic $npc_orgasm
	if shgopsex_count > 0:
		act 'Сменить позу: раком': dynamic $shgopsex_anal
...
$shgopsex_cumface = {
	*clr & cla
	...
	gs 'stat'
	gs 'zz_render','','common/sex/facial/<<rand(0,4)>>.jpg'
...
$shgopsex_cumbelly = {
	*clr & cla
	...
	gs 'stat'
	gs 'zz_render','','common/sex/cum_body/<<rand(0,4)>>.jpg'
...
$shgopsex_cumvag = {
	*clr & cla
	minut += rand(1,3)
	gs 'stat'
	gs 'zz_render','','common/sex/cum_pussy/<<rand(0,4)>>.jpg'
...
$shgopsex_cumanal = {
	*clr & cla
	...
	gs 'zz_funcs', 'cum', 'ass'
	gs 'stat'
	gs 'zz_render','','common/sex/cum_ass/<<rand(0,4)>>.jpg'
...
-Давай Светик, возьми у меня в ротик тоже, - так мило и по простому попросил Витек.
...
Через пару минут подключился и Дэн, ваш рот уже сношали трое, то по очереди, то пытаясь сразу три хуя пропихнуть...'
	gs 'zz_render','','','<<$txt_sex_group_bjall>>'
}
!отсос по очереди
$text_sex_group_bj = {
...
По их довольным стонам, вы поняли, что это им безумно нравится.'
	gs 'zz_render','','','<<$txt_sex_group_vag>>'
}
...
Постепенно темп члена в вашей попке нарастал, вместе с ним нарастало и ваше собственное возбуждение.'
	gs 'zz_render','','','<<$txt_sex_group_anal>>'
}
...
Не выдержал такой картины Витек и подошел к вашему рту и присунул свою залупу, вы начали умело обсасывать его хуй, активно подмахивая попкой активнее насаживаясь на хуи ребят.'
	gs 'zz_render','','','<<$txt_sex_group_dpa>>'
}
...
$text_sex_group_triple = {
	...
	gs 'zz_render','','','<<$txt_sex_group_triple>>'
}
!выбор секса:
$shgopsex_group = {
	cla
	gs 'stat'
	group_sex_text = RAND(0,4)
...
$shgopsex_group_bj = {
	*clr & cla
	...
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/sex/oral/bj'+rand(1,3)+'.jpg'
...
$shgopsex_group_bja = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_groupnpc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/sex/oral/bja'+rand(1,25)+'.jpg'
...
$shgopsex_group_vag = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_groupnpc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/sex/vag'+rand(1,26)+'.jpg'
...
$shgopsex_group_anal = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_groupnpc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/sex/anal'+rand(1,18)+'.jpg'
...
$shgopsex_group_dp = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_groupnpc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/sex/dp'+rand(1,30)+'.jpg'
	dynamic $gop_groupsex_analplugin
	dynamic $gop_groupsex_lubri
...
$shgopsex_group_twoanal = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_groupnpc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/sex/dpa'+rand(1,10)+'.jpg'
	dynamic $gop_groupsex_analplugin
	dynamic $gop_groupsex_lubri
...
$shgopsex_group_tripleanal = {
	*clr & cla
	...
	gs 'stat'
	dynamic $ind_stamina_groupnpc
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/sex/triple'+rand(1,4)+'.jpg'
	dynamic $gop_groupsex_analplugin
	dynamic $gop_groupsex_lubri
...
$shgopsex_group_cumface = {
	*clr & cla
	...
	minut += rand(2,5)
	gs 'stat'
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/cum/cumface'+rand(1,23)+'.jpg'
	gs 'zz_render','','','<<$gop_text_npcr>> обкончали ваше милое личико.'
	act 'Уйти': gt'shulga_room'
}
!окончание в рот
$shgopsex_group_swallow = {
	*clr & cla
	...
	minut += rand(2,5)
	gs 'stat'
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/cum/swallow'+rand(1,12)+'.jpg'
	gs 'zz_render','','','<<$gop_text_npcr>> излили сперму вам в рот и вы её проглотили.'
	act 'Уйти': gt'shulga_room'
}
!окончание на тело
$shgopsex_group_cumbelly = {
	*clr & cla
	...
	minut += rand(2,5)
	gs 'stat'
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/cum/cumbody'+rand(1,15)+'.jpg'
	gs 'zz_render','','','<<$gop_text_npcr>> обтрухали все ваше тело.'
	act 'Уйти': gt'shulga_room'
}
!окончание в пизду
$shgopsex_group_cumvag = {
	*clr & cla
	...
	minut += rand(2,5)
	gs 'stat'
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/cum/cumvag'+rand(1,6)+'.jpg'
	gs 'zz_render','','','<<$gop_text_npcr>> по очереди излились вам в пизду.'
	act 'Уйти': gt'shulga_room'
}
!окончание в попу
$shgopsex_group_cumanal = {
	*clr & cla
	...
	minut += rand(2,5)
	gs 'stat'
	gs 'zz_render','','images/zsoft/pod_ezd/shulgahome/bigroom/gopsex/group/cum/cumass'+rand(1,9)+'.jpg'
	gs 'zz_render','','','<<$gop_text_npcr>> по очереди накачали вашу жопу спермой.'
	act 'Уйти': gt'shulga_room'
}