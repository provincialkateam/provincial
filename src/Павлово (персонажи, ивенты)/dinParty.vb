...
$Party_rex_talk ={
	*clr & cla
	...
	if Rex_Love = 1:
		...
		i = rand(1,7)
		if i = 1: gs 'zz_render', '', '', '- На планете есть семь чудес Света, но есть еще одно, самое очаровательное и красивое - это ты!'
	...
	if Rex_Love = 0:
		...
		i = rand(1,7)
		if i = 1:gs 'zz_render', '', '', '- Светик, ты очень красивая, а красивых девушек надо размножать.'
		if i = 2:gs 'zz_render', '', '', '- Не грусти, а то грудь не будет расти, веселись и сексом займись, ахаха.'
		if i = 3:gs 'zz_render', '', '', '- Светик, от тебя так приятно пахнет! Что ты сегодня пила?'
