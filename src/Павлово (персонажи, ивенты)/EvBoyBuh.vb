if $args[0] = 'main' or $args[0] = '':
	...
	elseif func('zz_reputation','get') <= 2:
		if rand(0,10) = 0:
			gs 'zz_render', '', '', '<<$bname>> сказал:**\\\- Светик '+iif(func('zz_reputation','get') = 1,'я слышал, что ты всем даешь','я слышал, что ты в рот берешь')+'...///**Парень достал член из штанов.'
			act 'Сесть на корточки': gt 'EvBoyBuh','boy_bj'
	...
	elseif func('zz_reputation','get') = 3:
		gs 'zz_render', '', '', '<<$bname>> сказал:**\\\- Светик ты же <<$gnikname>>. Я хочу поразвлечься!///**Парень достал член из штанов.'
		act 'Сесть на корточки': gt 'EvBoyBuh','boy_bj'
		gs 'EvBoyBuh','boy_decline'
	elseif func('zz_reputation','get') = 4:
		gs 'zz_render', '', '', '<<$bname>> сказал6**\\\- Светик ты же <<$gnikname>>. Обсужи меня, вот тебе 300 рублей...///**После этого парень достал член из штанов.'
		...
		if dom >= 10:
			act 'Обругать парня':
				*clr & cla
				dom += 2
				minut += 3
				if func('zz_reputation','get') > 0 and rand(0,1) = 0: gs 'zz_reputation','edit',3
				gs 'stat'
...
if $args[0] = 'boy_sex':
	*clr & cla
	...
	horny += rand(5,10)
	guy += 1
	if func('zz_reputation','get') = 0 and RAND(0,100) >= 50: gs 'zz_reputation','set',1
	pose = 1
	dynamic $venerasiak
	gs 'stat'
...
if $args[0] = 'boy_bj':
	*clr & cla
	...
	dom -= 1
	gs 'stat'
	gs 'zz_render','','common/sex/public_bj/<<rand(0,4)>>.gif', 'Вы сели на корточки перед парнем и стали дрочить его член.'
	act 'Сосать':
		*clr & cla
		minut += 5
		horny += 10
