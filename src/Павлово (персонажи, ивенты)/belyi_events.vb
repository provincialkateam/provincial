if $args[0] = 'lake_start':
	if belgangQW = 0:
		...
		act 'Не помешаете':
			...
			minut += 5
			gs 'stat'
			gs 'zz_render', '', 'pavlovo/belyi/take_belly.webm', 'Вы пожали плечами и постарались отвлечься от этих братков закрыв глаза. Но не тут то было, вы почувствовали, что вас по животу что-то легонько гладит и щекочет. Открыв глаза вы увидели, что толстопузый мужик водит кончиком своего пальца по вашему животу.'
		...
		act 'Извиняться':
			...
			act 'Я заплачу! Только отпусти!':
				...
				act'Хорошо, завтра все отдам.':
					...
					belgangday = daystart
					gs 'stat'
					gs 'zz_render', '', 'pavlovo/belyi/belyi_lake.jpg', 'Вы согласились с условиями и Сиплый отпустил вашу руку. Плешивый усмехнулся показав золотые фиксы:**\\\- И учти Светик, не отдашь <<belgangPay>> рублей, будешь отрабатывать. А в ментовку сунешся, мы тебя в лесочке прикопаем.///**После этого бандиты поднялись и ушли с пляжа.'
					act 'Встать': gt'glake'
				end
...
if $args[0] = 'house_bj':
	...
	sex_act -= 1
	gs 'stat'
	gs 'zz_render', '', 'pavlovo/belyi/sex/bj'+rand(1,3)+'.webm', 'Вы берете член в свой ротик и начинаете его сосать ощущая как твердый стержень ходит в вашем ротике. Белый взял вас за волосы и начал двигать членом навстречу вашим губам, буквально насаживая вашу голову на свой член.'
	if sex_act <= 0:
		...
		act 'Открыть рот':
			...
			gs 'zz_funcs', 'cum', 'lip'
			gs 'stat'
			gs 'zz_render', '', 'pavlovo/belyi/sex/cum.webm', 'Член начинает кончать и теплые струи вязкой спермы попадают вам в рот и на лицо. Вскоре поток спермы иссякает оставляя вас с противным вкусом во рту и обтекающей белой слизью по коже вашего лица.'
...
if $args[0] = 'house_virgin':
	...
	if bel_know_virgin = 0:
		bel_know_virgin += 1
		gs 'zz_render', '', '', '\\- Прошу, не нужно, пожалуйста! - //взмолились вы, глядя на Белого.\\ - Я ещё девочка...//**\\\- И чо? Ща мы это быстро исправим! -/// хохотнул в ответ Белый.**\\- Ну пожалуйста, не надо,-// продолжили вы упрашивать потупив глазки. \\- '+iif(age < 18,'Меня мамка заругает, если узнает...','Я для будущего мужа девственность берегу...')+'//**\\\- Хера себе! -/// удивился Белый.\\\ - '+iif(age < 18,'Такая здоровая кобыла, а мамку боится...','Типа правильная что-ли?')+'///**Белый сел на кровать и почесал пузо:**\\\- Ладно, становись рачком тогда, попользуем другую твою дырку!///'+iif(anus < 10,'**\\- А может я лучше ротиком? -// попробовали вы уклонится от новой напасти.**\\\- Не борзей, Света, я и так сегодня что-то добрый, -/// отвел Белый, хлопнув вас по заду. \\\- Давай, вставай рачком и раслабь булки...///','')
	...
	elseif bel_know_virgin > 0 and bel_know_virgin < 10:
		...
		act 'Давай лучше в попу':
			cla & *clr
			gs 'zz_render', '', 'pavlovo/belyi/sex/doggy_style'+iif(anus <= 10,1,iif(anus <= 25,2,3)), '\\- Не, давай всё таки лучше в попу... - //сказали вы, привычно занимая коленно-локтевую позицию.'
...
if $args[0] = 'house_sex':
	...
	pose = 0
	gs 'stat'
	gs 'zz_render', '', 'pavlovo/belyi/sex/sex'+iif(vagina = 0,'',rand(1,3))+'.webm'
...
if $args[0] = 'meet':
	...
	if belgangQW = 1:
		gs 'zz_render', '', '', iif($loc = 'glake','Пока вы загораете к вам подсаживается Белый со своими братками, и почесывая пузо, лениво спрашивает:**\\\- Ну чо Светик, прокатимся?///','К вам подъезжает чёрный, потрёпаный БМВ и из открытого водительского окна выглядывает толстая плешивая рожа Белого:**\\\- Короче Светик, садись, покатаемся.'+iif(bel_refuse >= 3,' Возражения не принимаются!',''))+'///'
		act iif($loc = 'glake','Ехать с белым','Сесть в машину'):
			...
			bel_refuse = 0
			gs 'stat'
			gs 'zz_render', '', 'pavlovo/belyi/go_car'+iif(month >= 4 and month <= 10,'','_winter'), 'Вы сели в машину и Белый поехал по городу.'
			if rand(1,100) <= 30 and $loc ! 'glake':
				gs 'zz_render', '', '', 'Белый расстегивая ширинку сказал:**\\\- Светик , у меня мало времени, поэтому давай побыстрей...///'
				act'Отсосать':
					cla & *clr
					bj += 1
					gs 'zz_funcs', 'cum', 'lip'
					...
					money += 1000
					gs 'stat'
					gs 'zz_render', '', 'pavlovo/belyi/car_bj'+rand(1,3)+'.webm', 'Вы наклонились к члену Белого и начали его сосать, толстяк долго не продержался и противная сперма стала выстреливать вам в рот. Выплевывать её было некуда и вы сглотнули. Когда Белый прекратил кончать, он остановил машину и сунув вам 1000 рублей выпроводил вас на улицу.'
		...
		if dom > 0 and bel_refuse < 3:
			act 'Отказаться':
				cla & *clr
				bel_refuse += 1
				gs 'zz_render', '', 'pavlovo/belyi/belyi_'+iif($loc = 'glake','lake','car'), '\\- Слушай, Белый, я сейчас никак не могу, давай в другой раз? - попросили вы Белого.**\\\- Лады, в другой, так в другой, - недовольным голосом ответил он и '+iif($loc = 'glake','сплюнув на землю пошёл прочь вместе со своей братвой.','рыкнув двигателем умчался прочь.')
				act 'Уйти': gt $loc, $metka
			end
		end
	end
	if belgangQW = 2:
		...
		act iif($loc = 'glake','Ехать с белым','Сесть в машину'):
			...
			gs 'stat'
			gs 'zz_render', '', 'pavlovo/belyi/ride'+iif(month >= 4 and month <= 10,'','_winter'), 'Вы сели на заднее сиденье и Белый поехал по городу.'
	...
	!if belgangQW = 3:
	!	belgangQW = 1
	!	gs 'zz_render', '', '', iif($loc = 'glake','Пока вы загораете к вам подсаживается Белый со своими со своими братками','Возле вас остановился, видавший виды, чёрный БМВ и из него вылезли братки. Белый тут же подошел к вам и спросил')+':**\\\- Ну как дела Света? Есть хорошие новости?///'
	!	if money >= belgangPay:
	!		act 'Отдать деньги':
	!			cla & *clr
	!			money -= belgangPay
	!			gs 'stat'
	!			gs 'zz_render', '', 'pavlovo/belyi/money.jpg', 'Вы достали деньги и передали их бандитам. Белый усмехнулся:\\\- Надеюсь пересчитывать не надо? А то мы можем вернуться.///**Вы отрицательно покачали головой:**\\- Тут все деньги. И лучше пересчитайте при мне.//**Белый посчитал деньги:\\\- Лады, вся сумма на месте. Ну не хворай Светик, думаю мы ещё не раз пересечёмся.**'+iif($loc = 'glake','Он поднялся на ноги и неспешным шагом отправился дальше.','После этого вся банда села опять в БМВ и тот, рыкнув мощным мотором, уехал.')
	!			act 'Уйти': gt $loc, $metka
	!		end
	!	else
	!		act 'У меня нет таких денег':
	!			cla & *clr
	!			gs 'zz_render', '', 'pavlovo/belyi/crying_meet.jpg', 'Вы едва не плача покачали головой:**\\- У меня нет таких денег.//**Плешивый нахмурился:**\\\- Ай ай ай, Светик, как нехорошо, ну ничего, я тебя в одно теплое место пристрою, там ты мне все живо отработаешь.///**Вас тут же грубо схватили и '+iif($loc = 'glake','поволокли к выходу с пляжа. Напрасно вы брыкались и звали на помощь, все присутствующие в это время на пляже старательно отводили глаза, не решаясь перечить мордоворотам Белого. Дойдя до машины, вас силой посадили на заднее сидение','силой посадили в машину')+'. Вся банда расселась по местам и БМВ, взревев, помчался по трассе в сторону города.'
	!			act 'Ехать':
	!				cla & *clr
	!				if cloth[0] < 3: gs 'zz_clothing2','redress',cloth[3]
		...
		if shameless['flag'] > 0:
			if dance < 50:
				...
				act'Сесть на корточки':
					...
					gs 'stat'
					gs 'zz_render', '', 'pavlovo/belyi/gang/bj<<bel_picnic>>.webm', 'Вы садитесь на корточки в центр круга и начинаете обрабатывать своим ротиком четыре члена.'
					fuckerrand = rand(1,4)
					gmember = fuckerrand
...
if $args[0] = 'gang_2':
	...
	times_picknick -= 1
	pose = 0
	gs 'zz_render', '', 'pavlovo/belyi/gang/mis'+iif(bel_picnic = 1,rand(0,2),rand(3,5))+'.webm'
	gs 'zz_dynamic_sex', 'sex_start'
...
if $args[0] = 'gang_3':
	...
	pose = 1
	if rand(0,1) = 0:
		gs 'zz_render', '', 'pavlovo/belyi/gang/dog'+iif(bel_picnic = 1,rand(0,2),rand(3,5))+'.webm'
		gs 'zz_dynamic_sex', 'sex_start'
		gs 'zz_dynamic_sex', 'vaginal', 'dick'
		gs 'zz_dynamic_sex', 'sex_cum'
	else
		gs 'zz_render', '', 'pavlovo/belyi/gang/anal'+iif(bel_picnic = 1,rand(0,2),rand(3,5))+'.webm'
		gs 'zz_dynamic_sex', 'anal_start', 'dick'
...
if $args[0] = 'gang_4':
	...
	gs 'zz_render', '', 'pavlovo/belyi/gang/dp'+iif(bel_picnic = 1,rand(0,2),rand(3,5))+'.webm', '<<$boy>> сажает вас на себя готовясь войти вам во влагалище. <<$boy2>> устраивается сзади вас готовясь войти в вашу попу, а оставшиеся двое бандитов подставляют вам свои члены ко рту...'
	...
	act'Сидеть':
		...
		gs 'zz_funcs', 'cum', 'face'
		gs 'zz_funcs', 'cum', 'lip'
		gs 'zz_render', '', 'pavlovo/belyi/gang/cum<<bel_picnic>>.webm', 'Горяие струи спермы начинают подать вам на лицо, в глаза, растекатmся по щекам и затекать в рот. Наконец-то бандюки прекращают кончать и оставляют вас в покое...'
		act'Встать': gt 'belyi_events','gang_end'
	end
end
if $args[0] = 'gang_end':
	...
	minut += 60
	i = rand(1,100)
	gs 'zz_render', '', 'pavlovo/belyi/gang/picnic_end'+iif(i < 60,'',bel_picnic), 'Бандиты начали собираться обратно, громхо хохоча и обсуждая прошедшую гулянку.'
	if belgangPay > 0:
		belgangPay -= 1000
		if belgangPay > 0:
	...
	if i < 60:
		...
		act 'Идти к дороге': gt 'road','main','1'
	end
end
if $args[0] = 'bottle_vag_light':
	...
	protect = 1
	gs 'stat'
	gs 'zz_render', '', 'pavlovo/belyi/gang/bottle_vag_light<<bel_picnic>>.webm'
...
if $args[0] = 'bottle_vag_hard':
	...
	protect = 1
	gs 'stat'
	gs 'zz_render', '', 'pavlovo/belyi/gang/bottle_vag_hard<<bel_picnic>>.webm'
	gs 'zz_dynamic_sex','sex_start', 1
	gs 'zz_dynamic_sex','vaginal','bottle'
	if rand(1,100) <= 80:
...
if $args[0] = 'bottle_anal_light':
	...
	dick = 10
	gs 'stat'
	gs 'zz_render', '', 'pavlovo/belyi/gang/bottle_anal_light<<bel_picnic>>.webm'
...
if $args[0] = 'bottle_anal_hard':
	...
	dick = 40
	gs 'stat'
	gs 'zz_render', '', 'pavlovo/belyi/gang/bottle_anal_hard<<bel_picnic>>.webm'
	gs 'zz_dynamic_sex','anal_start','bottle'
	gs 'zz_dynamic_sex','anal','bottle'
