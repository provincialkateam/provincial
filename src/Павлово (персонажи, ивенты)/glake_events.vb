	...
	if vnesh >= i:
		...
		elseif i = 4:
			...
			act'Света':
				cla & *clr
				gs 'stat'
				gs 'zz_render', '', 'pavlovo/glake/lake_boy/boy.jpg', 'Вы отвечаете парню, что вас зовут Света.'
				i = RAND(0,200)
				if vnesh >= i:
					...
					act'Выпить пива':
						...
						gs 'zz_funcs', 'alko', 3
						gs 'stat'
						gs 'zz_render', '', 'pavlovo/glake/lake_boy/beer.jpg', 'Вы выпили пива с парнем. Пока выпили, он всё время травил вам байки и анекдоты, стараясь вас развеселить. Вдруг он положил свою ладонь вам на ногу:\\\- Еще по пиву Светик?///'
	...
	act'Целоваться':
		...
		if rand(1,100) < 70:
			...
			act'Дрочить':
				...
				minut += 5
				gs 'stat'
				gs 'zz_render', '', 'pavlovo/glake/lake_boy/hj.gif','Вы берете член парня в руку и начинаете его дрочить, вскоре <<$bname>> сдавленно говорит:**\\\- Света, я сейчас кончу...///'
...
if $args[0] = 'LBZhouse_bj':
	...
	gs 'zz_render', '', '','Вскоре <<$bname>> сдавленно говорит:**\\\- Света, я сейчас кончу...///'
	act'Сосать дальше':
		cla & *clr
		gs 'zz_funcs', 'cum', 'lip'
	...
	if voyeurism_start >= 2:
		act 'Подсматривать за нудистами':
			if voyeurism_nudist < 5 or func('zz_reputation','get') > 2 or shameless['flag'] > 1:
				...
				cla & *clr
				gs 'zz_render','','pavlovo/glake/self/self'+iif(cloth[0] = 2,1,iif(lobok <= 0,2,iif(lobok <= 2,3,4)))+'.jpg','Вы хотите ещё поподсматривать за нудистами, но вдруг замечаете, что на вас начали странно коситься.**\\"— Наверное хватит пока подсматривать..."// — думаете вы и смотрите прямо перед собой.'
				act 'Назад': gt 'gLakeNude'
			end
		end
	end
...
if $args[0] = 'gLakeNude_sex_strings':
	...
	if $args[1] = 'start_ev4': $result = 'Как тень вы бросаетесь в сторону леса. Кусты и ветки больно царапают по вашей коже. Сухие иголки от хвойных деревьев вонзаются в ступни, заставляя вас с грустью вспомнинать об обуви, которая остались лежать на пляже.**<dh>– По крайней мере они меня не заметили и здесь меня не видно с пляжа.</dh> – думаете вы.**Аккуратно пройдясь по лесу, вы обнаруживаете кусты, за которыми открывается шикарный вид на купающуюся парочку. Понимая, что делать всё равно нечего, вы присаживаетесь и продолжаете наблюдать.'
	...
	if $args[1] = 'start_ev17': $result = 'Сперма тяжёлыми каплями падает на перепачканный песком животик девушки. Вы видите, как она выгибается, пытаясь поймать язычком хотя-бы капельку.**Закончив парень обессилено валится на девушку сверху. Похоже ничего интересного больше не будет.'
