...
if $args[0] = 'meet_misha':
	if func('zz_weather','is_day')=1 and cumface > 0 and rand(0,3) = 0 and sosedday ! day:
		*clr & cla
		sosedday = day
		gs 'zz_render', '', 'pavlovo/misha/misha.jpg',iif(sosedknow = 0,'Внезапно вас кто-то окликает:**\\\- Света...///** Вы оборачиваетесь и видите соседа дядю Мишу. Он смотрит на ваше лицо с удивлением:**\\\- Это что у тебя на лице?///**Вы '+iif(shameless['flag'] < 2,'краснеете и смущаетесь',iif(shameless['flag'] = 2,'краснеете','улыбаетесь'))+', стараясь рукой стереть сперму с лица.**\\\- Ну ну, все понятно... -/// ухмыляется он и идет дальше, по своим делам.','Вы опять столкнулись нос к носу с дядей Мишей, вашим соседом. Он хмыкнул, посмотрев на вас:**\\\- Ну Светик ты и шлюха...///')
		sosedknow = 1
		...
		act 'Присоединиться к субботнику':
			*clr & cla
			...
			act 'Закончить':
				...
				water -= 1
				son -= 1
				gs 'zz_reputation','edit', rand(5,10)
				gt 'gdk'
			end
		end
	end
	...
	*clr & cla
	...
	if (func('zz_reputation','get') >= 3 or $npc['42,qwDyadyaMisha'] >= 1) and dmishaday ! day:
		...
		if i = 1 and func('zz_reputation','get') >= 3:
			gs 'zz_render', '', '', 'Увидев вас, он предложил:**\\\-Света, может ублажишь старика, отсосешь по быстрому? А я тебе 100 рублей дам!///'
			act 'Сосать':
				*clr & cla
				dmishaday = day
				money += 100
		...
		elseif i = 2:
			gs 'zz_render', '', '', 'Увидев вас, он предложил:**\\\-"Света может ублажишь старика, зайдёшь в гости? Я тебе уже пятихатку приготовил!"///'
		...
		act 'Подойти':
			*clr & cla
			manna +=5
			gs 'stat'
			gs 'zz_render', '', 'pavlovo/misha/misha.jpg', iif(cumface = 0,'Пока дядя Миша курил, вы беседовали о всякой ерунде. Докурив сигарету, он попрощался с вами, и удалился.',iif(sosedknow = 0,'Дядя Миша смотрит на ваше лицо с удивлением:**\\\- Это что у тебя на лице?///**Вы '+iif(shameless['flag'] < 2,'краснеете и смущаетесь',iif(shameless['flag'] = 2,'краснеете','улыбаетесь'))+', стараясь рукой стереть сперму с лица.**\\\- Ну ну, всё понятно... -/// ухмыляется он и затушив сигарету уходит.','Дядя Миша хмыкнул, посмотрев на вас:**\\\- Ну Светик ты и шлюха...///'))
...
if $args[0] = 'smoke_boys':
	*clr & cla
	...
	if func('zz_reputation','get') = 3:
		gs 'zz_render', '', '', 'Проходя мимо вы услышали их перешептывания:**\\\- Это же Светик, местная соска! Давай ее накормим, а если получиться, то и выебем!///**Вдруг один из них подошел к вам и, вынул из штанов член, сказал:**\\\- Это тебе, ты же знаешь что надо с этим делать, соска?///'
		if dom > 0:
			act 'Послать':
				*clr & cla
				minut +=2
				dom -= 1
				gs 'stat'
				gs 'zz_render', '', 'pavlovo/home/events/screams.jpg', '\\- Пацаны вы чего вообще охуели тут? Мозги все прокурили?, -// возмутились вы. \\- **\\Быстро съебались отсюда, а то закричу, весь дом сбежится посмотреть на вас оболтусов!//**Ребята решили не испытывать судьбу и быстро удалились, кинув на прощание:**\\\- Ничего, ещё встретимся!///'
				act 'Уйти': gt'pod_ezd', 'etaj_2'
			end
		end
					...
					*clr & cla
					...
					act 'Далее':
						*clr & cla
						...
						gs 'zz_render', '', '', iif(shameless['flag'] < 3 and stat_agape < 3 and stat_vgape < 3,'Вы активно подмахиваете ему, что бы поскорее свалить, пока никто не заметил ваше падение.**','')+'Его друг стоит в стороне, наблюдает и дрочит.'
						dynamic $accview_ggsex
						act 'Далее':
							*clr & cla
							minut += 5
					...
					*clr & cla
					...
					act 'Расслабиться':
						*clr & cla
						...
						lesbian += 1
						gs 'stat'
						gs 'zz_render', '', 'pavlovo/home/milova_event/3.jpg', 'Юля говорит вам, приспутив свои брюки:**\\\- Света, я хочу, чтобы ты полизала меня...///'
						act 'Далее':
							*clr & cla
							minut += 2
							horny += 5
								...
								*clr & cla
								...
								act 'Далее':
									*clr & cla
									minut += 2
									horny += 5
									gs 'zz_render', '', 'pavlovo/home/milova_event/6.jpg', '\\\- Подожди Светик, -/// прерывает вас Юля. \\\- Смотри, что у меня с собой есть...///** Она роется в сумочке идостает из резиновый член:**\\\- Держи, это то что мне сейчас нужно больше всего!///'
									act 'Далее':
										*clr & cla
										minut += 5
										horny += 5
		...
		*clr & cla
		...
		act 'Сосать дальше':
			*clr & cla
			...
			gs 'zz_funcs', 'cum', 'lip'
			gs 'stat'
			gs 'zz_render', '', 'images/pavlovo/home/events/shulga_cum.gif', '**Вы понимали, что в любой момент может кто-то появиться, поэтому быстро довели Ваську до оргазма. Он излился вам в рот, заодно запачкав спермой и лицо, и спрятал свой член в штаны.**\\- Доволен?, -// спросили вы, пытаясь вытереть лицо.**\\\- Ага, замечательный и безотказный ротик у тебя!, -///, сделав такой своеобразный комплимент, <<$shulga_name_rnd>> удалился.'
			act 'Уйти': gt'pod_ezd', 'etaj_3'
		end
	end
end
...
if $args[0] = 'front_door':
	...
	elseif i = 2:
		*clr & cla
		...
		voyeurism += 1
		gs 'stat'
		gs 'zz_render', '', 'pavlovo/home/events/ev2.jpg', 'Вы увидели, как на лестничной клетке два пацана быстро сношали какую-то девку, судя по наряду проститутку.**Лицо разглядеть вы не смогли, так как девушка была зажата между парнями.'
		dynamic $masturb_gg
