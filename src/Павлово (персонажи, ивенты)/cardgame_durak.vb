...
$card_game_durak = {
	*clr & cla
	...
	gs 'zz_render','','','Ваша статистика игр: побед <<card_game_durak_win>>, поражений <<card_game_durak_loss>>, ничьих <<card_game_durak_draw>>, вас раздевали <<card_game_durak_nude>>'
	if playcardgame_count <= 0:'**<<$rnd_rebyata>> сказал: - "Что-то уже надоело сегодня играть в карты, давай телек посмотрим или пивка попьем."'
...
$card_game_durak_play = {
	*clr & cla
	...
	gs 'zz_render','','','<table width="100%" cellspacing="0" cellpadding="5"><td width="200" valign="top"><img src="images/zsoft/pod_ezd/shulgahome/bigroom/card/<<cgd_clothes_set>>-<<cgd_clothes_set1>>.jpg"></td><td valign="top">'
	gs 'zz_render','','','<<$cgd_clothes>>.
	<<$cgd_vasya_clothes>>.
	<<$cgd_den_clothes>>.
	<<$cgd_vitek_clothes>>.'
		if card_game_durak_clothes <=0:
			...
			gs 'zz_render','','','<b>Игра закончена, на вас не осталось одежды, вы проиграли</b>**Победил: <b><<$rnd_cgd_win_npc>></b>**Вам выполнять его желание'
		...
		if card_game_durak_npc_vasya <=0 or card_game_durak_npc_den <=0 or card_game_durak_npc_vitek <=0:
			...
			gs 'zz_render','','','<b>Игра закончена, остался голым <<$cgd_name_igrok>></b>.'
			rnd_cgd_win_select = RAND(0,100)
			if func('zz_reputation','get') < 3:
		...
		if card_game_durak_clothes >0 and card_game_durak_npc_vasya >0 and card_game_durak_npc_den >0 and card_game_durak_npc_vitek >0:
			...
			if card_game_rand <= 60:
				...
				gs 'zz_render','','','<img src="images/zsoft/pod_ezd/shulgahome/bigroom/card/card3.jpg">'
				gs 'zz_render','','','У вас на руках хорошие карты и вы без труда выигрываете кон. <b>Проиграл <<$cgd_name_npc>></b>.'
			...
			if card_game_rand > 60 and card_game_rand < 95:
				...
				gs 'zz_render','','','<img src="images/zsoft/pod_ezd/shulgahome/bigroom/card/card1.jpg">'
				gs 'zz_render','','','У вас на руках плохие карты. <b>Вы проиграли</b>.'
				act 'Раздать': dynamic $card_game_durak_play
			end
			if card_game_rand>=95:
				card_game_durak_draw +=1
				gs 'zz_render','','','<img src="images/zsoft/pod_ezd/shulgahome/bigroom/card/card1.jpg">'
				gs 'zz_render','','','У вас на руках простенькие карты, но вы так удачно вывернулись, что получилось сыграть вничью.'
				act 'Раздать': dynamic $card_game_durak_play
			end
		end
	gs 'zz_render','','','</td></tr></table>'
}
!--------------------------!
...
$card_game_rough_sex = {
	*clr & cla
	...
	if $npc['9,sex'] = 0: $npc['9,sex'] = 1 & guy += 1
	gs 'stat'
	gs 'zz_render','','common/sex/facefuck/<<rand(0,4)>>.gif',func('cardgame_durak_strings','14')
	act 'Далее':
		*clr & cla
		minut +=10
		anal += 1
		...
		horny += 10
		gs 'stat'
		gs 'zz_render','','common/sex/anal/<<rand(0,4)>>.gif',func('cardgame_durak_strings', '15')
		if analplugin = 1:
			analplugin = 0
		...
		act 'Далее':
			*clr & cla
			...
			son -= 2
			gs 'stat'
			gs 'zz_render','','common/sex/anal/<<rand(0,4)>>.gif',func('cardgame_durak_strings', '19')
			if horny >=70:
				orgasm += 1
				horny = 0
				manna += 35
	...
	*clr & cla
	...
	act 'Далее':
		*clr & cla
		minut +=15
		gs 'stat'
		gs 'zz_render','','','<center>Проигравшему пришлось выполнять ваше желание.</center>'
		...
		*clr & cla
		...
		act 'Далее':
			*clr & cla
			...
			cumfrot += 1
			gs 'stat'
			gs 'zz_render','','common/sex/facial/<<rand(0,4)>>.jpg', func('cardgame_durak_strings', '48')
			act 'Уйти': gt 'shulga_room'
		end
	end
}