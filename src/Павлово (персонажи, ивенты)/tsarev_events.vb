...
! 105 - 112 - обдумывания в своей комнате по детективной ветке
! 113 - назначена свиданка в Сити с сослуживцем Царева
! 115 - свидание прошло успешно, Светка соблазнила сослуживца
! 120 - Светка узнала от сослуживца об автомойке, дальше надо уточнить у отчима об автомойках в Павлово
! 125 - Светка размышляет об автомойках
! 127 - Светка расспросила отчима об автомойках в Павлово, внезапно оказалась только одна
! 130 - Светка устроилась на работу на автомойку
! 131-135 - Светка работает на автомойке
! 135 - беседа с Владом
! 140 - успешное завершение беседы с Владом
...
if $args[0] = 'meet_teacher':
	*clr & cla
	...
	if $npc['26,relation'] >= 80 and vnesh >= 60:
		act 'Соблазнять':
			*clr & cla
			if $npc['26,qwMain'] < 10: $npc['26,qwMain'] += 2
			gs 'npc_editor','get_npc_profile',26
			gs 'zz_render','','',func('tsarev_str',17)
			if $npc['26,qwMain'] = 10 and shameless['flag'] >= 2:
				...
				*clr & cla
				...
				act 'Оголить задницу':
					*clr & cla
					horny += 3
					gs 'npc_editor','change_rep','up',26,1
					gs 'zz_render', 'Нужна фота - девушка собирает упавшие книги с задранной юбкой','pavlovo/tsarev/school/5',func('tsarev_str',21) + func('tsarev_str',iif(cloth[1]=1,22,23)) + func('tsarev_str',24)
					act 'Дальше': gt 'zz_school'
				end
			end
...
if $args[0] = 'home_second_sex':
	...
	minut += rand(3,5)
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/home_sex/1/'+_stage+iif(_stage=10,'.gif','.webm'),func('tsarev_str',148+_stage)
...
if $args[0] = 'home_second_sex_ender':
	*clr & cla
	gs 'zz_render','','images/pavlovo/tsarev/home_sex/1/13.jpg',func('tsarev_str',190)
	$npc['26,qwMain'] = 54
	minut += rand(10,15)
	act 'Уйти': gt 'gorodok'
end
...
if $args[0] = 'teacher_choice_detective3':
	*clr & cla
	...
	$npc['26,qwMain'] = 103
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/detective/2',func('tsarev_str',208)
	gs 'zz_render','','',func('tsarev_str',iif(intel < 50,209,210))
	act 'Уйти': minut += rand(10,15) & gt 'gorodok'
end
...
if $args[0] = 'teacher_choice_detective4':
	*clr & cla
	...
	$npc['26,qwMain'] = 105
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/detective/6',func('tsarev_str',211)
	act '...': gt 'bedrPar'
end
! ветка детективная - зацепка-1 - ГГ в своей комнате размышляет-2
if $args[0] = 'teacher_choice_detective5':
	*clr & cla
	minut += rand(5,15)
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/detective/7',func('tsarev_str',107+$npc['26,qwMain'])
	$npc['26,qwMain'] += 1
	act '...': gt 'bedrPar'
end
! свидание в кафешке
if $args[0] = 'teacher_choice_detective6':
	*clr & cla
	gs 'zz_render','','pavlovo/tsarev/detective/8',func('tsarev_str',220)
	act 'Идти в кафе':
		*clr & cla
		minut += rand(5,10)
		gs 'stat'
		gs 'zz_render','','pavlovo/tsarev/detective/9',func('tsarev_str',221)
		act 'Отказаться':
			*clr & cla
			gs 'zz_render','','pavlovo/tsarev/detective/9',func('tsarev_str',222)
			$npc['26,qwMain'] = 255
			act 'Уйти': gt 'zz_park'
		end
		act 'Идти с Суворовым': gt 'tsarev_events','teacher_choice_detective7'
	end
end
! поход с Суворовым на квартиру
if $args[0] = 'teacher_choice_detective7':
	*clr & cla
	minut += rand(20,30)
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/detective/10',func('tsarev_str',223)
	act '...':
		*clr & cla
		gs 'zz_render','','pavlovo/tsarev/detective/11',func('tsarev_str',224)
		act 'Упрашивать':
			*clr & cla
			gs 'zz_render','','pavlovo/tsarev/detective/13',func('tsarev_str',225)
			act '...': gt 'gameover',9
		end
		act 'Материть':
			*clr & cla
			gs 'zz_render','','pavlovo/tsarev/detective/12',func('tsarev_str',226)
			act '...':
				*clr & cla
				gs 'zz_render','','pavlovo/tsarev/detective/12',func('tsarev_str',227)
				act 'Уйти':
					$npc['26,qwMain'] = 255
					minut += rand(10,15)
					gt 'south'
				end
				act 'Остаться': gt 'tsarev_events','teacher_choice_detective8'
			end
		end
	end
end
! поход с Суворовым на квартиру - соблазнение
if $args[0] = 'teacher_choice_detective8':
	*clr & cla
	_stage = args[1]
	minut += rand(5,10)
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/detective/<<_stage+15>>',func('tsarev_str',229+_stage)
	act iif(_stage < 6,'...','Уйти'):
		if _stage < 6:
			gt 'tsarev_events','teacher_choice_detective8',_stage+1
		else
			minut += rand(10,15)
			$npc['26,qwMain'] = 115
			$npc['26,qwMain_day'] = day
...
! Светка назначает свиданку Суворову
if $args[0] = 'teacher_choice_detective9':
	*clr & cla
	if hour >= 8 and hour <= 18:
		gs 'zz_render','','',func('tsarev_str',236)
		act 'Назначить свиданку':
			*clr & cla
			gs 'zz_render','','',func('tsarev_str',237)
			$npc['26,qwMain'] += 1
...
! второй и третий поход к Суворову на квартиру
if $args[0] = 'teacher_choice_detective10':
	*clr & cla
	gs 'zz_render','','pavlovo/tsarev/detective/30',func('tsarev_str',238)
	act 'Войти':
		*clr & cla
		gs 'zz_render','','pavlovo/tsarev/detective/31',func('tsarev_str',239)
		act '...':
			*clr & cla
			gs 'zz_render','','pavlovo/tsarev/detective/32',func('tsarev_str',240)
			act 'Сосать':
				*clr & cla
				bj += 1
				horny += rand(5,10)
				minut += rand(3,7)
				gs 'stat'
				gs 'zz_render','','pavlovo/tsarev/detective/33',func('tsarev_str',241)
				gs 'tsarev_events','teacher_choice_detective10_sex'
			end
			act 'Попросить полизать':
				*clr & cla
				...
				minut += rand(3,7)
				gs 'stat'
				gs 'zz_render','','pavlovo/tsarev/detective/34',func('tsarev_str',242)
				gs 'tsarev_events','teacher_choice_detective10_sex'
			end
		end
	end
end
if $args[0] = 'teacher_choice_detective10_sex':
	act 'Дать сзади':
		*clr & cla
		sex += 1
		horny += rand(10,20)
		minut += rand(5,10)
		gs 'stat'
		gs 'zz_render','','pavlovo/tsarev/detective/35',func('tsarev_str',243)
		act '...': gt 'tsarev_events','teacher_choice_detective10_sex_end'
	end
	act 'Запрыгнуть на Суворова':
		*clr & cla
		sex += 1
		horny += rand(10,20)
		minut += rand(5,10)
		gs 'stat'
		gs 'zz_render','','pavlovo/tsarev/detective/36',func('tsarev_str',244)
		act '...': gt 'tsarev_events','teacher_choice_detective10_sex_end'
	end
end
if $args[0] = 'teacher_choice_detective10_sex_end':
	*clr & cla
	_stage = args[1]
	if _stage = 0:
		horny = 0
	...
	minut += rand(1,3)
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/detective/<<37+_stage>>',func('tsarev_str',245+_stage)
	! после первого свидания - ГГ уходит
	! после второго свидания - ГГ может остаться и пораспрашивать
	if $npc['26,qwMain'] = 116:
		$npc['26,qwMain'] = 117
		act '<b>Уйти<b>': gt 'south'
	else
		act iif(_stage < 4,'...','<b>Уйти<b>') :
			if _stage < 4:
				gt 'tsarev_events','teacher_choice_detective10_sex_end',_stage+1
			else
				$npc['26,qwMain'] = 120
				minut += rand(5,10)
				killvar '_stage'
...
! ГГ размышляет у себя в комнате об автомойках в Павлово
if $args[0] = 'teacher_choice_detective11':
	*clr & cla
	$npc['26,qwMain'] = 125
	gs 'zz_render','','pavlovo/tsarev/detective/6',func('tsarev_str',250)
	act 'Хм...': gt 'bedrPar'
end
! ГГ расспрашивает отчима об автомойках в Павлово
if $args[0] = 'teacher_choice_detective12':
	*clr & cla
	gs 'zz_render','','pavlovo/tsarev/detective/42',func('tsarev_str',251)
	act '...':
		*clr & cla
		gs 'zz_render','','pavlovo/tsarev/detective/42',func('tsarev_str',252)
		act '...':
			*clr & cla
			gs 'zz_render','','pavlovo/tsarev/detective/42',func('tsarev_str',253)
			$npc['26,qwMain'] = 127
			$npc['26,qwMain_day'] = day
			act 'Уйти': gt 'korrPar'
		end
...
! ГГ устраивается работать на автомойку
if $args[0] = 'teacher_choice_detective13':
	*clr & cla
	$npc['26,qwMain'] = 130
	$npc['26,qwMain_day'] = day
	gs 'zz_render','','pavlovo/tsarev/detective/43',func('tsarev_str',254)
	act '<b>Уйти</b>':
		minut += rand(10,15)
		gt 'gorodok'
	end
end
! во время работы к ГГ подходит Влад
if $args[0] = 'teacher_choice_detective14':
	*clr & cla
	minut += rand(5,10)
	gs 'stat'
	gs 'zz_render','','pavlovo/tsarev/detective/44',func('tsarev_str',255)
	act 'Послать':
		*clr & cla
		! закрываем мойку и ветку учителя
		$npc['0,carwash'] = 0
		$npc['26,qwMain'] = 255
		gs 'zz_render','','pavlovo/tsarev/detective/45',func('tsarev_str',256)
		act '<b>Уйти</b>':
			minut += rand(10,15)
			gt 'gorodok'
		end
	...
	act 'Согласиться':
		*clr & cla
		...
		$npc['26,qwMain_day'] = day
		gs 'stat'
		gs 'zz_render','','pavlovo/tsarev/detective/46',func('tsarev_str',257)
		act 'Пошли докажу':
			*clr & cla
			minut += rand(5,10)
			gs 'stat'
			gs 'zz_render','','pavlovo/tsarev/detective/47',func('tsarev_str',258)
			act '...':
				*clr & cla
				gs 'zz_render','','pavlovo/tsarev/detective/46',func('tsarev_str',259)
				act '...':
					*clr & cla
					$npc['26,qwMain'] = 140
					gs 'zz_render','','pavlovo/tsarev/detective/46',func('tsarev_str',260)
					act '<b>Уйти</b>':
						minut += rand(10,15)
						gt 'gorodok'
					end
		...
		act 'Убеждать':
			*clr & cla
			gs 'zz_render','','pavlovo/tsarev/detective/46',func('tsarev_str',261)
			act '<b>Уйти</b>':
				minut += rand(10,15)
				gt 'gorodok'
			end
