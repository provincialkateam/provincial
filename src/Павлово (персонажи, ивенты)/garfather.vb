...
if TorgVokzalTimes > 0 and YouCanGar = 0 and $father['location'] = 'gargazel':
	act 'Просить разрешения хранить в гараже товар':
		...
		if $npc['35,relation'] >= 80 or father['touched'] = 1:
			...
			gs 'zz_render','','','Отчим отрицательно покачал головой: \\\- Нет Света. У меня и так там мало места.///'
		end
...
if horny >= 70 and $npc['35,qwOtchim'] >= 5 and father['dadqw_day'] ! day and $npc['35,relation'] >= 80:
	act 'Соблазнить':
		*clr & cla
		...
		gs 'zz_render','','images/nigma/gor/father/gar/father.gar.pre'+rand(0,5),'Вы подошли к Газельке, и взяли губку. Вы наклонились, чтобы помыть машину, выставив попку на обозрение отчиму.
			...
			*clr & cla
			...
			act 'В рот':
				*clr & cla
				...
				minut += 5
				gs 'stat'
				gs 'zz_render','','images/nigma/gor/father/gar/father.gar.0_'+rand(0,4),'Отчим открыл дверь Газельки, и сел на пассажирское сидение. Он притянул вашу голову к себе, и насадил ваш рот на член.
				...
				act 'Вынуть изо рта':
					*clr & cla
					...
					minut += 10
					gs 'stat'
					gs 'zz_render','','common/sex/bj_cum/'+rand(0,4),'Вы вынули член изо рта и струя спермы тут же хлестнула вам по лицу. Вы зажмурили глаза боясь, что сперма попадет вам в глаза и почувствовали как еще одна струя горячей кончи приземлилась вам на лицо.'
					act '<B>Отойти</B>': gt 'gargazel'
				end
			end
			act 'Дать':
				*clr & cla
				...
				dick = 17
				$boy = 'Отчим'
				gs 'zz_render','','images/nigma/gor/father/gar/father.gar.1_'+rand(0,2),'Отчим поднял вас с колен, и развернул лицом к Газельке. Вы опперлись руками об машину, подставив ему попку.'
				...
				act 'Дальше':
					*clr & cla
					...
					gs 'stat'
					gs 'zz_render','','images/nigma/gor/father/gar/father.gar.cum1_0','Наконец, вы чувствуете как член начинает пульсировать внутри вас. Отчим делает еще несколько толчков, а затем наваливаете на вас всем телом, заливая вашу попку своей спермой.'
