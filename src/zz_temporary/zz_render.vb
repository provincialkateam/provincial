...
$ren_img = $args[1]
!-- font-size --!
exec('js:setFontSize('+(16+$settings['font_size']*2)+');')
!--
...
if len($ren_img) > 4:
	...
	if $_img_ext = 'webm':
		$ren_str += '<video class="vplayer" preload="metadata" autoplay loop><source src="' + $ren_img + '" type=''video/ogg; codecs="theora, vorbis"''></video>'
	else
		$ren_str += '<img src="' + $ren_img + $_img_ext + '">'
	end
	$ren_str += '</div>'+ iif($settings['image_render_debug'] = 1,'<span>'+ $ren_img + iif($_img_ext='webm','',$_img_ext)  + '</span>','')
end
! text row
if len($args[2]) > 0:
