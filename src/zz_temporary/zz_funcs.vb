...
if $args[0] = 'sport':
	...
	gs 'zz_clothing2','decrease_condition',func('zz_clothing2','decrease_condition_level')
end
! Кожа
...
if $args[0] = 'money':
	...
	elseif money + karta >= args[1]:
		...
		result = 0
		gs 'zz_funcs','message','images/common/icons/money.png','<red>Денег нет, но вы держитесь!</red>'
	end
	if result = 1: gs 'stat'
end
!---
...
if $args[0] = 'payment':
	...
	if _sum_money + _sum_karta = 0:
		gs 'zz_funcs','message','images/common/icons/money.png','<red>У вас не недостаточно денег.</red>'
...
if $args[0] = 'set_statuses':
	...
	if drugs['status'] > 0:
		if drugs['status'] > 75:
			if drugs['effect'] = 0:
				$_statuses[17] += 'Вы под кайфом и чувствуете себя окрыленно.<br>'
			else
				$_statuses[17] += 'Вы под кайфом, но ощущения не очень.<br>'
			end
		elseif (drugs['status'] > 50 and drugs['status'] <= 75) or (drugs['status'] > 0 and drugs['status'] <= 25):
			$_statuses[17] += 'Вы чувствуете себя нехорошо после приема наркотиков.<br>'
		elseif drugs['status'] > 25 and drugs['status'] <= 50:
			$_statuses[17] += 'У вас очень плохое самочуствие после приема наркотиков.<br>'
		end
	else
		$_statuses[17] = ''
	end
	...
	if cafe['0,work'] = 1 and week mod 2 = 0 and hour >= 8 and hour <= 16:
		$_statuses[21] += 'В 16 часов вам надо быть на работе в кафе DelParco.'
	end
	!--
	$_statuses[22] = ''
	if svidanieA = 1:
