...
if $args[0] = 'read_porn':
	act 'Листать порножурнал':
		*clr & cla
		...
		if pornMagazine = 1:
			gs 'zz_render','','','Скукота, этот журнал вы уже наизусть запомнили. Кажется, пора купить новый.'
		else
			pornMagazine -= 1
			horny += 10
			minut += 5
			gs 'stat'
			gs 'zz_render','','','Вы листаете порножурнал, читая истории и разглядывая картинки. Потихоньку возбуждение начинает охватывать вас.'
		end
		act 'Выйти':gt $curloc
	end
end
...
if $args[0] = 'crossing':
	...
	minut += 30 * _training
	gs 'zz_funcs','sport',_training*4
	if _training = 1:
		gs 'zz_render','','common/sport/cross/cross_park'+iif(snow <= 0,'','_winter'),'Вы бегаете около получаса, не давая себе заплыть жирком.'
	else
		gs 'zz_render','','common/sport/cross/cross'+iif(snow <= 0,'','_winter'),'Вы в течении часа бегаете стадиону, развивая скорость и навыки бега.'
	end
	killvar '_training'
	gs 'stat'
end
...
if $args[0] = 'home_workout':
	...
	if func('zz_drugs','block') = 1: exit
	$zz_str = ''
	if obruch > 0: $zz_str += '<a href="exec: gs''zz_common'',''sitrobrd''">крутить обруч</a>, '
	if skak > 0: $zz_str += '<a href="exec: gs''zz_common'',''sitrskakd''">прыжки на скакалке</a>, '
	$zz_str += '<a href="exec: gs''zz_common'',''sitrpressd''">упражнения на пресс</a>, '
	$zz_str += '<a href="exec: gs''zz_common'',''sitrpushd''">отжимания</a>, '
	if workout < 3: gs 'zz_render','','','В тесной комнатке однако есть место, где можно поделать разные спортивные упражнения, такие как ' + mid($zz_str, 1, len($zz_str) - 2)
	killvar '$zz_str'
end
if $args[0] = 'sitrobrd':
	*clr & cla
...
if $args[0] = 'get_clothing':
	if cloth[0] = 0 and cloth[1] = 1:
		$result = 'images/common/clothing/01.jpg'
	else
		$result = func('zz_clothing2','get_image')
	end
end
! киска
if $args[0] = 'get_pussy':
...
if $args[0] = 'error':
	*clr & cla
	...
	act 'Далее >>':
		! закрываем ветку банды, если использована аварийка для побега
		if banda['stage'] > 0 and banda['stage'] < 20: banda['stage'] = 255
		!---
