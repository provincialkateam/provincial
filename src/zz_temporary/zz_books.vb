...
if $args[0] = 'main':
	*clr & cla
	...
	:loop_shelf
	if $books['<<i>>,exists'] = 1 and i ! reading['sister_book'] and i ! reading['in_bag']:
		...
		$_str += '<td width=150 align=center><a href="exec: reading[''in_bag''] = <<i>> & gt ''zz_books'', ''main''">Положить в сумочку</a></td>'
		$_str += '</tr>'
	end
	i += 1
...
if $args[0] = 'read':
	*clr & cla
	_book = args[1]
	$_str = '<center>Прочтено ' + iif($books['<<_book>>,page']/10 = 0,$books['<<_book>>,page'] * 10,($books['<<_book>>,page'] mod 10) * 10) + ' из 400 страниц</center>'
...
if $args[0] = 'read_procedure':
	gs 'zz_funcs','message','', '<green>Вы прочли несколько страниц</green>'
	$books['<<_book>>,page'] += 1
	! bonuses
	reading['read_per_day'] += 1
	minut += rand(50,70)
	if reading['read_per_day'] => 2:
		blizoruk += 1
	end
	if nerdism > 0:
		nerdism += 20
...
if $args[0] = 'erotic_enable':
	...
	*clr & cla
	$_str = '\\- Привет, сестренка, что читаешь?// - спросили вы Аньку.<br>'
	if (sisterknowslut + SisterKnowMastr) > 0:
		$_str += '\\\- Да ничего такого, чего б ты не знала!/// - хихикнула она. - \\\Эротику!///<br>\\- Дай и мне почитать// - не угомонивались вы, пытаясь забрать у Аньки книгу.<br>\\\- Отстань, завтра возьмешь, я уже дочитываю.<br>- Не понравится эта - сходи на рынок - за центральным входом второй ряд налево и в самый конец - там лоток с порнухой и эротикой. Вот только порножурналы я не советовала б тащить домой, а то от мамки влетит по первое число.///'
	else
		$_str += '\\\- Да так, любовный роман.../// - ответила Анька, краснея и пытаясь спрятать книгу.<br>Быстро сообразив, что сестренка от вас что-то скрывает, вы ловко повалили ее на кровать и отобрали книгу - \\Эммануэль Арсан. «Эммануэль». Ничего себе, Анька, это же порнуха!// - как на духу произнесли вы, прочитав название книги.<br>\\\Никакая это не порнуха, а эротика. Поинтереснее твоих любовных романов с розовыми мечтами./// - отсекла Анька. - \\\Почитай как-нибудь, тебе понравится. Я оставлю на полке.<br>- Не понравится эта - сходи на рынок - за центральным входом второй ряд налево и в самый конец - там лоток с пор... эротикой.///'
	end
...
if $args[0] = 'erotic_block':
	...
	*clr
	...
	if $npc['37,relation'] < 40:
		...
		$_str += '\\\- Это что еще такое? Порнуху в дом принесла, блядища ты этакая?! Нет, что б полезное что почитать!/// - кричала мать, тряся книгой у ваc перед лицом.<br>Ругаясь, мать собрала все книги, выдавшиейся ей порнографическими и ушла, громко хлопнув дверью.'
		...
		$_str += '\\\- Это что еще такое, Света? Порнуху в дом принесла?/// - строго спросила мать, и, явно ожидая ответа, ткнула вам под нос книгу. Но не успели вы открыть рот, как она продолжила:<br>\\\- И не смей меня перебивать! Куда ж мир катится, в наше время-то...///'
		gs 'zz_render', '', '', $_str
		minut += 5
		cla
		act 'Молчать':
		...
		act 'Перебить':
			...
			gs 'zz_render', '', '', '\\- Да в учебнике биологии порнографии больше, чем этих книгах, это эротика//, - неожиданно, как для себя, так и для матери, выпалили вы. Мать, не сообразив, что ответить, лишь отдала вам книгу и сказав - \\\уж лучше так с взрослой жизнью знакомится, чем с непонятными особями где-то в подворотье...///, ушла.'
			act '...': gt 'bedrPar'
		end
	end
	killvar '$_str'
...
if $args[0] = 'buy':
	*clr
	local $message = func('zz_books','get_name', args[1])
	gs 'zz_funcs','message','', '<green>Вы купили книгу <<$message>></green>'
...
if $args[0] = 'read_on_walk':
	*clr & cla
	...
	if reading['read_per_day'] >= iif(nerdism = 0,3,5):
		...
		_book = i
		gs 'zz_books', 'read_procedure'
		$_str = func('zz_books','get_name',i)+'<br>Прочтено ' + iif($books['<<i>>,page']/10 = 0,$books['<<i>>,page'] * 10,($books['<<i>>,page'] mod 10) * 10) + ' из 400 страниц.<br>'
		...
		if $loc ! 'zz_park':
			...
			if sunWeather <= 2:
				tan += iif(cloth[0] > 0,1,2)
			else
				tan += iif(cloth[0] > 0,2,4)
			end
		end
	...
	elseif $loc = 'gadfield':
		...
		gs 'zz_render', '','common/beach/reading_<<$loc>>'+iif(cloth[0] = 0,'_nude'+rand(1,3),rand(1,3))+'.jpg',$_str
	end
	killvar '$_str'
end
