	...
	*clr & cla
	...
	if hour < 18:
		...
		if (hour = 11 and minut > 45) or (hour = 12 and minut < 15):
			! ---
			! проверка количества прогулов - для запуска ивентов с отработкой у директора
			gs 'absent_events','check'
			! ---
			! птушники пришли в гости
			if rand(0,5) = 1 and $npc['0,park_slut_counter'] >= 5 and $npc['0,park_slut_event'] ! day and shameless['flag'] >= 3: gt 'park_events','slut_ev2'
			! ---
...
if $args[0] = 'homework':
	if (hour < 9 or hour >= 15) and school['certificate'] = 0 and school['block'] < 3 and school['vacation'] = 0 and school['homework'] = 1:
		act 'Учить уроки':
			*clr & cla
			! brother_help_qw
			if $loc = 'bedrPar' and hour = 21 and week < 5 and ($npc['39,qwPornoFootball'] >= 16 or $npc['39,sex'] > 4) and $npc['39,relation'] > 80 and func('zz_clothing2','is_skirt') = 1 and brother_help_qw < 4 and brother_help_qw_day ! day: gt 'brother_events','brother_help_qw'
...
if $args[0] = 'mini_break_class':
	...
	$_mini[8] = 'Наташка Белова, сидя за партой и приспустив колготки, что-то пишет на голом бедре, то и дело глядя в листок, который держит в руке. "Теперь понятно - как она всегда контрольные на "отлично" сдаёт, заучка хитрожопая!" - ухмыляетесь вы. Надо взять её метод на вооружение!"'
	i = rand(0,arrsize('$_mini')-1)
