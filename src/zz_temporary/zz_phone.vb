if $args[0] = '' or $args[0] = 'menu':
	*clr & cla
	...
	$_str += '<td><img src="'+$_img+'buttons/maps.png"></td>'
...
if $args[0] = 'box':
	...
	$_box += '<div class="phone_box_name">'+iif(_box_id=900,$nameBoyfrend[numnpcboy],$_contact['<<_box_id>>,name'])+'</div>'
...
if $args[0] = 'box_ext':
	...
	$_box = '<center><div class="phone_box">'
	$_box += '<div class="phone_box_content" style="left:'+iif(_box_padding = 1,60,iif(_box_padding=-1,40,20))+'px;">'
	$_box += '<div class="phone_box_image"><img src="'+iif(_box_id = -1,func('zz_common','get_face'),'images/common/npc/<<_box_id>>.jpg')+'"></div>'
	$_box += '<div class="phone_box_name">'+$npc['<<_box_id>>,name']+'</div>'
	$_box += '<div class="phone_box_text">'+$_box_text+'</div>'
	$_box += '</div></div></center>'
	gs 'zz_render','','',$_box
	killvar '$_box'
...
if $args[0] = 'dialogues':
	...
	if _call_npc = 37:
		...
		$_dialogue['0,ans'] = iif($npc['37,relation'] < 40,'<font color=red>Что на этот раз натворила?</font>','Света, что-то случилось?')
		...
		if $npc['37,relation'] < 40:
			...
			elseif lcase($sister['location']) = 'gadhouse':
				$_dialogue['1,ans'] = 'Света, с тобой все в порядке? В дедушки с бабушкой в деревне мы.'
			else
	...
	if _call_npc = 35:
		...
		$_dialogue['0,ans'] = iif($npc['35,relation'] < 40,'<font color=red>Что на этот раз натворила?</font>','Света, что-то случилось?')
		$_dialogue['1,que'] = 'Да ничего, хотела спросить где ты?'
		if $npc['35,relation'] < 40:
			...
			elseif lcase($father['location']) = 'gadhouse':
				$_dialogue['1,ans'] = 'Света, с тобой все в порядке? В Гадюкино мы.'
			else
		...
		if $father['in_garage'] = 'gargazel' and father['in_garage'] = 1 and father['hadsex'] = 1:
			...
			$_dialogue['2,ans'] = 'Ох, Света, <a href="exec:minut+=10 & gs''zz_phone'',''clearing'' & gt ''gargazel''">приходи</a>, порадуй папочку.'
		end
	end
	!Мишка
	if _call_npc = 48:
		if $npc['38,qwKolka'] = 8:
				...
				$_dialogue['1,ans'] = 'А, Света, не узнал прости!'
		...
		elseif $npc['38,qwKolka'] = 9:
			$_dialogue['0,que'] = 'Алло, Миша, Света на линии!'
			$_dialogue['0,ans'] = 'Привет, Света, как житуха, есть дело ко мне?'
	...
	if _call_npc = arrsize('$_contact')/2 - 2:
		...
		$_dialogue['0,ans'] = 'Здравствуйте, недалеко от Вас есть машина, будет ' + iif($control_point='north' or $control_point='south' or $control_point='center' or $control_point='gorodok','с минуты на минуту.','минут через 20.')
...
if $args[0] = 'taxi':
	*clr & cla
	...
	_taxi_time = hour*60 + minut + iif($control_point='north' or $control_point='south' or $control_point='center' or $control_point='gorodok',2,20)
	! суточный переход
...
if $args[0] = 'boyfriend_call':
	...
	gs 'zz_phone','list'
	! boyfriend
	gs 'zz_phone','box',900,'Привет, Света, давай сходим куда-нибудь погулять.',0
	wait 700
...
!---
! входящий звонок от Тани
if $args[0] = 'tanya_call':
	gt 'tanya_events','tanya_call'
end
if $args[0] = 'tanya_call_decline':
	gt 'tanya_events','tanya_call_decline'
end
!---