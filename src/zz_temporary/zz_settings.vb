...
if $args[0] = 'apply_general':
	...
	$settings['font_size'] = $_tmp['font_size']
...
if $args[0] = 'tab0':
	...
	if $_tmp['font_size'] = '': $_tmp['font_size'] = $settings['font_size']
	...
	$_str += '<tr bgcolor=#f3f4ee><td>Автосохранение во время сна<br><font size=-1 color=red><i>(Внимание: хранится в Слот 20)</i></font></td><td>Включено</td><td colspan=2><a href="exec:gs''zz_settings'',''set_setting'',''autosave''">'+iif($_tmp['autosave']=0,'Выключено','Включено')+'</a></td></tr>'
	...
	$_str += '<tr bgcolor=#f3f4ee><td>Размер шрифта<br><font size=-1><i>(16-26px)</i></font></td><td>16</td><td colspan=2><a href="exec:gs''zz_settings'',''set_setting_ext'',''font_size'',5">'+(16+$_tmp['font_size']*2)+'</a></td></tr>'
	...
	!
	$_str += '<tr bgcolor=#f3f4ee><td>Language(язык)</td><td colspan=2><a href="exec:gs''zz_settings'',''set_language''">'+iif($settings['language']='','Русский','English')+'</a></td></tr>'
	$_str += '</table></center>'
	gs 'zz_render','','',$_str
	killvar '$_str'
end
...
if $args[0] = 'set_language':
	!$settings['language'] = '' для ру
...
! настройка на 0-max значений
! args[2] - максимальное значение диапазона
if $args[0] = 'set_setting_ext':
	_ex_setting = val($_tmp[$args[1]])
	_ex_setting_max = args[2]
	if _ex_setting >= _ex_setting_max:
		$_tmp[$args[1]] = 0
	else
...
if $args[0] = 'tab1':
	...
	if $_tmp['background_music'] = '': $_tmp['background_music'] = $settings['background_music']
	if $_tmp['mini_events'] = '': $_tmp['mini_events'] = $settings['mini_events']
	if $_tmp['show_map'] = '': $_tmp['show_map'] = $settings['show_map']
	if $_tmp['payment_mode'] = '': $_tmp['payment_mode'] = $settings['payment_mode']
	!--
	...
	$_str += '<tr bgcolor=#f3f4ee><td>Телосложение</td><td>Обычное</td><td><a href="exec:gs''zz_settings'',''set_setting_ext'',''bodytype'',2">'+func('zz_funcs','get_item_string','Худощавое,Обычное,Пышка',val($_tmp['bodytype']))+'</a></td></tr>'
	...
	$_str += '<tr bgcolor=#f3f4ee><td>Грудь</td><td>Средняя</td><td><a href="exec:gs''zz_settings'',''set_setting_ext'',''body_tits'',2">'+func('zz_funcs','get_item_string','Маленькая,Средняя,Большая',val($_tmp['body_tits']))+'</a></td></tr>'
	$_str += '<tr bgcolor=#f3f4ee><td colspan=3><b>Геймплей</b></td></tr>'
	$_str += '<tr bgcolor=#f3f4ee><td>Отображение карты</td><td>Свертнутая</td><td><a href="exec:gs''zz_settings'',''set_setting'',''show_map''">'+iif($_tmp['show_map']=0,'Свернутая','Развернутая')+'</a></td></tr>'
	$_str += '<tr bgcolor=#f3f4ee><td>Мини-ивенты</td><td>Включены</td><td><a href="exec:gs''zz_settings'',''set_setting'',''mini_events''">'+iif($_tmp['mini_events']=0,'Выключены','Включены')+'</a></td></tr>'
	...
	$_str += '<tr bgcolor=#f3f4ee><td>Схема списания средств где доступна одновременная<br>оплата наличными деньгами и по пластиковой карте</td><td>Наличка ИЛИ Пластик</td><td><a href="exec:gs''zz_settings'',''set_setting'',''payment_mode''">'+iif($_tmp['payment_mode']=0,'Наличка ИЛИ Пластик','Наличка И Пластик')+'</a></td></tr>'
	$_str += '<tr bgcolor=#f3f4ee><td colspan=3><b>Фоновая музыка</b></td></tr>'
	$_str += '<tr bgcolor=#f3f4ee><td>Громкость фоновой музыки<br><font size=-1><i>(0 - трек не проигрывается)</i></font></td><td>0</td><td><a href="exec:gs''zz_settings'',''set_setting_ext'',''background_music'',10">'+($_tmp['background_music']*10)+'</a></td></tr>'
	$_str += '</table></center>'
	gs 'zz_render','','',$_str
	killvar '$_str'
end
if $args[0] = 'apply_preferences':
	...
	$settings['background_music'] = $_tmp['background_music']
	$settings['autotampon'] = $_tmp['autotampon']
	$settings['auto_wipes'] = $_tmp['auto_wipes']
	$settings['mini_events'] = $_tmp['mini_events']
	$settings['show_map'] = $_tmp['show_map']
	$settings['payment_mode'] = $_tmp['payment_mode']
	gt 'zz_settings','tabs',_selected_page
end
! modding page
if $args[0] = 'tab2':
