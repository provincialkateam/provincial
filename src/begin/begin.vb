if $args[0] = '' or $args[0] = 'main':
	...
	$_str += '<td width=15></td>'
	$_str += '<td>'+func('begin','box','dancer','Творческая','Навык танцев<br>Навык вокала<br>Причёска')+'</td>'
	$_str += '</tr>'
	...
	$_str += '<td width=15></td>'
	$_str += '<td>'+func('begin','box','playgirl','Пацанка','Бонус в отношения<br>Бонус к доминантности<br>Цветные линзы')+'</td>'
	!$_str += '<td width=15></td>'
	!$_str += '<td>'+func('begin','box','enrollee','Абитуриент','18 лет**Школьный аттестат**Старт с города**')+'</td>'
	$_str += '<td width=15></td>'
	$_str += '<td>'+func('begin','box','pornhub','Pornhub','Видео<br>Много видео<br>Очень много видео')+'</td>'
	$_str += '</tr>'
	$_str += '</table>'
	*pl '<center><font size=8 color=maroon>Выберите персонажа:</font>'+$_str+'</center>'
	killvar '$_str'
	exit
end
if $args[0] = 'box':
	...
	if $args[1] = 'pornhub':
		$result += '<a href="http://pornhub.com">'+$args[2]+'</a>'
	else
...
if $args[0] = 'start_date':
	...
	$_str += '</tr>'
	*pl '<center><font size=8 color=maroon>Выберите время и место начала игры:</font>'+$_str+'</center>'
	killvar '$_str'
	exit
end
if $args[0] = 'box2':
...
if $args[0] = 'hero':
	...
	sergebazarkuh_count = 5
	!---
	condomProver = 3
	!Для решения проблем 0-го дня
	apteka_init = -1
	...
	gs 'zz_clothing2','add_to_wardrobe',rand(10,29)
	gs 'zz_clothing2','add_to_wardrobe',rand(30,49)
	cloth[0] = 3
	!if $_start ! 'enrollee':
	...
	killvar '$_hero_type'
	if $_start = 'start2':
		$control_point = 'gadukino'
		killvar '$_start'
		gt 'Gaddvor'
	else
		$control_point = 'gorodok'
		x_end = 7
		x = 7
		y_end = 8
		y = 8
