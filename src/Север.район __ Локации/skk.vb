...
if $args[0] = '' or $args[0] = 'main':
	*clr & cla
	...
	act '<b>В город</b>': minut += rand(5,10) & gt 'north'
...
if $args[0] = 'sport':
	*clr & cla
	...
	act 'Тренажерный зал': gt 'skk_gym','holl'
	if $npc['0,karate_day'] ! day and hour >= 12 and hour <= 19 and func('zz_drugs','block') = 0: act 'Секция карате': minut += 5 & gt 'skk_events',iif($npc['0,karate'] = 0,'karate_init','karate_training')
	if $npc['0,karate_count'] > 0: act 'В душевую': minut += 5 & gt 'zz_bathroom','init','skk','sport'
end
! второй этаж
if $args[0] = 'second_floor':
	*clr & cla
	...
	if hour >= 10 and hour <= 18: act 'Кинотеатр "ГЭС"': gt 'cinema','init','skk','second_floor',1
end
! третий этаж
if $args[0] = 'third_floor':
	*clr & cla
	...
	act 'Контейнерные перевозки "Логистика"': gt 'logist'
	act 'На второй этаж': gt 'skk','second_floor'
