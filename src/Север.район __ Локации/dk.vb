if $args[0] = 'start' or $args[0] = '':
	*clr & cla
	...
	gs 'zz_render', 'Дом культуры имени Ленина','city/north/dk/dk', func('dk_strings', 'local_str1')
	if hour >= 9 and hour <= 20: act 'Идти в дом культуры': gt 'dk', 'hall'
	if avtoskul >= 1 and avtoskul < 11 and skulday ! day and week >= 6 and money >= 1000 and func('zz_drugs','block') = 0:
		act 'Учиться на права за отдельную плату (1000 руб 2 часа)':
			*clr & cla
			...
			avtoskul += 1
			minut += 120
			gs 'zz_render', '', 'city/north/dk/hall', func('dk_strings', 'local_str2')
			act 'Уйти': gt 'dk'
		end
	end
	act 'Уйти': gt 'north'
...
if $args[0] = 'hall':
	*clr & cla
	...
	! --
	gs 'zz_render', 'Дом культуры','city/north/dk/hall', func('dk_strings','local_str3')
	if money >= 1000:
	...
	if hour >= 9 and hour <= 20 and week < 6 and func('zz_drugs','block') = 0:
		if age >= 18 and spravka_prava = 1 and prava = 0:
			act 'Поменять справку на права':
				*clr & cla
				...
				prava = 1
				minut += 30
				gs 'zz_render','','city/north/dk/hall','Вы отдаёте сотруднику ГИБДД справку об успешной сдаче экзамена на права и говорите что вам уже исполнилось 18 лет. Сотрудник ищет вас в базе данных и кивает. Вас фотографируют и выдают готовые права. Теперь вы можете управлять автомобилем.'
				act 'Уйти':gt 'dk','hall'
			end
		end
		if avtoskul = 0 and money >= 30000 and prava = 0 and spravka_prava = 0:
			act 'Оплатить обучение на права (30 тыс.руб)':
				*clr & cla
				money -= 30000
				avtoskul = 1
				gs 'zz_render','','city/north/dk/hall', func('dk_strings', 'local_str27')
				act 'Уйти':gt 'dk','hall'
			end
		end
		if secrDiplom = 0 and sekrObu = 0 and money >= 15000:
			act 'Оплатить обучение на секретаря (15 тыс.руб)':
				*clr & cla
				money -= 15000
				sekrObu = 1
				gs 'zz_render','','city/north/dk/hall', func('dk_strings', 'local_str28')
				act 'Уйти':gt 'dk','hall'
			end
		end
		if sekrObu > 0 and sekrObu < 11 and secrday ! day:
			act 'Учиться на секретаря (2 часа)':
				*clr & cla
				...
				sekrObu += 1
				minut += 120
				gs 'zz_render','','pic/kurs', func('dk_strings', 'local_str29')
		...
		elseif sekrObu >= 11 and secrday ! day:
			act 'Получить диплом секретаря':
				*clr & cla
				...
				minut += 15
				secrDiplom = 1
				gs 'zz_render','','pic/kurs', func('dk_strings', 'local_str30')
				act 'Уйти':gt 'dk','hall'
			end
		end
		if avtoskul >= 1 and avtoskul < 11 and skulday ! day:
			act 'Учиться на права (2 часа)':
				*clr & cla
				...
				avtoskul += 1
				minut += 120
				gs 'zz_render','','pic/dk2', func('dk_strings', 'local_str31')
				act 'Уйти': gt 'dk','hall'
			end
		elseif avtoskul >= 11 and skulday ! day:
			act 'Сдать на права 2 часа':
				*clr & cla
				...
				avtoskul = 0
				minut += 120
				gs 'zz_render','','pic/dk3','Вы ездили с сотрудником ГИБДД в течение 2х часов по городу, он остался удовлетворен и даже не нашел к чему придраться.'
				act 'Уйти': gt 'dk','hall'
				if age >= 18:
		...
		if money >= 1000 and masash < 10 and masashshoolday ! day:
			act 'Учиться массажу':
				*clr & cla
				...
				minut += 120
				gs 'stat'
				gs 'zz_render','','city/north/dk/hall', func('dk_strings', 'local_str32')
...
if $args[0] = 'song_club':
	*clr & cla
	minut += 5
	gs 'stat'
	gs 'zz_render', 'Дом культуры', 'city/north/dk/hall'
	! --- ветка Сони ---
	if hour >= 10 and hour <= 19 and $npc['25,qwSonya'] = 85 and $npc['25,qwSonya_day'] ! day: act 'Писать демо': gt 'sonya','record_demo'
	if (hour = 18 or hour = 19) and $npc['25,qwSonya'] > 0:
		...
		if $npc['25,qwSonya'] = 80 and $npc['25,qwSonya_day'] ! day: act 'Спросить руководителя про демо': gt 'sonya','talk_about_demo_with_boss'
			...
			if RAND(0,10) >= 8:
				...
				act 'Встать на колени и взять в рот':
					*clr & cla
					...
					minut += 10
					gs 'stat'
					gs 'zz_render', '', 'common/sex/bj/<<rand(0,4)>>.gif', func('dk_strings', 'local_str7')
					act 'Подставить рот':
						*clr & cla
						horny += 10
						minut += 10
						...
						cumlip += 1
						gs 'stat'
						gs 'zz_render', '', 'common/sex/bjcum/<<rand(0,4)>>.gif', func('dk_strings', 'local_str8')
						act 'Уйти':gt 'dk','hall'
					end
				end
			else
	...
	elseif money >= 500 and vokalday ! day and alexeySex > 0 and alexeyVopros = 0:
		act 'Идти на уроки вокала (500 рублей 1 час)':
			*clr & cla
			...
			vokalday = day
			gs 'stat'
			gs 'zz_render','','city/north/dk/sound', func('dk_strings', 'local_str11')
			act 'Давай забудем, что между нами что-то было':
				*clr & cla
				money -= 500
				alexeyVopros = 1
				gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str12')
				act 'Уйти':gt 'dk','hall'
			end
			act 'Если не будешь так напиваться, то мы могли бы попробовать еще раз':
				*clr & cla
				alexeyVopros = 2
				gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str13')
				act 'Уйти':gt 'dk','hall'
			end
		end
	elseif money >= 500 and vokalday ! day and alexeySex = 0:
		act 'Идти на уроки вокала (500 рублей 1 час)':
			*clr & cla
			...
			gs 'stat'
			if alexeyQW = 1:
				gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str14')
			elseif alexeyQW >= 2 and alexeyQW < 10:
				horny += 10
				gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str15')
			elseif alexeyQW >= 10 and alexeyQW < 15:
				horny += 10
				gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str16')
			elseif alexeyQW >= 15 and alexeyQW < 20:
				horny += 20
				gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str17')
			elseif alexeyQW >= 20:
				!Ну типа как повезет
				if rand(100) <= 5:
				!вероятность наверно 5%
					gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str18')
				else
					gs 'zz_render', '', 'city/north/dk/sound', func('dk_strings', 'local_str19')
						...
						*clr & cla
						...
						act 'Идти к Алексею домой':
							*clr & cla
							...
							minut += 120
							gs 'stat'
							gs 'zz_render','','city/north/dk/sound', func('dk_strings', 'local_str21')
							act 'Встать на колени и взять в рот':
								*clr & cla
								guy += 1
								alexeySex = 1
								...
								minut += 10
								gs 'stat'
								gs 'zz_render', '', 'common/sex/bj/<<rand(0,4)>>.gif', func('dk_strings', 'local_str22')
								act 'Раздеться':
									*clr & cla
									minut += 10
									$boy = 'Алексей'
									...
									pose = 1
									gs 'stat'
									gs 'zz_render', '', 'qwest/alexey/sex', func('dk_strings', 'local_str23')
									...
									act 'Стонать':
										*clr & cla
										...
										pose = 1
										gs 'stat'
										gs 'zz_render','','qwest/alexey/anal'
