if $ARGS[0] = 'start':
	*clr & cla
	...
	if harakBoyA = 2: 'Света, может пригласишь меня к себе на чашечку кофе?'
...
if $ARGS[0] = 'minet':
	*clr
	...
	gs'brosok'
	gs 'stat'
	gs 'zz_render', '','common/sex/bj/<<rand(0,4)>>.gif', func('sexm_strings', 'local_str39')
...
if $ARGS[0] = 'doggy':
	...
	gs 'zz_dynamic_sex', 'vaginal', 'dick'
	gs 'zz_dynamic_sex', 'sex_cum'
	act 'Поправить одежду и уйти': gt 'south'
end
if $ARGS[0] = 'blow':
	minut += 20
	gs 'stat'
	...
	act 'Взять в рот':
		*clr & cla
		...
		gs 'brosok'
		gs 'stat'
		gs 'zz_render', '','common/sex/bj/<<rand(0,4)>>.gif', func('sexm_strings', 'local_str76')
		gs 'zz_dynamic_sex', 'bj'
		act 'Продолжать сосать':
			*clr & cla
			swallow += 1
	...
	act 'Подрочить ему':
		*clr & cla
		...
		bfA -= iif(harakBoyA = 0,1,harakBoyA*5)
		act 'Вытереть руки и уйти': gt 'south'
	end
end
if $ARGS[0] = 'gangrape':
