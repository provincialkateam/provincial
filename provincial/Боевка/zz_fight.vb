...
elseif _finish = 3:
	health -= 20
    $_str += '<br>Удар получился точным и сильным, но <<$enemy[_enemy]>> тоже не промах. Обмениваясь ударами, вы таки посадили '+iif(_enemy>=4,'ее','его')+' на жопу, но в подарок после драки вам остались разбитый нос и фингал под глазом.'
elseif _finish = 4:
	health -= 40
...
elseif _finish = 5:
	...
	health -= 80
    $_str += '<br>Но <<$enemy[_enemy]>> явно сильнее вас в уличных драках и несколькими ударами умудряется разбить вам нос и поставить огромный фингал под глазом, заставляя признать поражение и умолять о пощаде.'
end
if health < 10:health = 10
horny = 0
manna = iif(_finish >= 4,0,100)
fingal += iif(_finish = 6,10,iif(_finish < 3,0,5))
