...
if $args[0] = 'vor':
	*clr & cla
	minut += 60
	gs 'zz_render','','images/img/spaln/g0', 'Вы легли на кровать, как вдруг услышали какой-то звук в коридоре. Только вы решили встать посмотреть как в комнату вбежали двое в масках, не успели вы рта раскрыть как у вашего горла оказался нож.
	\\\- Только пискни сука, прирежу!/// - прохрипел один из них. Они связали вам руки и один отправился обыскивать квартиру, а второй остался вас сторожить. Он сидел и внимательно вас рассматривал, затем взял нож и начал водить им по вашему телу:
	\\\- Что, страшно?/// - спросил он.
	Решив что лучше вам молчать вы ничего не ответили, а потупив взгляд смотрели в пол.
	\\\- Вижу что страшно,/// - ухмыльнулся он. Затем он начал щупать вас, вы попытались отодвинуться, но нож вновь оказался у вашего горла:
	\\\- Сиди смирно сучка, пока на перо не посадил/// - прохрипел он и продолжил вас ощупывать. Наконец вернулся второй:
	\\\- Все валим отсюда/// - сказал он.
	...
	if vnesh >= 60:
		...
		sex += 1
		rape += 1
		gs 'zz_render','','','\\\- А что, смачная соска, когда еще такую доведется трахнуть/// - сказал первый.'
		act 'Умолять':
			*clr & cla
			minut += 30
			gs 'zz_render','','images/img/spaln/g1.jpg', 'Вы сжались от ужаса.
			\\\- Ну что, соска, вставай на колени и открывай рот, да смотри, коснешься зубами, от уха до уха глотку вскрою/// - сказал второй доставая член.
			Вы начали умолять не трогать вас, но получив оплеуху встали на колени. Он ткнул членом вам в губы, вы нехотя открыли рот и тут же его член оказался у вас во рту, сжав руками вашу голову он начал трахать вас в рот, за тем тоже сделал второй, потом они попытались засунуть вам в рот оба члена, но решив, что их члены будут друг друга касаться, а это по их мнению, даже если они в женском рту, голубизной попахивает, вернулись к первому варианту и продолжили насиловать ваш рот по очереди. Все происходящее должно было вызывать у вас отвращение, что и было первые пару минут, но затем вы с удивлением поймали себя на том, что их руки теперь просто лежат на вашей голове, а все движения вы делаете сами и уже активно работаете языком.
			\\\- Давай раком вставай!/// - скомандовал один.'
			act 'Встать':
				*clr & cla
				picrand = 38
				orgasm += 1
				gs 'zz_render','','images/img/spaln/g2.jpg', 'Вы поднялись и нагнулись один из них встал сзади и приставив член к вашей киске, вошел внутрь.
				\\\- Да она все течет, слышь Серый этой сучке нравится!/// - сказал он.'
				'\\\- Пасть закрой/// - рыкнул на него второй, вставляя член вам в рот. Они продолжили насиловать вас с двух сторон, но вас переполняло желание, вы стояли раком со связанными за спиной руками и двое незнакомцев имели вас. Вы старательно подмахивали одному одновременно обсасывая член второго как вдруг в глазах потемнело, по телу пробежала приятная дрожь и вы забились в оргазме.
				Тот что имел ваш рот выдернул член:
				\\\- Что с этой сукой, у неё что эпилепсия?/// - вскрикнул он.
				\\\- Да, нет, я же говорил что её нравиться, эта сучка кончила!/// - ответил второй.
				Они развязали вам руки:
				\\\- Посмотрим на что она способна с развязанными руками/// - сказал один.'
				act 'Продолжать':xgt'sexdvoe','var'
			end
		end
	else
		gs 'zz_render','','','\\\- Валим я сказал/// - повторил первый. - \\\- И пасть ей закрой/// - добавил он. Второй не стал спорить и взяв белье из вашего шкафа затолкал вам в рот.
		\\\- Если раньше чем через полчаса вылезешь, тебе не жить/// - сказал первый и они ушли.
		Вы не стали выжидать полчаса и начали пытаться освободиться, хорошо что узлы они толком вязать не умели и скоро вам удалось развязаться'
		act 'Оценить ущерб':
			*clr & cla
			gs 'zz_render','','images/pic/sitr'+iif(remsitr = 0,'',2),'Когда все кончилось, вы проходите по квартире посмотреть что пропало.'
			if money > 0:
				money = 0
				gs 'zz_render','','','Они забрали <<money>> рублей которые вы носили с собой.'
			end
			if komp = 1:
				gs 'zz_render','','','Они забрали ноутбук.'
				komp = 0
			end
