...
var qspSlotTitleText = "Save ";
var qspSlotEmptyText = " <span class='qsp-dialog-save-slots-empty'>(empty)</span>";
...
function qspShowSaveSlotsDialog(content)
{
	...
	$("#qsp-dialog-save-slots-name").html((qspSaveSlotsModeOpen == 1 ? "Load game" : "Save game"));
...
function qspGetDefaultSaveSlotHtml(slot, index)
{
	...
	var div = "<div class='qsp-save-slot-" + (active ? "enabled" : "disabled") + " qsp-button'" + 
					(active ? " onclick='javascript:qspCloseSaveSlots(" + (i + 1) + ");'>" : ">") + 
					slotName +
				"</div>";
	return div;
}
...
{
	return "<div class='qsp-action qsp-button'><a id='qsp-click-action-"+ index +"' " + 
